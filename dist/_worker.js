...
buffer/index.js:
   ...
   * @license  MIT
   *)
*/var nn={};const j=new bi,on=nn.DATABASE_URL||"postgresql://neondb_owner:npg_1PBkOmAWRcf2@ep-round-recipe-aqdgkjfj-pooler.c-8.us-east-1.aws.neon.tech/neondb?sslmode=require",B=Cs(on);function es(e){if(!e)return"";const t=e.split(/\s+/);for(let s=1;s<t.length;s++)if(t[s].endsWith("구")&&!t[s].endsWith("시구"))return t[s].replace(/구$/,"");for(let s=1;s<t.length;s++)if(t[s].endsWith("시")&&t[s]!==t[0])return t[s].replace(/(특별시|광역시|특별자치시|시)$/,"");return t[0]?t[0].replace(/(특별자치도|도)$/,""):""}function Ls(e){if(!e)return"";const t=e.split(/\s+/);return t[0]?t[0].replace(/(특별시|광역시|특별자치시|특별자치도|도|시)$/,""):""}function ln(e){return{마사지:"MassageTherapist",헤드스파:"BeautySalon",피부관리:"BeautySalon",헤어:"HairSalon",메이크업:"BeautySalon",왁싱:"BeautySalon",반영구:"BeautySalon",병원:"MedicalBusiness",그외:"LocalBusiness"}[e]||"LocalBusiness"}function dn(e){const t=es(e.address),s=Ls(e.address),r=t||s;return`${e.name} ${r?r+" ":""}${e.category} 추천 | 마이뷰티맵`}function cr(e){const t=es(e.address),s=Ls(e.address),r=Array.isArray(e.tags)&&e.tags.length?e.tags.join("·")+" ":"",a=t||s,n=e.price?` 가격 ${e.price}.`:"",l=e.desc?" "+e.desc.slice(0,40):"";return`${e.name} | ${a?a+" ":""}${e.category} 잘하는 곳. ${r}예약·위치·가격 한눈에 확인!${n}${l}`.slice(0,160)}function cn(e,t){return`${t} ${e} 추천 TOP | 마이뷰티맵`}function pn(e,t,s){return`${t} ${e} 잘하는 곳 ${s}곳 추천! 가격·위치·예약·후기까지 마이뷰티맵에서 한눈에 확인하세요.`}function un(e,t){const s=t;return[`${s} ${e}`,`${s} ${e} 추천`,`${s} ${e} 잘하는 곳`,`${s} ${e} 예약`,`${s} ${e} 가격`,`${s} ${e} 후기`,`${s} ${e} 맛집`,`${s} 뷰티샵`,`${s} 뷰티 추천`,`${e} 추천`].join(",")}function ve(){return new Date(Date.now()+540*60*1e3).toISOString().slice(0,10)}function pr(){return new Date(Date.now()+540*60*1e3-864e5).toISOString().slice(0,10)}function ur(e){const t=(e??"").trim();if(!t)return"";if(/^[A-Za-z0-9_-]{11}$/.test(t))return t;const s=t.match(/(?:youtu\.be\/|youtube\.com\/(?:watch\?(?:.*&)?v=|embed\/|shorts\/|v\/))([A-Za-z0-9_-]{11})/);return s?s[1]:t}function hn(e){return e.plan!=="shoot"||!e.paid_until?!1:new Date(e.paid_until)>=new Date}function Ue(e){return{id:e.id,name:e.name,category:e.category,tags:e.tags??[],price:e.price??"",address:e.address??"",district:e.district??"",lat:parseFloat(e.lat)||37.5326,lng:parseFloat(e.lng)||127.0246,smartPlaceUrl:e.smart_place_url??"",youtubeId:e.youtube_id??"",featured:e.featured??!1,thumbnail:e.thumbnail??"",phone:e.phone??"",desc:e.description??"",active:e.active??!0,displayMode:e.display_mode??"both",plan:e.plan??"basic",paidUntil:e.paid_until??null,paymentStatus:e.payment_status??"unpaid",paymentMemo:e.payment_memo??"",views:parseInt(e.view_cnt)||0,feedSP:parseInt(e.feed_sp)||0,mapSP:parseInt(e.map_sp)||0,isPremium:hn(e),isRecommended:e.is_recommended??!1}}function fn(e,t,s,r){const n=(s-e)*Math.PI/180,l=(r-t)*Math.PI/180,o=Math.sin(n/2)**2+Math.cos(e*Math.PI/180)*Math.cos(s*Math.PI/180)*Math.sin(l/2)**2;return 6371*2*Math.atan2(Math.sqrt(o),Math.sqrt(1-o))}let Ws=!1,zt=null;async function gn(){if(!Ws)return zt||(zt=(async()=>{try{await B`ALTER TABLE shops ADD COLUMN IF NOT EXISTS report_token   TEXT`}catch{}try{await B`ALTER TABLE shops ADD COLUMN IF NOT EXISTS is_recommended BOOLEAN NOT NULL DEFAULT false`}catch{}try{await B`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS feed_view    INTEGER NOT NULL DEFAULT 0`}catch{}try{await B`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS catalog_view INTEGER NOT NULL DEFAULT 0`}catch{}try{await B`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS map_view     INTEGER NOT NULL DEFAULT 0`}catch{}try{await B`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS vts_feed     INTEGER NOT NULL DEFAULT 0`}catch{}try{await B`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS vts_catalog  INTEGER NOT NULL DEFAULT 0`}catch{}try{await B`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS vts_map      INTEGER NOT NULL DEFAULT 0`}catch{}try{await B`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS rec_view     INTEGER NOT NULL DEFAULT 0`}catch{}try{await B`CREATE TABLE IF NOT EXISTS honey_items (
      ...
      )`}catch{}try{await B`ALTER TABLE honey_items ADD COLUMN IF NOT EXISTS coupang_url2 TEXT NOT NULL DEFAULT ''`}catch{}try{await B`CREATE TABLE IF NOT EXISTS shorts_items (
        id               SERIAL PRIMARY KEY,
        ...
        created_at       TIMESTAMPTZ NOT NULL DEFAULT NOW()
      )`;try{await B`ALTER TABLE shorts_items ADD COLUMN IF NOT EXISTS smart_place_url TEXT NOT NULL DEFAULT ''`}catch{}try{await B`ALTER TABLE shorts_items ADD COLUMN IF NOT EXISTS view_cnt INTEGER NOT NULL DEFAULT 0`}catch{}try{await B`ALTER TABLE shorts_items ADD COLUMN IF NOT EXISTS sp_cnt INTEGER NOT NULL DEFAULT 0`}catch{}}catch{}Ws=!0})(),zt)}j.use("*",async(e,t)=>(await gn(),t()));j.get("/api/shops",async e=>{const t=e.req.query("category")??"",s=(e.req.query("q")??"").toLowerCase(),r=parseFloat(e.req.query("lat")??""),a=parseFloat(e.req.query("lng")??""),n=e.req.query("nearby")==="1",l=e.req.query("shuffle")==="1";let u=(await B`
  ...
  `).map(Ue);if(t==="recommended"?u=u.filter(f=>f.isRecommended):t&&t!=="all"&&(u=u.filter(f=>f.category===t)),s&&(u=u.filter(f=>f.name.toLowerCase().includes(s)||(f.tags||[]).some(g=>g.toLowerCase().includes(s))||f.district.toLowerCase().includes(s))),n&&!isNaN(r)&&!isNaN(a))u=u.map(f=>({...f,dist:fn(r,a,f.lat,f.lng)})).filter(f=>f.dist<=20).sort((f,g)=>f.isPremium&&!g.isPremium?-1:!f.isPremium&&g.isPremium?1:f.dist-g.dist);else if(l){const f=u.filter(x=>x.isPremium),g=u.filter(x=>!x.isPremium);for(let x=f.length-1;x>0;x--){const v=Math.floor(Math.random()*(x+1));[f[x],f[v]]=[f[v],f[x]]}for(let x=g.length-1;x>0;x--){const v=Math.floor(Math.random()*(x+1));[g[x],g[v]]=[g[v],g[x]]}u=[...f,...g]}else u.sort((f,g)=>f.isPremium&&!g.isPremium?-1:!f.isPremium&&g.isPremium?1:0);return e.json(u)});j.get("/api/geocode",async e=>{var o,u,f,g,x;const t=e.req.query("query")??"";if(!t)return e.json({error:"query required"},400);const s=v=>v.replace(/(로|길|번길)\s+(\d)/g,"$1$2"),r=[],a=s(t);a!==t&&r.push(a),r.push(t);const n=a.trim().split(/\s+/);n.length>3&&r.push(n.slice(0,-1).join(" "));const l=async v=>(await fetch(`https://maps.apigw.ntruss.com/map-geocode/v2/geocode?query=${encodeURIComponent(v)}`,{headers:{"X-NCP-APIGW-API-KEY-ID":"xjjg4490h8","X-NCP-APIGW-API-KEY":"RB4DFA4ZEF2iHtkerlNrqzoG8P8YHwE2UddGrAtD"}})).json();try{let v=null;for(const E of r){const S=await l(E);if((o=S.addresses)!=null&&o.length){v=S.addresses[0];break}}if(!v)return e.json({error:"주소를 찾을 수 없어요"},404);const y=v.addressElements||[],d=((u=y.find(E=>{var S;return(S=E.types)==null?void 0:S.includes("SIDO")}))==null?void 0:u.longName)||"",p=((f=y.find(E=>{var S;return(S=E.types)==null?void 0:S.includes("SIGUGUN")}))==null?void 0:f.longName)||"",b=((g=y.find(E=>{var S,O;return((S=E.types)==null?void 0:S.includes("DONGMYUN"))||((O=E.types)==null?void 0:O.includes("RI"))}))==null?void 0:g.longName)||"",w=[d,p,b].filter(Boolean).join(" ")||((x=v.roadAddress)==null?void 0:x.split(" ").slice(0,3).join(" "))||"";return e.json({lat:parseFloat(v.y),lng:parseFloat(v.x),address:v.roadAddress||v.jibunAddress,district:w})}catch{return e.json({error:"지오코딩 실패"},500)}});j.get("/api/shops/all",async e=>{const t=await B`
    SELECT s.*, COALESCE(st.view_cnt,0) as view_cnt,
           COALESCE(st.feed_sp,0) as feed_sp, COALESCE(st.map_sp,0) as map_sp
    FROM shops s LEFT JOIN stats st ON s.id = st.shop_id
  ...
  `;return s.length?e.json(Ue(s[0])):e.json({error:"not found"},404)});j.post("/api/admin/shops",async e=>{const t=await e.req.json(),s=Array.isArray(t.tags)?t.tags:(t.tags??"").split(",").map(n=>n.trim()).filter(Boolean),a=(await B`
    ...
    RETURNING *
  `)[0];return await B`INSERT INTO stats (shop_id) VALUES (${a.id}) ON CONFLICT DO NOTHING`,e.json(Ue(a))});j.put("/api/admin/shops/:id",async e=>{const t=+e.req.param("id"),s=await e.req.json(),r=Array.isArray(s.tags)?s.tags:(s.tags??"").split(",").map(n=>n.trim()).filter(Boolean),a=await B`
    UPDATE shops SET
      name            = ${s.name},
      category        = ${s.category},
      tags            = ${r},
    ...
    WHERE id = ${t}
    RETURNING *
  `;return a.length?(await B`INSERT INTO stats (shop_id) VALUES (${t}) ON CONFLICT DO NOTHING`,e.json(Ue(a[0]))):e.json({error:"not found"},404)});j.put("/api/admin/shops/:id/recommended",async e=>{const t=+e.req.param("id"),{isRecommended:s}=await e.req.json(),r=await B`
  ...
  `;return r.length?e.json({ok:!0,isRecommended:r[0].is_recommended}):e.json({error:"not found"},404)});j.post("/api/track/rec/:id",async e=>{const t=+e.req.param("id"),s=ve();try{await B`INSERT INTO daily_stats (shop_id, stat_date, rec_view)
    ...
    WHERE id = ${t}
    RETURNING *
  `;return r.length?e.json(Ue(r[0])):e.json({error:"not found"},404)});j.delete("/api/admin/shops/:id",async e=>{const t=+e.req.param("id");return await B`DELETE FROM shops WHERE id = ${t}`,e.json({ok:!0})});j.post("/api/track/view/:id",async e=>{const t=+e.req.param("id"),s=ve();let r="feed";try{const a=await e.req.json();["feed","catalog","map"].includes(a==null?void 0:a.source)&&(r=a.source)}catch{}await B`INSERT INTO stats (shop_id, view_cnt) VALUES (${t}, 1)
      ON CONFLICT (shop_id) DO UPDATE SET view_cnt = stats.view_cnt + 1`;try{await B`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS feed_view    INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS catalog_view INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS map_view     INTEGER NOT NULL DEFAULT 0`}catch{}return r==="feed"?await B`INSERT INTO daily_stats (shop_id, stat_date, view_cnt, feed_view)
                VALUES (${t}, ${s}, 1, 1)
              ON CONFLICT (shop_id, stat_date) DO UPDATE
              ...
              ON CONFLICT (shop_id, stat_date) DO UPDATE
                SET view_cnt  = daily_stats.view_cnt  + 1,
                    map_view  = daily_stats.map_view  + 1`,e.json({ok:!0})});j.post("/api/track/sp/:id",async e=>{const t=+e.req.param("id"),s=ve();let r=null;try{const a=await e.req.json();r=(a==null?void 0:a.viewSrc)||null}catch{}try{await B`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS vts_feed    INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS vts_catalog INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS vts_map     INTEGER NOT NULL DEFAULT 0`}catch{}return await B`INSERT INTO stats (shop_id, feed_sp) VALUES (${t}, 1)
      ON CONFLICT (shop_id) DO UPDATE SET feed_sp = stats.feed_sp + 1`,r==="feed"?await B`INSERT INTO daily_stats (shop_id, stat_date, feed_sp, vts_feed)
              ...
              ON CONFLICT (shop_id, stat_date) DO UPDATE
                ...
                VALUES (${t}, ${s}, 1)
              ON CONFLICT (shop_id, stat_date) DO UPDATE
                SET feed_sp = daily_stats.feed_sp + 1`,e.json({ok:!0})});j.post("/api/track/mapsp/:id",async e=>{const t=+e.req.param("id"),s=ve();let r=null;try{const a=await e.req.json();r=(a==null?void 0:a.viewSrc)||null}catch{}try{await B`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS vts_feed    INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS vts_catalog INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS vts_map     INTEGER NOT NULL DEFAULT 0`}catch{}return await B`INSERT INTO stats (shop_id, map_sp) VALUES (${t}, 1)
  ...
  `,e.json({ok:!0})});j.get("/api/admin/daily-visits",async e=>{const t=await B`
    ...
    LIMIT 30
  `;return e.json(t)});j.post("/api/admin/reset-stats",async e=>(await B`UPDATE stats SET view_cnt=0, feed_sp=0, map_sp=0`,await B`DELETE FROM daily_visits`,await B`DELETE FROM daily_stats`,e.json({ok:!0})));j.post("/api/admin/init-daily-stats",async e=>(await B`
  ...
  `,await B`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS feed_view    INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS catalog_view INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS map_view     INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS vts_feed     INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS vts_catalog  INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS vts_map      INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS rec_view     INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE shops ADD COLUMN IF NOT EXISTS report_token   TEXT`,await B`ALTER TABLE shops ADD COLUMN IF NOT EXISTS is_recommended BOOLEAN NOT NULL DEFAULT false`,e.json({ok:!0})));j.get("/api/admin/stats",async e=>{const t=ve(),s=pr();await B`
    CREATE TABLE IF NOT EXISTS daily_stats (
      shop_id      INTEGER NOT NULL,
      stat_date    DATE    NOT NULL,
      view_cnt     INTEGER NOT NULL DEFAULT 0,
  ...
  `;try{await B`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS feed_view    INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS catalog_view INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS map_view     INTEGER NOT NULL DEFAULT 0`}catch{}const r=await B`
  ...
  `,u={};o.forEach(b=>{u[b.shop_id]=parseInt(b.total_rec)||0});const f=await B`
    SELECT
      SUM(view_cnt) as total_views,
      SUM(feed_sp)  as total_feed_sp,
      SUM(map_sp)   as total_map_sp
    FROM stats
  `,g=await B`SELECT COUNT(*) as cnt FROM shops WHERE active = true`,x=f[0]||{},v=a[0]||{},y=n[0]||{},d=await B`
    SELECT shop_id,
  ...
  `,e.json({ok:!0}))});j.get("/api/admin/calendar",async e=>{const t=parseInt(e.req.query("year")||String(new Date().getFullYear())),s=parseInt(e.req.query("month")||String(new Date().getMonth()+1)),r=`${t}-${String(s).padStart(2,"0")}-01`,a=new Date(t,s,0).toISOString().slice(0,10),n=await B`
    SELECT stat_date::text as stat_date,
           COALESCE(SUM(view_cnt),0) as views,
           COALESCE(SUM(feed_sp),0)  as feed_sp,
           COALESCE(SUM(map_sp),0)   as map_sp,
  ...
  `,x={};g.forEach(y=>{x[y.visit_date]=parseInt(y.visit_cnt)||0});const v=g.reduce((y,d)=>y+(parseInt(d.visit_cnt)||0),0);return e.json({year:t,month:s,monthTotal:{views:parseInt(f.views)||0,feedSP:parseInt(f.feed_sp)||0,mapSP:parseInt(f.map_sp)||0,visits:v},daily:n.map(y=>({date:y.stat_date,visits:x[y.stat_date]||0,views:parseInt(y.views)||0,feedSP:parseInt(y.feed_sp)||0,mapSP:parseInt(y.map_sp)||0,activeShops:parseInt(y.active_shops)||0})),shopDetail:o.map(y=>({id:y.id,name:y.name,category:y.category,thumbnail:y.thumbnail,views:parseInt(y.views)||0,feedSP:parseInt(y.feed_sp)||0,mapSP:parseInt(y.map_sp)||0}))})});j.get("/api/admin/inquiries",async e=>{const t=await B`SELECT * FROM inquiries ORDER BY created_at DESC`;return e.json(t)});j.get("/favicon.ico",e=>hr());j.get("/favicon.svg",e=>hr());function hr(e){return new Response('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32"><rect width="32" height="32" rx="8" fill="#FF4D7D"/><text x="16" y="23" font-size="18" text-anchor="middle">💄</text></svg>',{headers:{"Content-Type":"image/svg+xml","Cache-Control":"public,max-age=86400"}})}j.get("/og-image.jpg",e=>{try{const t=xr.join(process.cwd(),"public","og-image.jpg"),s=yr.readFileSync(t);return new Response(s,{headers:{"Content-Type":"image/jpeg","Cache-Control":"public,max-age=86400"}})}catch{return e.notFound()}});j.post("/api/admin/upload-thumbnail",async e=>{const t=await e.req.json(),{shopId:s,dataUrl:r}=t;return!r||!s?e.json({error:"required"},400):(await B`UPDATE shops SET thumbnail = ${r} WHERE id = ${s}`,e.json({ok:!0,url:r}))});j.get("/api/shorts",async e=>{const t=await B`
    SELECT * FROM shorts_items WHERE active = true ORDER BY sort_order ASC, id DESC
  `;return e.json(t)});j.get("/api/admin/shorts",async e=>{const t=await B`SELECT * FROM shorts_items ORDER BY sort_order ASC, id DESC`;return e.json(t)});j.post("/api/admin/shorts",async e=>{const t=await e.req.json(),s=await B`
    ...
    RETURNING *
  `;return e.json(s[0])});j.put("/api/admin/shorts/:id",async e=>{const t=+e.req.param("id"),s=await e.req.json(),r=await B`
    UPDATE shorts_items SET
      ...
      active          = ${s.active!==!1}
    WHERE id = ${t} RETURNING *
  `;return e.json(r[0])});j.delete("/api/admin/shorts/:id",async e=>{const t=+e.req.param("id");return await B`DELETE FROM shorts_items WHERE id = ${t}`,e.json({ok:!0})});async function ut(){await B`
  ...
  `}j.post("/api/track/shorts/view/:id",async e=>{const t=+e.req.param("id"),s=ve();await B`UPDATE shorts_items SET view_cnt = view_cnt + 1 WHERE id = ${t}`;try{await ut(),await B`
    ...
    `}catch{}return e.json({ok:!0})});j.post("/api/track/shorts/sp/:id",async e=>{const t=+e.req.param("id"),s=ve();await B`UPDATE shorts_items SET sp_cnt = sp_cnt + 1 WHERE id = ${t}`;try{await ut(),await B`
      INSERT INTO shorts_daily_stats (shorts_id, stat_date, sp_cnt)
      VALUES (${t}, ${s}, 1)
      ON CONFLICT (shorts_id, stat_date)
      DO UPDATE SET sp_cnt = shorts_daily_stats.sp_cnt + 1
    `}catch{}return e.json({ok:!0})});j.get("/api/admin/shorts/stats/summary",async e=>{const t=ve(),s=pr(),r=new Date(Date.now()-6*864e5).toISOString().slice(0,10);try{await ut();const[a]=await B`
    ...
    `;return e.json(s)}catch(s){return e.json({error:String(s)},500)}});j.get("/api/admin/shorts/stats/daily",async e=>{const t=new Date(Date.now()-25056e5).toISOString().slice(0,10);try{await ut();const s=await B`
      ...
      GROUP BY stat_date
      ORDER BY stat_date ASC
    `;return e.json(s)}catch(s){return e.json({error:String(s)},500)}});j.get("/api/admin/shorts/stats/item/:id",async e=>{const t=+e.req.param("id"),s=new Date(Date.now()-29*864e5).toISOString().slice(0,10);try{await ut();const r=await B`
      SELECT stat_date::text as date, view_cnt as views, sp_cnt as sp
      FROM shorts_daily_stats
      WHERE shorts_id = ${t} AND stat_date >= ${s}
      ORDER BY stat_date ASC
    `;return e.json(r)}catch(r){return e.json({error:String(r)},500)}});async function ts(){await B`
  ...
  `}j.post("/api/track/session/start",async e=>{try{await ts(),await B`DELETE FROM visitor_sessions WHERE entered_at < NOW() - INTERVAL '7 days'`;const t=await e.req.json(),s=t.id||"",r=t.device||"unknown";return s?(await B`
      INSERT INTO visitor_sessions (id, device)
      VALUES (${s}, ${r})
      ON CONFLICT (id) DO UPDATE SET last_seen = NOW()
    `,e.json({ok:!0})):e.json({ok:!1})}catch(t){return e.json({ok:!1,error:String(t)})}});j.post("/api/track/session/update",async e=>{try{await ts();const t=await e.req.json(),{id:s,duration_sec:r,tabs_visited:a,shorts_count:n,book_count:l,exited:o}=t;return s?(await B`
      UPDATE visitor_sessions SET
        ...
        exited       = ${o||!1}
      WHERE id = ${s}
    `,e.json({ok:!0})):e.json({ok:!1})}catch{return e.json({ok:!1})}});j.get("/api/admin/sessions",async e=>{try{await ts();const t=ve(),s=await B`
      ...
      LIMIT 200
    `;return e.json(s)}catch{return e.json([])}});j.get("/api/admin/sessions/summary",async e=>{try{await ts();const t=ve(),[s]=await B`
      SELECT
        COUNT(*)                                        as total,
...
`)}
</urlset>`;return e.text(w,200,{"Content-Type":"application/xml; charset=utf-8"})});j.get("/shop/:id",async e=>{const t=e.req.header("x-forwarded-proto")||"https",s=e.req.header("x-forwarded-host")||e.req.header("host")||"www.mybeautymap.co.kr",r=`${t}://${s}`,a=+e.req.param("id");if(isNaN(a))return e.redirect("/");const n=await B`
    SELECT s.*, COALESCE(st.view_cnt,0) as view_cnt,
           COALESCE(st.feed_sp,0) as feed_sp
    FROM shops s LEFT JOIN stats st ON s.id = st.shop_id
    WHERE s.id = ${a} AND s.active = true
  `;if(!n.length)return e.redirect("/");const l=Ue(n[0]);return e.html(vn(l,r))});j.get("/c/:category/:region",async e=>{const t=e.req.header("x-forwarded-proto")||"https",s=e.req.header("x-forwarded-host")||e.req.header("host")||"www.mybeautymap.co.kr",r=`${t}://${s}`,a=decodeURIComponent(e.req.param("category")),n=decodeURIComponent(e.req.param("region")),o=(await B`
    SELECT s.*, COALESCE(st.view_cnt,0) as view_cnt,
           COALESCE(st.feed_sp,0) as feed_sp
    FROM shops s LEFT JOIN stats st ON s.id = st.shop_id
  ...
  `,f=await B`
    ...
    GROUP BY s.id
    ORDER BY total DESC
  `,g=f.findIndex(te=>te.id===a.id)+1,x=f.length,v=await B`
    SELECT COALESCE(view_cnt,0) as views,
           COALESCE(feed_sp,0)  as feed_sp,
           COALESCE(map_sp,0)   as map_sp
    FROM stats WHERE shop_id = ${a.id}
  `;return e.json({shop:{id:a.id,name:a.name,category:a.category,address:a.address},thisMonth:{views:parseInt((y=l[0])==null?void 0:y.views)||0,feedSP:parseInt((d=l[0])==null?void 0:d.feed_sp)||0,mapSP:parseInt((p=l[0])==null?void 0:p.map_sp)||0,feedView:parseInt((b=l[0])==null?void 0:b.feed_view)||0,catalogView:parseInt((w=l[0])==null?void 0:w.catalog_view)||0,mapView:parseInt((E=l[0])==null?void 0:E.map_view)||0,vtsFeed:parseInt((S=l[0])==null?void 0:S.vts_feed)||0,vtsCatalog:parseInt((O=l[0])==null?void 0:O.vts_catalog)||0,vtsMap:parseInt((M=l[0])==null?void 0:M.vts_map)||0},lastMonth:{views:parseInt((_=o[0])==null?void 0:_.views)||0,feedSP:parseInt((C=o[0])==null?void 0:C.feed_sp)||0,mapSP:parseInt((L=o[0])==null?void 0:L.map_sp)||0,feedView:parseInt((k=o[0])==null?void 0:k.feed_view)||0,catalogView:parseInt((R=o[0])==null?void 0:R.catalog_view)||0,mapView:parseInt((A=o[0])==null?void 0:A.map_view)||0,vtsFeed:parseInt((z=o[0])==null?void 0:z.vts_feed)||0,vtsCatalog:parseInt(($=o[0])==null?void 0:$.vts_catalog)||0,vtsMap:parseInt((P=o[0])==null?void 0:P.vts_map)||0},total:{views:parseInt((U=v[0])==null?void 0:U.views)||0,feedSP:parseInt((V=v[0])==null?void 0:V.feed_sp)||0,mapSP:parseInt((Y=v[0])==null?void 0:Y.map_sp)||0},daily30:u,rank:g,rankTotal:x})});j.get("/report/:token",e=>{const t=e.req.param("token");return e.html(wn(t))});j.get("/admin",e=>e.html(En()));j.get("/map-admin",e=>e.redirect("/admin"));j.get("/map",e=>e.html(xn()));j.get("/api/resolve-naver",async e=>{const t=e.req.query("url")||"";if(!t)return e.json({error:"no url"},400);try{const s=t.match(/place\/([0-9]+)/);if(s)return e.json({resolved:`https://m.place.naver.com/place/${s[1]}/home`});const n=((await fetch(t,{method:"HEAD",redirect:"manual",headers:{"User-Agent":"Mozilla/5.0 (compatible; bot)"}})).headers.get("location")||"").match(/place\/([0-9]+)/);return n?e.json({resolved:`https://m.place.naver.com/place/${n[1]}/home`}):e.json({resolved:t})}catch{return e.json({resolved:t})}});j.get("/reserve",e=>{const t=e.req.query("url")||"",s=e.req.query("name")||"";return t?e.html(`<!DOCTYPE html>
<html lang="ko">
<head>
<meta charset="UTF-8">
...
</html>`):e.text("url required",400)});j.get("/",e=>{const t=e.req.header("x-forwarded-proto")||"https",s=e.req.header("x-forwarded-host")||e.req.header("host")||"www.mybeautymap.co.kr",r=`${t}://${s}`;return e.html(bn(r))});const Qs=["전체","마사지","헤드스파","피부관리","헤어","메이크업","왁싱","반영구","병원","그외"],Gs={전체:"🏠",마사지:"💆",헤드스파:"🧖",피부관리:"✨",헤어:"💇",메이크업:"💄",왁싱:"🌸",반영구:"👁",병원:"🏥",그외:"🌟"},mn=`<button class="cp cp-rec" onclick="filterFeed(this,'recommended')">⭐ 추천</button>`;function bn(e="https://www.mybeautymap.co.kr"){return`<!DOCTYPE html>
<html lang="ko">
<head>
<meta charset="UTF-8"/>
...
<div class="cat-bar" id="catBar">
  <div class="cat-scroll">
    ${Qs.map((t,s)=>`<button class="cp${s===0?" active":""}" onclick="filterFeed(this,'${t==="전체"?"all":t}')">${Gs[t]} ${t}</button>${s===0?mn:""}`).join("")}
  </div>
</div>
...
<section id="mapScreen">
  ...
  <div class="map-cat-bar" id="mapCatBar">
    ${Qs.map((t,s)=>`<button class="mc${s===0?" active":""}" onclick="filterMap(this,'${t==="전체"?"all":t}')">${Gs[t]} ${t}</button>`).join("")}
  </div>
  <!-- 지도 iframe -->
...
</html>`}function yn(e,t,s,r){const a=cn(e,t),n=pn(e,t,s.length),l=un(e,t),o=`${r}/c/${encodeURIComponent(e)}/${encodeURIComponent(t)}`,u={마사지:"💆",헤드스파:"🧖",피부관리:"✨",헤어:"💇",메이크업:"💄",왁싱:"🌸",반영구:"👁",병원:"🏥",그외:"🌟"},f=u[e]||"🌟",g=["마사지","헤드스파","피부관리","헤어","메이크업","왁싱","반영구","병원","그외"],x=JSON.stringify({"@context":"https://schema.org","@type":"ItemList",name:a,description:n,url:o,numberOfItems:s.length,itemListElement:s.map((y,d)=>({"@type":"ListItem",position:d+1,name:y.name,url:`${r}/shop/${y.id}`,description:cr(y)}))}),v=JSON.stringify({"@context":"https://schema.org","@type":"BreadcrumbList",itemListElement:[{"@type":"ListItem",position:1,name:"마이뷰티맵",item:r},{"@type":"ListItem",position:2,name:`${t} ${e}`,item:o}]});return`<!DOCTYPE html>
<html lang="ko">
<head>
<meta charset="UTF-8"/>
<meta name="viewport" content="width=device-width,initial-scale=1.0,maximum-scale=1.0,user-scalable=no"/>
...
</html>`}function wn(e){return`<!DOCTYPE html>
<html lang="ko">
<head>
<meta charset="UTF-8"/>
<meta name="viewport" content="width=device-width,initial-scale=1.0,maximum-scale=1.0,user-scalable=no"/>
...
</html>`}const Ys=new bi,Sn=Object.assign({"/src/index.tsx":j});let fr=!1;for(const[,e]of Object.entries(Sn))e&&(Ys.route("/",e),Ys.notFound(e.notFoundHandler),fr=!0);if(!fr)throw new Error("Can't import modules from ['/src/index.tsx','/app/server.ts']");export{Ys as default};
