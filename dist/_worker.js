...
*/var un={};const j=new _i,fn=un.DATABASE_URL||"postgresql://neondb_owner:npg_1PBkOmAWRcf2@ep-round-recipe-aqdgkjfj-pooler.c-8.us-east-1.aws.neon.tech/neondb?sslmode=require",B=As(fn);function ss(t){if(!t)return"";const e=t.split(/\s+/);for(let s=1;s<e.length;s++)if(e[s].endsWith("구")&&!e[s].endsWith("시구"))return e[s].replace(/구$/,"");for(let s=1;s<e.length;s++)if(e[s].endsWith("시")&&e[s]!==e[0])return e[s].replace(/(특별시|광역시|특별자치시|시)$/,"");return e[0]?e[0].replace(/(특별자치도|도)$/,""):""}function Os(t){if(!t)return"";const e=t.split(/\s+/);return e[0]?e[0].replace(/(특별시|광역시|특별자치시|특별자치도|도|시)$/,""):""}function hn(t){return{마사지:"MassageTherapist",헤드스파:"BeautySalon",피부관리:"BeautySalon",헤어:"HairSalon",메이크업:"BeautySalon",왁싱:"BeautySalon",반영구:"BeautySalon",병원:"MedicalBusiness",그외:"LocalBusiness"}[t]||"LocalBusiness"}function gn(t){const e=ss(t.address),s=Os(t.address),i=e||s;return`${t.name} ${i?i+" ":""}${t.category} 추천 | 마이뷰티맵`}function gr(t){const e=ss(t.address),s=Os(t.address),i=Array.isArray(t.tags)&&t.tags.length?t.tags.join("·")+" ":"",a=e||s,n=t.price?` 가격 ${t.price}.`:"",l=t.desc?" "+t.desc.slice(0,40):"";return`${t.name} | ${a?a+" ":""}${t.category} 잘하는 곳. ${i}예약·위치·가격 한눈에 확인!${n}${l}`.slice(0,160)}function mn(t,e){return`${e} ${t} 추천 TOP | 마이뷰티맵`}function bn(t,e,s){return`${e} ${t} 잘하는 곳 ${s}곳 추천! 가격·위치·예약·후기까지 마이뷰티맵에서 한눈에 확인하세요.`}function vn(t,e){const s=e;return[`${s} ${t}`,`${s} ${t} 추천`,`${s} ${t} 잘하는 곳`,`${s} ${t} 예약`,`${s} ${t} 가격`,`${s} ${t} 후기`,`${s} ${t} 맛집`,`${s} 뷰티샵`,`${s} 뷰티 추천`,`${t} 추천`].join(",")}function ve(){return new Date(Date.now()+540*60*1e3).toISOString().slice(0,10)}function mr(){return new Date(Date.now()+540*60*1e3-864e5).toISOString().slice(0,10)}function br(t){const e=(t??"").trim();if(!e)return"";if(/^[A-Za-z0-9_-]{11}$/.test(e))return e;const s=e.match(/(?:youtu\.be\/|youtube\.com\/(?:watch\?(?:.*&)?v=|embed\/|shorts\/|v\/))([A-Za-z0-9_-]{11})/);return s?s[1]:e}function yn(t){return t.plan!=="shoot"||!t.paid_until?!1:new Date(t.paid_until)>=new Date}function Ue(t){return{id:t.id,name:t.name,category:t.category,tags:t.tags??[],price:t.price??"",address:t.address??"",district:t.district??"",lat:parseFloat(t.lat)||37.5326,lng:parseFloat(t.lng)||127.0246,smartPlaceUrl:t.smart_place_url??"",youtubeId:t.youtube_id??"",featured:t.featured??!1,thumbnail:t.thumbnail??"",phone:t.phone??"",desc:t.description??"",active:t.active??!0,displayMode:t.display_mode??"both",plan:t.plan??"basic",paidUntil:t.paid_until??null,paymentStatus:t.payment_status??"unpaid",paymentMemo:t.payment_memo??"",views:parseInt(t.view_cnt)||0,feedSP:parseInt(t.feed_sp)||0,mapSP:parseInt(t.map_sp)||0,isPremium:yn(t),isRecommended:t.is_recommended??!1}}function xn(t,e,s,i){const n=(s-t)*Math.PI/180,l=(i-e)*Math.PI/180,o=Math.sin(n/2)**2+Math.cos(t*Math.PI/180)*Math.cos(s*Math.PI/180)*Math.sin(l/2)**2;return 6371*2*Math.atan2(Math.sqrt(o),Math.sqrt(1-o))}const vs=10;let Wt=!1,it=null;async function vr(){if(!Wt)return it||(it=(async()=>{try{if(await B`
      ...
      `,await B`CREATE INDEX IF NOT EXISTS idx_se_session  ON session_events(session_id)`,await B`CREATE INDEX IF NOT EXISTS idx_se_occurred ON session_events(occurred_at)`,await B`ALTER TABLE shops             ADD COLUMN IF NOT EXISTS report_token   TEXT`,await B`ALTER TABLE shops             ADD COLUMN IF NOT EXISTS is_recommended BOOLEAN NOT NULL DEFAULT false`,await B`ALTER TABLE daily_stats       ADD COLUMN IF NOT EXISTS feed_view      INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats       ADD COLUMN IF NOT EXISTS catalog_view   INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats       ADD COLUMN IF NOT EXISTS map_view       INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats       ADD COLUMN IF NOT EXISTS vts_feed       INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats       ADD COLUMN IF NOT EXISTS vts_catalog    INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats       ADD COLUMN IF NOT EXISTS vts_map        INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats       ADD COLUMN IF NOT EXISTS rec_view       INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE honey_items       ADD COLUMN IF NOT EXISTS coupang_url2   TEXT    NOT NULL DEFAULT ''`,await B`ALTER TABLE shorts_items      ADD COLUMN IF NOT EXISTS smart_place_url TEXT   NOT NULL DEFAULT ''`,await B`ALTER TABLE shorts_items      ADD COLUMN IF NOT EXISTS view_cnt        INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE shorts_items      ADD COLUMN IF NOT EXISTS sp_cnt          INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE session_events    ADD COLUMN IF NOT EXISTS viewed_sec      INTEGER NOT NULL DEFAULT 0`,await B`INSERT INTO schema_migrations (version) VALUES (${vs}) ON CONFLICT DO NOTHING`,Wt=!0}catch(t){console.error("[migration] error:",t),it=null}})(),it)}j.use("*",async(t,e)=>(await vr(),e()));j.get("/api/shops",async t=>{const e=t.req.query("category")??"",s=(t.req.query("q")??"").toLowerCase(),i=parseFloat(t.req.query("lat")??""),a=parseFloat(t.req.query("lng")??""),n=t.req.query("nearby")==="1",l=t.req.query("shuffle")==="1";let p=(await B`
  ...
  `,t.json({ok:!0})});j.get("/api/admin/daily-visits",async t=>{const e=await B`
    ...
    LIMIT 30
  `;return t.json(e)});j.post("/api/admin/reset-stats",async t=>(await B`UPDATE stats SET view_cnt=0, feed_sp=0, map_sp=0`,await B`DELETE FROM daily_visits`,await B`DELETE FROM daily_stats`,t.json({ok:!0})));j.post("/api/admin/init-daily-stats",async t=>(Wt=!1,it=null,await B`DELETE FROM schema_migrations WHERE version = ${vs}`,await vr(),t.json({ok:!0})));j.get("/api/admin/stats",async t=>{const e=ve(),s=mr(),i=await B`
  ...
  `;return t.json(s[0])});j.put("/api/admin/shorts/:id",async t=>{const e=+t.req.param("id"),s=await t.req.json(),i=await B`
    UPDATE shorts_items SET
      ...
      active               = ${s.active!==!1}
    WHERE id = ${e} RETURNING *
  `;return t.json(i[0])});j.delete("/api/admin/shorts/:id",async t=>{const e=+t.req.param("id");return await B`DELETE FROM shorts_items WHERE id = ${e}`,t.json({ok:!0})});let Ys=!1;async function ft(){Ys||(await B`
    CREATE TABLE IF NOT EXISTS shorts_daily_stats (
      shorts_id  INTEGER NOT NULL,
      stat_date  DATE    NOT NULL,
      view_cnt   INTEGER NOT NULL DEFAULT 0,
  ...
  `,Ys=!0)}j.post("/api/track/shorts/view/:id",async t=>{const e=+t.req.param("id"),s=ve();await B`UPDATE shorts_items SET view_cnt = view_cnt + 1 WHERE id = ${e}`;try{await ft(),await B`
    ...
    `}catch{}return t.json({ok:!0})});j.get("/api/admin/shorts/stats/summary",async t=>{const e=ve(),s=mr(),i=new Date(Date.now()-6*864e5).toISOString().slice(0,10);try{await ft();const[a]=await B`
    ...
    `;return t.json(s)}catch(s){return t.json({error:String(s)},500)}});j.get("/api/admin/shorts/stats/item/:id",async t=>{const e=+t.req.param("id"),s=new Date(Date.now()-29*864e5).toISOString().slice(0,10);try{await ft();const i=await B`
      ...
      ORDER BY stat_date ASC
    `;return t.json(i)}catch(i){return t.json({error:String(i)},500)}});let Xs=!1;async function Nt(){Xs||(await B`
    CREATE TABLE IF NOT EXISTS visitor_sessions (
      id            TEXT    PRIMARY KEY,
      entered_at    TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      last_seen     TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  ...
  `,Xs=!0)}j.post("/api/track/session/start",async t=>{try{await Nt(),await B`DELETE FROM visitor_sessions WHERE entered_at < NOW() - INTERVAL '7 days'`;const e=await t.req.json(),s=e.id||"",i=e.device||"unknown";return s?(await B`
    ...
    `,t.json({ok:!0})):t.json({ok:!1})}catch(e){return t.json({ok:!1,error:String(e)})}});j.post("/api/track/session/update",async t=>{try{await Nt();const e=await t.req.json(),{id:s,duration_sec:i,tabs_visited:a,exited:n,shorts_count:l,shorts_book:o,feed_card_cnt:p,feed_book_cnt:f,map_pin_cnt:g,map_book_cnt:x,search_cnt:b,inquiry_cnt:y}=e;if(!s)return t.json({ok:!1});const d=(o||0)+(f||0)+(x||0);return await B`
      UPDATE visitor_sessions SET
        ...
        exited        = ${n||!1}
      WHERE id = ${s}
    `,t.json({ok:!0})}catch{return t.json({ok:!1})}});let Ks=!1;async function Ms(){Ks||(await B`
    CREATE TABLE IF NOT EXISTS session_events (
      id          BIGSERIAL PRIMARY KEY,
      session_id  TEXT        NOT NULL,
      occurred_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  ...
  `,await B`CREATE INDEX IF NOT EXISTS idx_se_session ON session_events(session_id)`,await B`CREATE INDEX IF NOT EXISTS idx_se_occurred ON session_events(occurred_at)`,Ks=!0)}j.post("/api/track/session/event",async t=>{try{await Ms();const e=await t.req.json(),{session_id:s,event_type:i,shop_id:a,shop_name:n,shop_cat:l,viewed_sec:o}=e;return!s||!i?t.json({ok:!1}):(await B`DELETE FROM session_events WHERE occurred_at < NOW() - INTERVAL '7 days'`,await B`
    ...
    `,t.json({ok:!0}))}catch(e){return t.json({ok:!1,error:String(e)})}});j.get("/api/admin/sessions/:sid/events",async t=>{try{await Ms();const e=t.req.param("sid"),s=await B`
      SELECT id, occurred_at, event_type, shop_id, shop_name, shop_cat
      FROM session_events
      WHERE session_id = ${e}
      ORDER BY occurred_at ASC
      LIMIT 200
    `;return t.json(s)}catch{return t.json([])}});j.get("/api/admin/shop-ranking",async t=>{try{await Ms();const e=Number(t.req.query("days")||7),s=await B`
      SELECT
        shop_id,
        shop_name,
        shop_cat,
...
Sitemap: ${e}://${s}/sitemap.xml`,200,{"Content-Type":"text/plain; charset=utf-8"})});j.get("/ads.txt",t=>t.text("google.com, pub-6943282483618134, DIRECT, f08c47fec0942fa0",200,{"Content-Type":"text/plain; charset=utf-8"}));j.get("/sitemap.xml",async t=>{const e=t.req.header("x-forwarded-proto")||"https",s=t.req.header("x-forwarded-host")||t.req.header("host")||"www.mybeautymap.co.kr",i=`${e}://${s}`,a=ve(),n=await B`SELECT id, name, category, address FROM shops WHERE active = true ORDER BY id`,l=new Set;for(const _ of n){const E=ss(_.address);E&&l.add(`${_.category}|||${E}`)}const o=[{loc:i,priority:"1.0",freq:"daily"},{loc:`${i}/map`,priority:"0.8",freq:"weekly"}],p=n.map(_=>({loc:`${i}/shop/${_.id}`,priority:"0.9",freq:"weekly",lastmod:a})),f=[...l].map(_=>{const[E,O]=_.split("|||");return{loc:`${i}/c/${encodeURIComponent(E)}/${encodeURIComponent(O)}`,priority:"0.8",freq:"weekly",lastmod:a}}),g=["마사지","헤드스파","피부관리","헤어","메이크업","왁싱","반영구"],x=["강남구","서초구","마포구","용산구","성동구","종로구","중구","송파구","강서구","분당구"],b=g.flatMap(_=>x.map(E=>({loc:`${i}/c/${encodeURIComponent(_)}/${encodeURIComponent(E)}`,priority:"0.7",freq:"weekly",lastmod:a}))),y=new Set(f.map(_=>_.loc)),d=b.filter(_=>!y.has(_.loc)),w=`<?xml version="1.0" encoding="UTF-8"?>
...
</html>`):t.text("url required",400)});j.get("/",t=>{const e=t.req.header("x-forwarded-proto")||"https",s=t.req.header("x-forwarded-host")||t.req.header("host")||"www.mybeautymap.co.kr",i=`${e}://${s}`;return t.html(_n(i))});const Js=["전체","마사지","헤드스파","피부관리","헤어","메이크업","왁싱","반영구","병원","그외"],Zs={전체:"🏠",마사지:"💆",헤드스파:"🧖",피부관리:"✨",헤어:"💇",메이크업:"💄",왁싱:"🌸",반영구:"👁",병원:"🏥",그외:"🌟"},wn=`<button class="cp cp-rec" onclick="filterFeed(this,'recommended')">⭐ 추천</button>`;function _n(t="https://www.mybeautymap.co.kr"){return`<!DOCTYPE html>
<html lang="ko">
<head>
<meta charset="UTF-8"/>
...
</html>`}const ei=new _i,Cn=Object.assign({"/src/index.tsx":j});let xr=!1;for(const[,t]of Object.entries(Cn))t&&(ei.route("/",t),ei.notFound(t.notFoundHandler),xr=!0);if(!xr)throw new Error("Can't import modules from ['/src/index.tsx','/app/server.ts']");export{ei as default};
