...
*/var on={};const $=new vi,dn=on.DATABASE_URL||"postgresql://neondb_owner:npg_1PBkOmAWRcf2@ep-round-recipe-aqdgkjfj-pooler.c-8.us-east-1.aws.neon.tech/neondb?sslmode=require",O=Ls(dn);function ts(e){if(!e)return"";const t=e.split(/\s+/);for(let s=1;s<t.length;s++)if(t[s].endsWith("구")&&!t[s].endsWith("시구"))return t[s].replace(/구$/,"");for(let s=1;s<t.length;s++)if(t[s].endsWith("시")&&t[s]!==t[0])return t[s].replace(/(특별시|광역시|특별자치시|시)$/,"");return t[0]?t[0].replace(/(특별자치도|도)$/,""):""}function Cs(e){if(!e)return"";const t=e.split(/\s+/);return t[0]?t[0].replace(/(특별시|광역시|특별자치시|특별자치도|도|시)$/,""):""}function ln(e){return{마사지:"MassageTherapist",헤드스파:"BeautySalon",피부관리:"BeautySalon",헤어:"HairSalon",메이크업:"BeautySalon",왁싱:"BeautySalon",반영구:"BeautySalon",병원:"MedicalBusiness",그외:"LocalBusiness"}[e]||"LocalBusiness"}function cn(e){const t=ts(e.address),s=Cs(e.address),i=t||s;return`${e.name} ${i?i+" ":""}${e.category} 추천 | 마이뷰티맵`}function pr(e){const t=ts(e.address),s=Cs(e.address),i=Array.isArray(e.tags)&&e.tags.length?e.tags.join("·")+" ":"",a=t||s,n=e.price?` 가격 ${e.price}.`:"",d=e.desc?" "+e.desc.slice(0,40):"";return`${e.name} | ${a?a+" ":""}${e.category} 잘하는 곳. ${i}예약·위치·가격 한눈에 확인!${n}${d}`.slice(0,160)}function pn(e,t){return`${t} ${e} 추천 TOP | 마이뷰티맵`}function un(e,t,s){return`${t} ${e} 잘하는 곳 ${s}곳 추천! 가격·위치·예약·후기까지 마이뷰티맵에서 한눈에 확인하세요.`}function fn(e,t){const s=t;return[`${s} ${e}`,`${s} ${e} 추천`,`${s} ${e} 잘하는 곳`,`${s} ${e} 예약`,`${s} ${e} 가격`,`${s} ${e} 후기`,`${s} ${e} 맛집`,`${s} 뷰티샵`,`${s} 뷰티 추천`,`${e} 추천`].join(",")}function ve(){return new Date(Date.now()+540*60*1e3).toISOString().slice(0,10)}function ur(){return new Date(Date.now()+540*60*1e3-864e5).toISOString().slice(0,10)}function fr(e){const t=(e??"").trim();if(!t)return"";if(/^[A-Za-z0-9_-]{11}$/.test(t))return t;const s=t.match(/(?:youtu\.be\/|youtube\.com\/(?:watch\?(?:.*&)?v=|embed\/|shorts\/|v\/))([A-Za-z0-9_-]{11})/);return s?s[1]:t}function hn(e){return e.plan!=="shoot"||!e.paid_until?!1:new Date(e.paid_until)>=new Date}function Ue(e){return{id:e.id,name:e.name,category:e.category,tags:e.tags??[],price:e.price??"",address:e.address??"",district:e.district??"",lat:parseFloat(e.lat)||37.5326,lng:parseFloat(e.lng)||127.0246,smartPlaceUrl:e.smart_place_url??"",youtubeId:e.youtube_id??"",featured:e.featured??!1,thumbnail:e.thumbnail??"",phone:e.phone??"",desc:e.description??"",active:e.active??!0,displayMode:e.display_mode??"both",plan:e.plan??"basic",paidUntil:e.paid_until??null,paymentStatus:e.payment_status??"unpaid",paymentMemo:e.payment_memo??"",views:parseInt(e.view_cnt)||0,feedSP:parseInt(e.feed_sp)||0,mapSP:parseInt(e.map_sp)||0,isPremium:hn(e),isRecommended:e.is_recommended??!1}}function gn(e,t,s,i){const n=(s-e)*Math.PI/180,d=(i-t)*Math.PI/180,o=Math.sin(n/2)**2+Math.cos(e*Math.PI/180)*Math.cos(s*Math.PI/180)*Math.sin(d/2)**2;return 6371*2*Math.atan2(Math.sqrt(o),Math.sqrt(1-o))}let Gs=!1,$t=null;async function mn(){if(!Gs)return $t||($t=(async()=>{try{await O`ALTER TABLE shops ADD COLUMN IF NOT EXISTS report_token   TEXT`}catch{}try{await O`ALTER TABLE shops ADD COLUMN IF NOT EXISTS is_recommended BOOLEAN NOT NULL DEFAULT false`}catch{}try{await O`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS feed_view    INTEGER NOT NULL DEFAULT 0`}catch{}try{await O`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS catalog_view INTEGER NOT NULL DEFAULT 0`}catch{}try{await O`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS map_view     INTEGER NOT NULL DEFAULT 0`}catch{}try{await O`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS vts_feed     INTEGER NOT NULL DEFAULT 0`}catch{}try{await O`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS vts_catalog  INTEGER NOT NULL DEFAULT 0`}catch{}try{await O`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS vts_map      INTEGER NOT NULL DEFAULT 0`}catch{}try{await O`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS rec_view     INTEGER NOT NULL DEFAULT 0`}catch{}try{await O`CREATE TABLE IF NOT EXISTS honey_items (
  ...
  `).map(Ue);if(t==="recommended"?u=u.filter(h=>h.isRecommended):t&&t!=="all"&&(u=u.filter(h=>h.category===t)),s&&(u=u.filter(h=>h.name.toLowerCase().includes(s)||(h.tags||[]).some(g=>g.toLowerCase().includes(s))||h.district.toLowerCase().includes(s))),n&&!isNaN(i)&&!isNaN(a))u=u.map(h=>({...h,dist:gn(i,a,h.lat,h.lng)})).filter(h=>h.dist<=20).sort((h,g)=>h.isPremium&&!g.isPremium?-1:!h.isPremium&&g.isPremium?1:h.dist-g.dist);else if(d){const h=u.filter(x=>x.isPremium),g=u.filter(x=>!x.isPremium);for(let x=h.length-1;x>0;x--){const b=Math.floor(Math.random()*(x+1));[h[x],h[b]]=[h[b],h[x]]}for(let x=g.length-1;x>0;x--){const b=Math.floor(Math.random()*(x+1));[g[x],g[b]]=[g[b],g[x]]}u=[...h,...g]}else u.sort((h,g)=>h.isPremium&&!g.isPremium?-1:!h.isPremium&&g.isPremium?1:0);return e.json(u)});$.get("/api/geocode",async e=>{var o,u,h,g,x;const t=e.req.query("query")??"";if(!t)return e.json({error:"query required"},400);const s=b=>b.replace(/(로|길|번길)\s+(\d)/g,"$1$2"),i=[],a=s(t);a!==t&&i.push(a),i.push(t);const n=a.trim().split(/\s+/);n.length>3&&i.push(n.slice(0,-1).join(" "));const d=async b=>(await fetch(`https://maps.apigw.ntruss.com/map-geocode/v2/geocode?query=${encodeURIComponent(b)}`,{headers:{"X-NCP-APIGW-API-KEY-ID":"xjjg4490h8","X-NCP-APIGW-API-KEY":"RB4DFA4ZEF2iHtkerlNrqzoG8P8YHwE2UddGrAtD"}})).json();try{let b=null;for(const _ of i){const E=await d(_);if((o=E.addresses)!=null&&o.length){b=E.addresses[0];break}}if(!b)return e.json({error:"주소를 찾을 수 없어요"},404);const y=b.addressElements||[],l=((u=y.find(_=>{var E;return(E=_.types)==null?void 0:E.includes("SIDO")}))==null?void 0:u.longName)||"",p=((h=y.find(_=>{var E;return(E=_.types)==null?void 0:E.includes("SIGUGUN")}))==null?void 0:h.longName)||"",v=((g=y.find(_=>{var E,M;return((E=_.types)==null?void 0:E.includes("DONGMYUN"))||((M=_.types)==null?void 0:M.includes("RI"))}))==null?void 0:g.longName)||"",w=[l,p,v].filter(Boolean).join(" ")||((x=b.roadAddress)==null?void 0:x.split(" ").slice(0,3).join(" "))||"";return e.json({lat:parseFloat(b.y),lng:parseFloat(b.x),address:b.roadAddress||b.jibunAddress,district:w})}catch{return e.json({error:"지오코딩 실패"},500)}});$.get("/api/shops/all",async e=>{const t=await O`
    SELECT s.*, COALESCE(st.view_cnt,0) as view_cnt,
           COALESCE(st.feed_sp,0) as feed_sp, COALESCE(st.map_sp,0) as map_sp
    FROM shops s LEFT JOIN stats st ON s.id = st.shop_id
  ...
  `;return s.length?e.json(Ue(s[0])):e.json({error:"not found"},404)});$.post("/api/admin/shops",async e=>{const t=await e.req.json(),s=Array.isArray(t.tags)?t.tags:(t.tags??"").split(",").map(n=>n.trim()).filter(Boolean),a=(await O`
    ...
    VALUES
      (${t.name??"새 업체"}, ${t.category??"피부관리"}, ${s},
       ...
       ${t.smartPlaceUrl??""}, ${fr(t.youtubeId??"")},
       ${t.featured??!1},
       ${t.thumbnail??""},
  ...
  `,u={};o.forEach(v=>{u[v.shop_id]=parseInt(v.total_rec)||0});const h=await O`
    SELECT
      SUM(view_cnt) as total_views,
      SUM(feed_sp)  as total_feed_sp,
      SUM(map_sp)   as total_map_sp
    FROM stats
  `,g=await O`SELECT COUNT(*) as cnt FROM shops WHERE active = true`,x=h[0]||{},b=a[0]||{},y=n[0]||{},l=await O`
    SELECT shop_id,
    ...
    FROM daily_stats WHERE stat_date = ${t}
  `,p={};return l.forEach(v=>{p[v.shop_id]={todayViews:parseInt(v.view_cnt)||0,todayFeedSP:parseInt(v.feed_sp)||0,todayMapSP:parseInt(v.map_sp)||0,todayFeedView:parseInt(v.feed_view)||0,todayCatalogView:parseInt(v.catalog_view)||0,todayMapView:parseInt(v.map_view)||0,todayVtsFeed:parseInt(v.vts_feed)||0,todayVtsCatalog:parseInt(v.vts_catalog)||0,todayVtsMap:parseInt(v.vts_map)||0,todayRecView:parseInt(v.rec_view)||0}}),e.json({stats:i.map(v=>{var w,_,E,M,R,k,L,C,S,N;return{id:v.id,name:v.name,category:v.category,thumbnail:v.thumbnail,youtubeId:v.youtube_id,featured:v.featured,active:v.active,views:parseInt(v.view_cnt)||0,feedSP:parseInt(v.feed_sp)||0,mapSP:parseInt(v.map_sp)||0,lat:parseFloat(v.lat)||0,lng:parseFloat(v.lng)||0,smartPlaceUrl:v.smart_place_url??"",address:v.address??"",district:v.district??"",phone:v.phone??"",plan:v.plan??"basic",paymentStatus:v.payment_status??"unpaid",paidUntil:v.paid_until?String(v.paid_until).slice(0,10):null,paymentMemo:v.payment_memo??"",displayMode:v.display_mode??"both",priceRange:v.price??"",tags:v.tags??[],description:v.description??"",isRecommended:v.is_recommended??!1,todayViews:((w=p[v.id])==null?void 0:w.todayViews)||0,todayFeedSP:((_=p[v.id])==null?void 0:_.todayFeedSP)||0,todayMapSP:((E=p[v.id])==null?void 0:E.todayMapSP)||0,todayFeedView:((M=p[v.id])==null?void 0:M.todayFeedView)||0,todayCatalogView:((R=p[v.id])==null?void 0:R.todayCatalogView)||0,todayMapView:((k=p[v.id])==null?void 0:k.todayMapView)||0,todayVtsFeed:((L=p[v.id])==null?void 0:L.todayVtsFeed)||0,todayVtsCatalog:((C=p[v.id])==null?void 0:C.todayVtsCatalog)||0,todayVtsMap:((S=p[v.id])==null?void 0:S.todayVtsMap)||0,todayRecView:((N=p[v.id])==null?void 0:N.todayRecView)||0,weekRecView:u[v.id]||0}}),totalViews:parseInt(x.total_views)||0,totalFeedSP:parseInt(x.total_feed_sp)||0,totalMapSP:parseInt(x.total_map_sp)||0,totalShops:parseInt(g[0].cnt)||0,todayViews:parseInt(b.views)||0,todayFeedSP:parseInt(b.feed_sp)||0,todayMapSP:parseInt(b.map_sp)||0,todayRecView:parseInt(b.rec_view)||0,yestViews:parseInt(y.views)||0,yestFeedSP:parseInt(y.feed_sp)||0,yestMapSP:parseInt(y.map_sp)||0,yestRecView:parseInt(y.rec_view)||0,weekChart:d.map(v=>({date:v.stat_date,views:parseInt(v.views)||0,feedSP:parseInt(v.feed_sp)||0,mapSP:parseInt(v.map_sp)||0,recView:parseInt(v.rec_view)||0}))})});$.post("/api/inquiry",async e=>{const t=await e.req.json();return!t.name||!t.phone?e.json({error:"required"},400):(await O`
  ...
  `,x={};g.forEach(y=>{x[y.visit_date]=parseInt(y.visit_cnt)||0});const b=g.reduce((y,l)=>y+(parseInt(l.visit_cnt)||0),0);return e.json({year:t,month:s,monthTotal:{views:parseInt(h.views)||0,feedSP:parseInt(h.feed_sp)||0,mapSP:parseInt(h.map_sp)||0,visits:b},daily:n.map(y=>({date:y.stat_date,visits:x[y.stat_date]||0,views:parseInt(y.views)||0,feedSP:parseInt(y.feed_sp)||0,mapSP:parseInt(y.map_sp)||0,activeShops:parseInt(y.active_shops)||0})),shopDetail:o.map(y=>({id:y.id,name:y.name,category:y.category,thumbnail:y.thumbnail,views:parseInt(y.views)||0,feedSP:parseInt(y.feed_sp)||0,mapSP:parseInt(y.map_sp)||0}))})});$.get("/api/admin/inquiries",async e=>{const t=await O`SELECT * FROM inquiries ORDER BY created_at DESC`;return e.json(t)});$.get("/favicon.ico",e=>hr());$.get("/favicon.svg",e=>hr());function hr(e){return new Response('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32"><rect width="32" height="32" rx="8" fill="#FF4D7D"/><text x="16" y="23" font-size="18" text-anchor="middle">💄</text></svg>',{headers:{"Content-Type":"image/svg+xml","Cache-Control":"public,max-age=86400"}})}$.get("/og-image.jpg",e=>{try{const t=wr.join(process.cwd(),"public","og-image.jpg"),s=xr.readFileSync(t);return new Response(s,{headers:{"Content-Type":"image/jpeg","Cache-Control":"public,max-age=86400"}})}catch{return e.notFound()}});$.post("/api/admin/upload-thumbnail",async e=>{const t=await e.req.json(),{shopId:s,dataUrl:i}=t;return!i||!s?e.json({error:"required"},400):(await O`UPDATE shops SET thumbnail = ${i} WHERE id = ${s}`,e.json({ok:!0,url:i}))});$.get("/api/shorts",async e=>{try{const t=await O`
      SELECT * FROM shorts_items WHERE active = true ORDER BY sort_order ASC, id DESC
    `;return e.json(t)}catch(t){return console.error("[/api/shorts] DB error:",t),e.json([],200)}});$.get("/api/admin/shorts",async e=>{const t=await O`SELECT * FROM shorts_items ORDER BY sort_order ASC, id DESC`;return e.json(t)});$.post("/api/admin/shorts",async e=>{const t=await e.req.json(),s=await O`
  ...
  `}$.post("/api/track/shorts/view/:id",async e=>{const t=+e.req.param("id"),s=ve();await O`UPDATE shorts_items SET view_cnt = view_cnt + 1 WHERE id = ${t}`;try{await ut(),await O`
    ...
    `,u=await O`SELECT COUNT(*) as cnt FROM shorts_items`,h=await O`SELECT COUNT(*) as cnt FROM shorts_items WHERE active = true`;return e.json({total_views:Number(a.total_views),total_sp:Number(a.total_sp),today_views:Number(n.views),today_sp:Number(n.sp),yest_views:Number(d.views),yest_sp:Number(d.sp),week_views:Number(o.views),week_sp:Number(o.sp),total_items:Number(u[0].cnt),active_items:Number(h[0].cnt)})}catch(a){return e.json({error:String(a)},500)}});$.get("/api/admin/shorts/stats/items",async e=>{const t=new Date(Date.now()-5184e5).toISOString().slice(0,10);try{await ut();const s=await O`
  ...
  `;const e=["shorts_book","feed_card_cnt","feed_book_cnt","map_pin_cnt","map_book_cnt","search_cnt","inquiry_cnt"];for(const t of e)try{await O`ALTER TABLE visitor_sessions ADD COLUMN IF NOT EXISTS ${O.unsafe(t)} INTEGER NOT NULL DEFAULT 0`}catch{}}$.post("/api/track/session/start",async e=>{try{await Rt(),await O`DELETE FROM visitor_sessions WHERE entered_at < NOW() - INTERVAL '7 days'`;const t=await e.req.json(),s=t.id||"",i=t.device||"unknown";return s?(await O`
    ...
    `,e.json({ok:!0})):e.json({ok:!1})}catch(t){return e.json({ok:!1,error:String(t)})}});$.post("/api/track/session/update",async e=>{try{await Rt();const t=await e.req.json(),{id:s,duration_sec:i,tabs_visited:a,exited:n,shorts_count:d,shorts_book:o,feed_card_cnt:u,feed_book_cnt:h,map_pin_cnt:g,map_book_cnt:x,search_cnt:b,inquiry_cnt:y}=t;if(!s)return e.json({ok:!1});const l=(o||0)+(h||0)+(x||0);return await O`
...
`)}
  ...
  `,h=await O`
    ...
    GROUP BY s.id
    ORDER BY total DESC
  `,g=h.findIndex(te=>te.id===a.id)+1,x=h.length,b=await O`
    SELECT COALESCE(view_cnt,0) as views,
           COALESCE(feed_sp,0)  as feed_sp,
           COALESCE(map_sp,0)   as map_sp
    FROM stats WHERE shop_id = ${a.id}
  `;return e.json({shop:{id:a.id,name:a.name,category:a.category,address:a.address},thisMonth:{views:parseInt((y=d[0])==null?void 0:y.views)||0,feedSP:parseInt((l=d[0])==null?void 0:l.feed_sp)||0,mapSP:parseInt((p=d[0])==null?void 0:p.map_sp)||0,feedView:parseInt((v=d[0])==null?void 0:v.feed_view)||0,catalogView:parseInt((w=d[0])==null?void 0:w.catalog_view)||0,mapView:parseInt((_=d[0])==null?void 0:_.map_view)||0,vtsFeed:parseInt((E=d[0])==null?void 0:E.vts_feed)||0,vtsCatalog:parseInt((M=d[0])==null?void 0:M.vts_catalog)||0,vtsMap:parseInt((R=d[0])==null?void 0:R.vts_map)||0},lastMonth:{views:parseInt((k=o[0])==null?void 0:k.views)||0,feedSP:parseInt((L=o[0])==null?void 0:L.feed_sp)||0,mapSP:parseInt((C=o[0])==null?void 0:C.map_sp)||0,feedView:parseInt((S=o[0])==null?void 0:S.feed_view)||0,catalogView:parseInt((N=o[0])==null?void 0:N.catalog_view)||0,mapView:parseInt((A=o[0])==null?void 0:A.map_view)||0,vtsFeed:parseInt((z=o[0])==null?void 0:z.vts_feed)||0,vtsCatalog:parseInt((j=o[0])==null?void 0:j.vts_catalog)||0,vtsMap:parseInt((B=o[0])==null?void 0:B.vts_map)||0},total:{views:parseInt((U=b[0])==null?void 0:U.views)||0,feedSP:parseInt((V=b[0])==null?void 0:V.feed_sp)||0,mapSP:parseInt((Y=b[0])==null?void 0:Y.map_sp)||0},daily30:u,rank:g,rankTotal:x})});$.get("/report/:token",e=>{const t=e.req.param("token");return e.html(_n(t))});$.get("/admin",e=>e.html(En()));$.get("/map-admin",e=>e.redirect("/admin"));$.get("/map",e=>e.html(wn()));$.get("/api/resolve-naver",async e=>{const t=e.req.query("url")||"";if(!t)return e.json({error:"no url"},400);try{const s=t.match(/place\/([0-9]+)/);if(s)return e.json({resolved:`https://m.place.naver.com/place/${s[1]}/home`});const n=((await fetch(t,{method:"HEAD",redirect:"manual",headers:{"User-Agent":"Mozilla/5.0 (compatible; bot)"}})).headers.get("location")||"").match(/place\/([0-9]+)/);return n?e.json({resolved:`https://m.place.naver.com/place/${n[1]}/home`}):e.json({resolved:t})}catch{return e.json({resolved:t})}});$.get("/reserve",e=>{const t=e.req.query("url")||"",s=e.req.query("name")||"";return t?e.html(`<!DOCTYPE html>
<html lang="ko">
<head>
<meta charset="UTF-8">
...
</html>`}function xn(e,t,s,i){const a=pn(e,t),n=un(e,t,s.length),d=fn(e,t),o=`${i}/c/${encodeURIComponent(e)}/${encodeURIComponent(t)}`,u={마사지:"💆",헤드스파:"🧖",피부관리:"✨",헤어:"💇",메이크업:"💄",왁싱:"🌸",반영구:"👁",병원:"🏥",그외:"🌟"},h=u[e]||"🌟",g=["마사지","헤드스파","피부관리","헤어","메이크업","왁싱","반영구","병원","그외"],x=JSON.stringify({"@context":"https://schema.org","@type":"ItemList",name:a,description:n,url:o,numberOfItems:s.length,itemListElement:s.map((y,l)=>({"@type":"ListItem",position:l+1,name:y.name,url:`${i}/shop/${y.id}`,description:pr(y)}))}),b=JSON.stringify({"@context":"https://schema.org","@type":"BreadcrumbList",itemListElement:[{"@type":"ListItem",position:1,name:"마이뷰티맵",item:i},{"@type":"ListItem",position:2,name:`${t} ${e}`,item:o}]});return`<!DOCTYPE html>
<html lang="ko">
<head>
<meta charset="UTF-8"/>
<meta name="viewport" content="width=device-width,initial-scale=1.0,maximum-scale=1.0,user-scalable=no"/>
...
<div class="wrap">
  ...
  <div class="list">
    ${s.length?s.map((y,l)=>{const p=Array.isArray(y.tags)?y.tags:[],v=(y.address||"").split(" ").slice(1,3).join(" "),w=l===0?"🥇":l===1?"🥈":l===2?"🥉":`${l+1}.`;return`<a class="card" href="/shop/${y.id}">
        ${y.thumbnail?`<img class="card-img" src="${y.thumbnail}" alt="${y.name} ${t} ${e}" loading="lazy"/>`:`<div class="card-ph">${h}</div>`}
        <div class="card-body">
...
</html>`}function _n(e){return`<!DOCTYPE html>
<html lang="ko">
<head>
<meta charset="UTF-8"/>
<meta name="viewport" content="width=device-width,initial-scale=1.0,maximum-scale=1.0,user-scalable=no"/>
