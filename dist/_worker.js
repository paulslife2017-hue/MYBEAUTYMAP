...
*/var un={};const $=new _i,fn=un.DATABASE_URL||"postgresql://neondb_owner:npg_1PBkOmAWRcf2@ep-round-recipe-aqdgkjfj-pooler.c-8.us-east-1.aws.neon.tech/neondb?sslmode=require",B=As(fn);function ss(t){if(!t)return"";const e=t.split(/\s+/);for(let s=1;s<e.length;s++)if(e[s].endsWith("구")&&!e[s].endsWith("시구"))return e[s].replace(/구$/,"");for(let s=1;s<e.length;s++)if(e[s].endsWith("시")&&e[s]!==e[0])return e[s].replace(/(특별시|광역시|특별자치시|시)$/,"");return e[0]?e[0].replace(/(특별자치도|도)$/,""):""}function Os(t){if(!t)return"";const e=t.split(/\s+/);return e[0]?e[0].replace(/(특별시|광역시|특별자치시|특별자치도|도|시)$/,""):""}function hn(t){return{마사지:"MassageTherapist",헤드스파:"BeautySalon",피부관리:"BeautySalon",헤어:"HairSalon",메이크업:"BeautySalon",왁싱:"BeautySalon",반영구:"BeautySalon",병원:"MedicalBusiness",그외:"LocalBusiness"}[t]||"LocalBusiness"}function gn(t){const e=ss(t.address),s=Os(t.address),i=e||s;return`${t.name} ${i?i+" ":""}${t.category} 추천 | 마이뷰티맵`}function gr(t){const e=ss(t.address),s=Os(t.address),i=Array.isArray(t.tags)&&t.tags.length?t.tags.join("·")+" ":"",a=e||s,n=t.price?` 가격 ${t.price}.`:"",d=t.desc?" "+t.desc.slice(0,40):"";return`${t.name} | ${a?a+" ":""}${t.category} 잘하는 곳. ${i}예약·위치·가격 한눈에 확인!${n}${d}`.slice(0,160)}function mn(t,e){return`${e} ${t} 추천 TOP | 마이뷰티맵`}function bn(t,e,s){return`${e} ${t} 잘하는 곳 ${s}곳 추천! 가격·위치·예약·후기까지 마이뷰티맵에서 한눈에 확인하세요.`}function vn(t,e){const s=e;return[`${s} ${t}`,`${s} ${t} 추천`,`${s} ${t} 잘하는 곳`,`${s} ${t} 예약`,`${s} ${t} 가격`,`${s} ${t} 후기`,`${s} ${t} 맛집`,`${s} 뷰티샵`,`${s} 뷰티 추천`,`${t} 추천`].join(",")}function ve(){return new Date(Date.now()+540*60*1e3).toISOString().slice(0,10)}function mr(){return new Date(Date.now()+540*60*1e3-864e5).toISOString().slice(0,10)}function br(t){const e=(t??"").trim();if(!e)return"";if(/^[A-Za-z0-9_-]{11}$/.test(e))return e;const s=e.match(/(?:youtu\.be\/|youtube\.com\/(?:watch\?(?:.*&)?v=|embed\/|shorts\/|v\/))([A-Za-z0-9_-]{11})/);return s?s[1]:e}function yn(t){return t.plan!=="shoot"||!t.paid_until?!1:new Date(t.paid_until)>=new Date}function Ue(t){return{id:t.id,name:t.name,category:t.category,tags:t.tags??[],price:t.price??"",address:t.address??"",district:t.district??"",lat:parseFloat(t.lat)||37.5326,lng:parseFloat(t.lng)||127.0246,smartPlaceUrl:t.smart_place_url??"",youtubeId:t.youtube_id??"",featured:t.featured??!1,thumbnail:t.thumbnail??"",phone:t.phone??"",desc:t.description??"",active:t.active??!0,displayMode:t.display_mode??"both",plan:t.plan??"basic",paidUntil:t.paid_until??null,paymentStatus:t.payment_status??"unpaid",paymentMemo:t.payment_memo??"",views:parseInt(t.view_cnt)||0,feedSP:parseInt(t.feed_sp)||0,mapSP:parseInt(t.map_sp)||0,isPremium:yn(t),isRecommended:t.is_recommended??!1}}function xn(t,e,s,i){const n=(s-t)*Math.PI/180,d=(i-e)*Math.PI/180,o=Math.sin(n/2)**2+Math.cos(t*Math.PI/180)*Math.cos(s*Math.PI/180)*Math.sin(d/2)**2;return 6371*2*Math.atan2(Math.sqrt(o),Math.sqrt(1-o))}const vs=10;let Wt=!1,it=null;async function vr(){if(!Wt)return it||(it=(async()=>{try{if(await B`
      ...
      `,await B`CREATE INDEX IF NOT EXISTS idx_se_session  ON session_events(session_id)`,await B`CREATE INDEX IF NOT EXISTS idx_se_occurred ON session_events(occurred_at)`,await B`ALTER TABLE shops             ADD COLUMN IF NOT EXISTS report_token   TEXT`,await B`ALTER TABLE shops             ADD COLUMN IF NOT EXISTS is_recommended BOOLEAN NOT NULL DEFAULT false`,await B`ALTER TABLE daily_stats       ADD COLUMN IF NOT EXISTS feed_view      INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats       ADD COLUMN IF NOT EXISTS catalog_view   INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats       ADD COLUMN IF NOT EXISTS map_view       INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats       ADD COLUMN IF NOT EXISTS vts_feed       INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats       ADD COLUMN IF NOT EXISTS vts_catalog    INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats       ADD COLUMN IF NOT EXISTS vts_map        INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats       ADD COLUMN IF NOT EXISTS rec_view       INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE honey_items       ADD COLUMN IF NOT EXISTS coupang_url2   TEXT    NOT NULL DEFAULT ''`,await B`ALTER TABLE shorts_items      ADD COLUMN IF NOT EXISTS smart_place_url TEXT   NOT NULL DEFAULT ''`,await B`ALTER TABLE shorts_items      ADD COLUMN IF NOT EXISTS view_cnt        INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE shorts_items      ADD COLUMN IF NOT EXISTS sp_cnt          INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE session_events    ADD COLUMN IF NOT EXISTS viewed_sec      INTEGER NOT NULL DEFAULT 0`,await B`INSERT INTO schema_migrations (version) VALUES (${vs}) ON CONFLICT DO NOTHING`,Wt=!0}catch(t){console.error("[migration] error:",t),it=null}})(),it)}$.use("*",async(t,e)=>(await vr(),e()));$.get("/api/shops",async t=>{const e=t.req.query("category")??"",s=(t.req.query("q")??"").toLowerCase(),i=parseFloat(t.req.query("lat")??""),a=parseFloat(t.req.query("lng")??""),n=t.req.query("nearby")==="1",d=t.req.query("shuffle")==="1";let u=(await B`
  ...
  `;return t.json(e.map(Ue))});$.get("/api/shops/:id",async t=>{const e=+t.req.param("id"),s=await B`
    SELECT s.*, COALESCE(st.view_cnt,0) as view_cnt,
           COALESCE(st.feed_sp,0) as feed_sp, COALESCE(st.map_sp,0) as map_sp
    FROM shops s LEFT JOIN stats st ON s.id = st.shop_id
    WHERE s.id = ${e}
  `;return s.length?t.json(Ue(s[0])):t.json({error:"not found"},404)});$.post("/api/admin/shops",async t=>{const e=await t.req.json(),s=Array.isArray(e.tags)?e.tags:(e.tags??"").split(",").map(n=>n.trim()).filter(Boolean),a=(await B`
    INSERT INTO shops
    ...
    RETURNING *
  `)[0];return await B`INSERT INTO stats (shop_id) VALUES (${a.id}) ON CONFLICT DO NOTHING`,t.json(Ue(a))});$.put("/api/admin/shops/:id",async t=>{const e=+t.req.param("id"),s=await t.req.json(),i=Array.isArray(s.tags)?s.tags:(s.tags??"").split(",").map(n=>n.trim()).filter(Boolean),a=await B`
    UPDATE shops SET
      name            = ${s.name},
      category        = ${s.category},
      tags            = ${i},
    ...
    WHERE id = ${e}
    RETURNING *
  `;return a.length?(await B`INSERT INTO stats (shop_id) VALUES (${e}) ON CONFLICT DO NOTHING`,t.json(Ue(a[0]))):t.json({error:"not found"},404)});$.put("/api/admin/shops/:id/recommended",async t=>{const e=+t.req.param("id"),{isRecommended:s}=await t.req.json(),i=await B`
    UPDATE shops SET is_recommended = ${!!s}
    WHERE id = ${e} RETURNING *
  `;return i.length?t.json({ok:!0,isRecommended:i[0].is_recommended}):t.json({error:"not found"},404)});$.post("/api/track/rec/:id",async t=>{const e=+t.req.param("id"),s=ve();try{await B`INSERT INTO daily_stats (shop_id, stat_date, rec_view)
                VALUES (${e}, ${s}, 1)
              ON CONFLICT (shop_id, stat_date) DO UPDATE
                SET rec_view = daily_stats.rec_view + 1`}catch{}return t.json({ok:!0})});$.put("/api/admin/shops/:id/payment",async t=>{const e=+t.req.param("id"),s=await t.req.json(),i=await B`
    UPDATE shops SET
      plan           = ${s.plan??"basic"},
    ...
    WHERE id = ${e}
    RETURNING *
  `;return i.length?t.json(Ue(i[0])):t.json({error:"not found"},404)});$.delete("/api/admin/shops/:id",async t=>{const e=+t.req.param("id");return await B`DELETE FROM shops WHERE id = ${e}`,t.json({ok:!0})});$.post("/api/track/view/:id",async t=>{const e=+t.req.param("id"),s=ve();let i="feed";try{const a=await t.req.json();["feed","catalog","map"].includes(a==null?void 0:a.source)&&(i=a.source)}catch{}return await B`INSERT INTO stats (shop_id, view_cnt) VALUES (${e}, 1)
      ON CONFLICT (shop_id) DO UPDATE SET view_cnt = stats.view_cnt + 1`,i==="feed"?await B`INSERT INTO daily_stats (shop_id, stat_date, view_cnt, feed_view)
              ...
              ON CONFLICT (shop_id, stat_date) DO UPDATE
                SET view_cnt  = daily_stats.view_cnt  + 1,
                    map_view  = daily_stats.map_view  + 1`,t.json({ok:!0})});$.post("/api/track/sp/:id",async t=>{const e=+t.req.param("id"),s=ve();let i=null;try{const a=await t.req.json();i=(a==null?void 0:a.viewSrc)||null}catch{}return await B`INSERT INTO stats (shop_id, feed_sp) VALUES (${e}, 1)
      ON CONFLICT (shop_id) DO UPDATE SET feed_sp = stats.feed_sp + 1`,i==="feed"?await B`INSERT INTO daily_stats (shop_id, stat_date, feed_sp, vts_feed)
              ...
              ON CONFLICT (shop_id, stat_date) DO UPDATE
                ...
                VALUES (${e}, ${s}, 1)
              ON CONFLICT (shop_id, stat_date) DO UPDATE
                SET feed_sp = daily_stats.feed_sp + 1`,t.json({ok:!0})});$.post("/api/track/mapsp/:id",async t=>{const e=+t.req.param("id"),s=ve();let i=null;try{const a=await t.req.json();i=(a==null?void 0:a.viewSrc)||null}catch{}return await B`INSERT INTO stats (shop_id, map_sp) VALUES (${e}, 1)
      ON CONFLICT (shop_id) DO UPDATE SET map_sp = stats.map_sp + 1`,i==="feed"?await B`INSERT INTO daily_stats (shop_id, stat_date, map_sp, vts_feed)
              ...
              ON CONFLICT (shop_id, stat_date) DO UPDATE
                SET map_sp  = daily_stats.map_sp  + 1,
                    vts_map = daily_stats.vts_map + 1`,t.json({ok:!0})});$.post("/api/track/visit",async t=>{const e=ve();return await B`
  ...
  `,t.json({ok:!0})});$.get("/api/admin/daily-visits",async t=>{const e=await B`
    SELECT visit_date::text as visit_date, visit_cnt
    FROM daily_visits
    ORDER BY visit_date DESC
    LIMIT 30
  `;return t.json(e)});$.post("/api/admin/reset-stats",async t=>(await B`UPDATE stats SET view_cnt=0, feed_sp=0, map_sp=0`,await B`DELETE FROM daily_visits`,await B`DELETE FROM daily_stats`,t.json({ok:!0})));$.post("/api/admin/init-daily-stats",async t=>(Wt=!1,it=null,await B`DELETE FROM schema_migrations WHERE version = ${vs}`,await vr(),t.json({ok:!0})));$.get("/api/admin/stats",async t=>{const e=ve(),s=mr(),i=await B`
  ...
  `,u={};o.forEach(v=>{u[v.shop_id]=parseInt(v.total_rec)||0});const h=await B`
    SELECT
      SUM(view_cnt) as total_views,
      SUM(feed_sp)  as total_feed_sp,
      SUM(map_sp)   as total_map_sp
    FROM stats
  `,g=await B`SELECT COUNT(*) as cnt FROM shops WHERE active = true`,x=h[0]||{},b=a[0]||{},y=n[0]||{},l=await B`
    SELECT shop_id,
  ...
  `,t.json({ok:!0}))});$.get("/api/admin/calendar",async t=>{const e=parseInt(t.req.query("year")||String(new Date().getFullYear())),s=parseInt(t.req.query("month")||String(new Date().getMonth()+1)),i=`${e}-${String(s).padStart(2,"0")}-01`,a=new Date(e,s,0).toISOString().slice(0,10),n=await B`
    SELECT stat_date::text as stat_date,
           COALESCE(SUM(view_cnt),0) as views,
           COALESCE(SUM(feed_sp),0)  as feed_sp,
           COALESCE(SUM(map_sp),0)   as map_sp,
    ...
    GROUP BY stat_date
    ORDER BY stat_date ASC
  `,d=t.req.query("date");let o=[];d&&(o=await B`
  ...
  `;return t.json(s[0])});$.put("/api/admin/shorts/:id",async t=>{const e=+t.req.param("id"),s=await t.req.json(),i=await B`
    UPDATE shorts_items SET
      ...
      active          = ${s.active!==!1}
    WHERE id = ${e} RETURNING *
  `;return t.json(i[0])});$.delete("/api/admin/shorts/:id",async t=>{const e=+t.req.param("id");return await B`DELETE FROM shorts_items WHERE id = ${e}`,t.json({ok:!0})});let Ys=!1;async function ft(){Ys||(await B`
    CREATE TABLE IF NOT EXISTS shorts_daily_stats (
      shorts_id  INTEGER NOT NULL,
      stat_date  DATE    NOT NULL,
      view_cnt   INTEGER NOT NULL DEFAULT 0,
  ...
  `,Ys=!0)}$.post("/api/track/shorts/view/:id",async t=>{const e=+t.req.param("id"),s=ve();await B`UPDATE shorts_items SET view_cnt = view_cnt + 1 WHERE id = ${e}`;try{await ft(),await B`
    ...
    `}catch{}return t.json({ok:!0})});$.post("/api/track/shorts/sp/:id",async t=>{const e=+t.req.param("id"),s=ve();await B`UPDATE shorts_items SET sp_cnt = sp_cnt + 1 WHERE id = ${e}`;try{await ft(),await B`
      INSERT INTO shorts_daily_stats (shorts_id, stat_date, sp_cnt)
      VALUES (${e}, ${s}, 1)
      ON CONFLICT (shorts_id, stat_date)
      DO UPDATE SET sp_cnt = shorts_daily_stats.sp_cnt + 1
    `}catch{}return t.json({ok:!0})});$.get("/api/admin/shorts/stats/summary",async t=>{const e=ve(),s=mr(),i=new Date(Date.now()-6*864e5).toISOString().slice(0,10);try{await ft();const[a]=await B`
    ...
    `;return t.json(s)}catch(s){return t.json({error:String(s)},500)}});$.get("/api/admin/shorts/stats/daily",async t=>{const e=new Date(Date.now()-25056e5).toISOString().slice(0,10);try{await ft();const s=await B`
      ...
      GROUP BY stat_date
      ORDER BY stat_date ASC
    `;return t.json(s)}catch(s){return t.json({error:String(s)},500)}});$.get("/api/admin/shorts/stats/item/:id",async t=>{const e=+t.req.param("id"),s=new Date(Date.now()-29*864e5).toISOString().slice(0,10);try{await ft();const i=await B`
  ...
  `,await B`CREATE INDEX IF NOT EXISTS idx_se_session ON session_events(session_id)`,await B`CREATE INDEX IF NOT EXISTS idx_se_occurred ON session_events(occurred_at)`,Ks=!0)}$.post("/api/track/session/event",async t=>{try{await Ms();const e=await t.req.json(),{session_id:s,event_type:i,shop_id:a,shop_name:n,shop_cat:d,viewed_sec:o}=e;return!s||!i?t.json({ok:!1}):(await B`DELETE FROM session_events WHERE occurred_at < NOW() - INTERVAL '7 days'`,await B`
    ...
    `,t.json({ok:!0}))}catch(e){return t.json({ok:!1,error:String(e)})}});$.get("/api/admin/sessions/:sid/events",async t=>{try{await Ms();const e=t.req.param("sid"),s=await B`
      SELECT id, occurred_at, event_type, shop_id, shop_name, shop_cat
      FROM session_events
      WHERE session_id = ${e}
      ORDER BY occurred_at ASC
      LIMIT 200
    `;return t.json(s)}catch{return t.json([])}});$.get("/api/admin/shop-ranking",async t=>{try{await Ms();const e=Number(t.req.query("days")||7),s=await B`
      SELECT
        shop_id,
        shop_name,
        shop_cat,
    ...
    `;return t.json(e.map(s=>({day:String(s.day).slice(0,10),total:Number(s.total),mobile:Number(s.mobile),booked:Number(s.booked),shorts_total:Number(s.shorts_total),feed_total:Number(s.feed_total),map_total:Number(s.map_total),avg_sec:Number(s.avg_sec)})))}catch{return t.json([])}});$.get("/api/admin/sessions",async t=>{try{await Nt();const e=ve(),s=await B`
      ...
      LIMIT 200
    `;return t.json(s)}catch{return t.json([])}});$.get("/api/admin/sessions/summary",async t=>{try{await Nt();const e=ve(),[s]=await B`
      SELECT
        COUNT(*)                                                          as total,
        COUNT(*) FILTER (WHERE device='mobile')                          as mobile,
  ...
  `;return t.json(e)});$.get("/api/admin/honey",async t=>{const e=await B`SELECT * FROM honey_items ORDER BY sort_order ASC, id DESC`;return t.json(e)});$.post("/api/admin/honey",async t=>{const e=await t.req.json(),s=await B`
    ...
    RETURNING *
  `;return t.json(s[0])});$.put("/api/admin/honey/:id",async t=>{const e=+t.req.param("id"),s=await t.req.json(),i=await B`
    UPDATE honey_items SET
      ...
      active      = ${s.active!==!1}
    WHERE id = ${e} RETURNING *
  `;return t.json(i[0])});$.delete("/api/admin/honey/:id",async t=>{const e=+t.req.param("id");return await B`DELETE FROM honey_items WHERE id = ${e}`,t.json({ok:!0})});$.get("/robots.txt",t=>{const e=t.req.header("x-forwarded-proto")||"https",s=t.req.header("x-forwarded-host")||t.req.header("host")||"www.mybeautymap.co.kr";return t.text(`User-agent: *
Allow: /
Disallow: /admin
Disallow: /api/

Sitemap: ${e}://${s}/sitemap.xml`,200,{"Content-Type":"text/plain; charset=utf-8"})});$.get("/ads.txt",t=>t.text("google.com, pub-6943282483618134, DIRECT, f08c47fec0942fa0",200,{"Content-Type":"text/plain; charset=utf-8"}));$.get("/sitemap.xml",async t=>{const e=t.req.header("x-forwarded-proto")||"https",s=t.req.header("x-forwarded-host")||t.req.header("host")||"www.mybeautymap.co.kr",i=`${e}://${s}`,a=ve(),n=await B`SELECT id, name, category, address FROM shops WHERE active = true ORDER BY id`,d=new Set;for(const _ of n){const E=ss(_.address);E&&d.add(`${_.category}|||${E}`)}const o=[{loc:i,priority:"1.0",freq:"daily"},{loc:`${i}/map`,priority:"0.8",freq:"weekly"}],u=n.map(_=>({loc:`${i}/shop/${_.id}`,priority:"0.9",freq:"weekly",lastmod:a})),h=[...d].map(_=>{const[E,O]=_.split("|||");return{loc:`${i}/c/${encodeURIComponent(E)}/${encodeURIComponent(O)}`,priority:"0.8",freq:"weekly",lastmod:a}}),g=["마사지","헤드스파","피부관리","헤어","메이크업","왁싱","반영구"],x=["강남구","서초구","마포구","용산구","성동구","종로구","중구","송파구","강서구","분당구"],b=g.flatMap(_=>x.map(E=>({loc:`${i}/c/${encodeURIComponent(_)}/${encodeURIComponent(E)}`,priority:"0.7",freq:"weekly",lastmod:a}))),y=new Set(h.map(_=>_.loc)),l=b.filter(_=>!y.has(_.loc)),w=`<?xml version="1.0" encoding="UTF-8"?>
...
`)}
</urlset>`;return t.text(w,200,{"Content-Type":"application/xml; charset=utf-8"})});$.get("/shop/:id",async t=>{const e=t.req.header("x-forwarded-proto")||"https",s=t.req.header("x-forwarded-host")||t.req.header("host")||"www.mybeautymap.co.kr",i=`${e}://${s}`,a=+t.req.param("id");if(isNaN(a))return t.redirect("/");const n=await B`
    SELECT s.*, COALESCE(st.view_cnt,0) as view_cnt,
           COALESCE(st.feed_sp,0) as feed_sp
    FROM shops s LEFT JOIN stats st ON s.id = st.shop_id
    WHERE s.id = ${a} AND s.active = true
  `;if(!n.length)return t.redirect("/");const d=Ue(n[0]);return t.html(En(d,i))});$.get("/c/:category/:region",async t=>{const e=t.req.header("x-forwarded-proto")||"https",s=t.req.header("x-forwarded-host")||t.req.header("host")||"www.mybeautymap.co.kr",i=`${e}://${s}`,a=decodeURIComponent(t.req.param("category")),n=decodeURIComponent(t.req.param("region")),o=(await B`
    SELECT s.*, COALESCE(st.view_cnt,0) as view_cnt,
           COALESCE(st.feed_sp,0) as feed_sp
    FROM shops s LEFT JOIN stats st ON s.id = st.shop_id
  ...
  `).map(Ue);return t.html(kn(a,n,o,i))});$.post("/api/admin/shops/:id/report-token",async t=>{const e=+t.req.param("id"),s=Array.from({length:12},()=>Math.floor(Math.random()*16).toString(16)).join(""),i=await B`
    UPDATE shops SET report_token = ${s} WHERE id = ${e} RETURNING id, report_token
  `;return i.length?t.json({token:i[0].report_token}):t.json({error:"not found"},404)});$.post("/api/report/:token/verify",async t=>{var y,l,p,v,w,_,E,O,M,k,C,L,S,R,A,z,j,D,U,V,Y;const e=t.req.param("token"),{phone4:s}=await t.req.json(),i=await B`SELECT * FROM shops WHERE report_token = ${e}`;if(!i.length)return t.json({error:"invalid"},404);const a=i[0];if(s!=="0000"){const K=(a.phone||"").replace(/[^0-9]/g,"").slice(-4);if(!K||K!==s)return t.json({error:"wrong"},401)}ve().slice(0,7);const d=await B`
    SELECT COALESCE(SUM(view_cnt),0)     as views,
           COALESCE(SUM(feed_sp),0)      as feed_sp,
           COALESCE(SUM(map_sp),0)       as map_sp,
