...
*/var un={};const j=new _i,fn=un.DATABASE_URL||"postgresql://neondb_owner:npg_1PBkOmAWRcf2@ep-round-recipe-aqdgkjfj-pooler.c-8.us-east-1.aws.neon.tech/neondb?sslmode=require",B=As(fn);function ss(e){if(!e)return"";const t=e.split(/\s+/);for(let s=1;s<t.length;s++)if(t[s].endsWith("구")&&!t[s].endsWith("시구"))return t[s].replace(/구$/,"");for(let s=1;s<t.length;s++)if(t[s].endsWith("시")&&t[s]!==t[0])return t[s].replace(/(특별시|광역시|특별자치시|시)$/,"");return t[0]?t[0].replace(/(특별자치도|도)$/,""):""}function Os(e){if(!e)return"";const t=e.split(/\s+/);return t[0]?t[0].replace(/(특별시|광역시|특별자치시|특별자치도|도|시)$/,""):""}function hn(e){return{마사지:"MassageTherapist",헤드스파:"BeautySalon",피부관리:"BeautySalon",헤어:"HairSalon",메이크업:"BeautySalon",왁싱:"BeautySalon",반영구:"BeautySalon",병원:"MedicalBusiness",그외:"LocalBusiness"}[e]||"LocalBusiness"}function gn(e){const t=ss(e.address),s=Os(e.address),i=t||s;return`${e.name} ${i?i+" ":""}${e.category} 추천 | 마이뷰티맵`}function gr(e){const t=ss(e.address),s=Os(e.address),i=Array.isArray(e.tags)&&e.tags.length?e.tags.join("·")+" ":"",a=t||s,n=e.price?` 가격 ${e.price}.`:"",d=e.desc?" "+e.desc.slice(0,40):"";return`${e.name} | ${a?a+" ":""}${e.category} 잘하는 곳. ${i}예약·위치·가격 한눈에 확인!${n}${d}`.slice(0,160)}function mn(e,t){return`${t} ${e} 추천 TOP | 마이뷰티맵`}function bn(e,t,s){return`${t} ${e} 잘하는 곳 ${s}곳 추천! 가격·위치·예약·후기까지 마이뷰티맵에서 한눈에 확인하세요.`}function vn(e,t){const s=t;return[`${s} ${e}`,`${s} ${e} 추천`,`${s} ${e} 잘하는 곳`,`${s} ${e} 예약`,`${s} ${e} 가격`,`${s} ${e} 후기`,`${s} ${e} 맛집`,`${s} 뷰티샵`,`${s} 뷰티 추천`,`${e} 추천`].join(",")}function ve(){return new Date(Date.now()+540*60*1e3).toISOString().slice(0,10)}function mr(){return new Date(Date.now()+540*60*1e3-864e5).toISOString().slice(0,10)}function br(e){const t=(e??"").trim();if(!t)return"";if(/^[A-Za-z0-9_-]{11}$/.test(t))return t;const s=t.match(/(?:youtu\.be\/|youtube\.com\/(?:watch\?(?:.*&)?v=|embed\/|shorts\/|v\/))([A-Za-z0-9_-]{11})/);return s?s[1]:t}function yn(e){return e.plan!=="shoot"||!e.paid_until?!1:new Date(e.paid_until)>=new Date}function Ue(e){return{id:e.id,name:e.name,category:e.category,tags:e.tags??[],price:e.price??"",address:e.address??"",district:e.district??"",lat:parseFloat(e.lat)||37.5326,lng:parseFloat(e.lng)||127.0246,smartPlaceUrl:e.smart_place_url??"",youtubeId:e.youtube_id??"",featured:e.featured??!1,thumbnail:e.thumbnail??"",phone:e.phone??"",desc:e.description??"",active:e.active??!0,displayMode:e.display_mode??"both",plan:e.plan??"basic",paidUntil:e.paid_until??null,paymentStatus:e.payment_status??"unpaid",paymentMemo:e.payment_memo??"",views:parseInt(e.view_cnt)||0,feedSP:parseInt(e.feed_sp)||0,mapSP:parseInt(e.map_sp)||0,isPremium:yn(e),isRecommended:e.is_recommended??!1}}function xn(e,t,s,i){const n=(s-e)*Math.PI/180,d=(i-t)*Math.PI/180,o=Math.sin(n/2)**2+Math.cos(e*Math.PI/180)*Math.cos(s*Math.PI/180)*Math.sin(d/2)**2;return 6371*2*Math.atan2(Math.sqrt(o),Math.sqrt(1-o))}const vs=10;let Wt=!1,it=null;async function vr(){if(!Wt)return it||(it=(async()=>{try{if(await B`
      ...
      `,await B`CREATE INDEX IF NOT EXISTS idx_se_session  ON session_events(session_id)`,await B`CREATE INDEX IF NOT EXISTS idx_se_occurred ON session_events(occurred_at)`,await B`ALTER TABLE shops             ADD COLUMN IF NOT EXISTS report_token   TEXT`,await B`ALTER TABLE shops             ADD COLUMN IF NOT EXISTS is_recommended BOOLEAN NOT NULL DEFAULT false`,await B`ALTER TABLE daily_stats       ADD COLUMN IF NOT EXISTS feed_view      INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats       ADD COLUMN IF NOT EXISTS catalog_view   INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats       ADD COLUMN IF NOT EXISTS map_view       INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats       ADD COLUMN IF NOT EXISTS vts_feed       INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats       ADD COLUMN IF NOT EXISTS vts_catalog    INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats       ADD COLUMN IF NOT EXISTS vts_map        INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats       ADD COLUMN IF NOT EXISTS rec_view       INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE honey_items       ADD COLUMN IF NOT EXISTS coupang_url2   TEXT    NOT NULL DEFAULT ''`,await B`ALTER TABLE shorts_items      ADD COLUMN IF NOT EXISTS smart_place_url TEXT   NOT NULL DEFAULT ''`,await B`ALTER TABLE shorts_items      ADD COLUMN IF NOT EXISTS view_cnt        INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE shorts_items      ADD COLUMN IF NOT EXISTS sp_cnt          INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE session_events    ADD COLUMN IF NOT EXISTS viewed_sec      INTEGER NOT NULL DEFAULT 0`,await B`INSERT INTO schema_migrations (version) VALUES (${vs}) ON CONFLICT DO NOTHING`,Wt=!0}catch(e){console.error("[migration] error:",e),it=null}})(),it)}j.use("*",async(e,t)=>(await vr(),t()));j.get("/api/shops",async e=>{const t=e.req.query("category")??"",s=(e.req.query("q")??"").toLowerCase(),i=parseFloat(e.req.query("lat")??""),a=parseFloat(e.req.query("lng")??""),n=e.req.query("nearby")==="1",d=e.req.query("shuffle")==="1";let p=(await B`
  ...
  `).map(Ue);if(t==="recommended"?p=p.filter(f=>f.isRecommended):t&&t!=="all"&&(p=p.filter(f=>f.category===t)),s&&(p=p.filter(f=>f.name.toLowerCase().includes(s)||(f.tags||[]).some(g=>g.toLowerCase().includes(s))||f.district.toLowerCase().includes(s))),n&&!isNaN(i)&&!isNaN(a))p=p.map(f=>({...f,dist:xn(i,a,f.lat,f.lng)})).filter(f=>f.dist<=20).sort((f,g)=>f.isPremium&&!g.isPremium?-1:!f.isPremium&&g.isPremium?1:f.dist-g.dist);else if(d){const f=p.filter(x=>x.isPremium),g=p.filter(x=>!x.isPremium);for(let x=f.length-1;x>0;x--){const b=Math.floor(Math.random()*(x+1));[f[x],f[b]]=[f[b],f[x]]}for(let x=g.length-1;x>0;x--){const b=Math.floor(Math.random()*(x+1));[g[x],g[b]]=[g[b],g[x]]}p=[...f,...g]}else p.sort((f,g)=>f.isPremium&&!g.isPremium?-1:!f.isPremium&&g.isPremium?1:0);return e.json(p)});j.get("/api/geocode",async e=>{var o,p,f,g,x;const t=e.req.query("query")??"";if(!t)return e.json({error:"query required"},400);const s=b=>b.replace(/(로|길|번길)\s+(\d)/g,"$1$2"),i=[],a=s(t);a!==t&&i.push(a),i.push(t);const n=a.trim().split(/\s+/);n.length>3&&i.push(n.slice(0,-1).join(" "));const d=async b=>(await fetch(`https://maps.apigw.ntruss.com/map-geocode/v2/geocode?query=${encodeURIComponent(b)}`,{headers:{"X-NCP-APIGW-API-KEY-ID":"xjjg4490h8","X-NCP-APIGW-API-KEY":"RB4DFA4ZEF2iHtkerlNrqzoG8P8YHwE2UddGrAtD"}})).json();try{let b=null;for(const _ of i){const E=await d(_);if((o=E.addresses)!=null&&o.length){b=E.addresses[0];break}}if(!b)return e.json({error:"주소를 찾을 수 없어요"},404);const y=b.addressElements||[],l=((p=y.find(_=>{var E;return(E=_.types)==null?void 0:E.includes("SIDO")}))==null?void 0:p.longName)||"",u=((f=y.find(_=>{var E;return(E=_.types)==null?void 0:E.includes("SIGUGUN")}))==null?void 0:f.longName)||"",v=((g=y.find(_=>{var E,O;return((E=_.types)==null?void 0:E.includes("DONGMYUN"))||((O=_.types)==null?void 0:O.includes("RI"))}))==null?void 0:g.longName)||"",w=[l,u,v].filter(Boolean).join(" ")||((x=b.roadAddress)==null?void 0:x.split(" ").slice(0,3).join(" "))||"";return e.json({lat:parseFloat(b.y),lng:parseFloat(b.x),address:b.roadAddress||b.jibunAddress,district:w})}catch{return e.json({error:"지오코딩 실패"},500)}});j.get("/api/shops/all",async e=>{const t=await B`
    SELECT s.*, COALESCE(st.view_cnt,0) as view_cnt,
           COALESCE(st.feed_sp,0) as feed_sp, COALESCE(st.map_sp,0) as map_sp
    FROM shops s LEFT JOIN stats st ON s.id = st.shop_id
  ...
  `,p={};o.forEach(v=>{p[v.shop_id]=parseInt(v.total_rec)||0});const f=await B`
    ...
    FROM stats
  `,g=await B`SELECT COUNT(*) as cnt FROM shops WHERE active = true`,x=f[0]||{},b=a[0]||{},y=n[0]||{},l=await B`
    SELECT shop_id,
  ...
  `,Xs=!0)}j.post("/api/track/session/start",async e=>{try{await Nt(),await B`DELETE FROM visitor_sessions WHERE entered_at < NOW() - INTERVAL '7 days'`;const t=await e.req.json(),s=t.id||"",i=t.device||"unknown";return s?(await B`
    ...
    `,e.json({ok:!0})):e.json({ok:!1})}catch(t){return e.json({ok:!1,error:String(t)})}});j.post("/api/track/session/update",async e=>{try{await Nt();const t=await e.req.json(),{id:s,duration_sec:i,tabs_visited:a,exited:n,shorts_count:d,shorts_book:o,feed_card_cnt:p,feed_book_cnt:f,map_pin_cnt:g,map_book_cnt:x,search_cnt:b,inquiry_cnt:y}=t;if(!s)return e.json({ok:!1});const l=(o||0)+(f||0)+(x||0);return await B`
      UPDATE visitor_sessions SET
        ...
        exited        = ${n||!1}
      WHERE id = ${s}
    `,e.json({ok:!0})}catch{return e.json({ok:!1})}});let Ks=!1;async function Ms(){Ks||(await B`
    CREATE TABLE IF NOT EXISTS session_events (
      id          BIGSERIAL PRIMARY KEY,
      session_id  TEXT        NOT NULL,
      occurred_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  ...
  `,await B`CREATE INDEX IF NOT EXISTS idx_se_session ON session_events(session_id)`,await B`CREATE INDEX IF NOT EXISTS idx_se_occurred ON session_events(occurred_at)`,Ks=!0)}j.post("/api/track/session/event",async e=>{try{await Ms();const t=await e.req.json(),{session_id:s,event_type:i,shop_id:a,shop_name:n,shop_cat:d,viewed_sec:o}=t;return!s||!i?e.json({ok:!1}):(await B`DELETE FROM session_events WHERE occurred_at < NOW() - INTERVAL '7 days'`,await B`
    ...
    `,e.json({ok:!0}))}catch(t){return e.json({ok:!1,error:String(t)})}});j.get("/api/admin/sessions/:sid/events",async e=>{try{await Ms();const t=e.req.param("sid"),s=await B`
      SELECT id, occurred_at, event_type, shop_id, shop_name, shop_cat
      FROM session_events
      WHERE session_id = ${t}
      ORDER BY occurred_at ASC
      LIMIT 200
    `;return e.json(s)}catch{return e.json([])}});j.get("/api/admin/shop-ranking",async e=>{try{await Ms();const t=Number(e.req.query("days")||7),s=await B`
      SELECT
        shop_id,
        shop_name,
        shop_cat,
...
Sitemap: ${t}://${s}/sitemap.xml`,200,{"Content-Type":"text/plain; charset=utf-8"})});j.get("/ads.txt",e=>e.text("google.com, pub-6943282483618134, DIRECT, f08c47fec0942fa0",200,{"Content-Type":"text/plain; charset=utf-8"}));j.get("/sitemap.xml",async e=>{const t=e.req.header("x-forwarded-proto")||"https",s=e.req.header("x-forwarded-host")||e.req.header("host")||"www.mybeautymap.co.kr",i=`${t}://${s}`,a=ve(),n=await B`SELECT id, name, category, address FROM shops WHERE active = true ORDER BY id`,d=new Set;for(const _ of n){const E=ss(_.address);E&&d.add(`${_.category}|||${E}`)}const o=[{loc:i,priority:"1.0",freq:"daily"},{loc:`${i}/map`,priority:"0.8",freq:"weekly"}],p=n.map(_=>({loc:`${i}/shop/${_.id}`,priority:"0.9",freq:"weekly",lastmod:a})),f=[...d].map(_=>{const[E,O]=_.split("|||");return{loc:`${i}/c/${encodeURIComponent(E)}/${encodeURIComponent(O)}`,priority:"0.8",freq:"weekly",lastmod:a}}),g=["마사지","헤드스파","피부관리","헤어","메이크업","왁싱","반영구"],x=["강남구","서초구","마포구","용산구","성동구","종로구","중구","송파구","강서구","분당구"],b=g.flatMap(_=>x.map(E=>({loc:`${i}/c/${encodeURIComponent(_)}/${encodeURIComponent(E)}`,priority:"0.7",freq:"weekly",lastmod:a}))),y=new Set(f.map(_=>_.loc)),l=b.filter(_=>!y.has(_.loc)),w=`<?xml version="1.0" encoding="UTF-8"?>
...
`)}
  ...
  `;if(!n.length)return e.redirect("/");const d=Ue(n[0]);return e.html(En(d,i))});j.get("/c/:category/:region",async e=>{const t=e.req.header("x-forwarded-proto")||"https",s=e.req.header("x-forwarded-host")||e.req.header("host")||"www.mybeautymap.co.kr",i=`${t}://${s}`,a=decodeURIComponent(e.req.param("category")),n=decodeURIComponent(e.req.param("region")),o=(await B`
    SELECT s.*, COALESCE(st.view_cnt,0) as view_cnt,
           COALESCE(st.feed_sp,0) as feed_sp
    FROM shops s LEFT JOIN stats st ON s.id = st.shop_id
...
</html>`}function kn(e,t,s,i){const a=mn(e,t),n=bn(e,t,s.length),d=vn(e,t),o=`${i}/c/${encodeURIComponent(e)}/${encodeURIComponent(t)}`,p={마사지:"💆",헤드스파:"🧖",피부관리:"✨",헤어:"💇",메이크업:"💄",왁싱:"🌸",반영구:"👁",병원:"🏥",그외:"🌟"},f=p[e]||"🌟",g=["마사지","헤드스파","피부관리","헤어","메이크업","왁싱","반영구","병원","그외"],x=JSON.stringify({"@context":"https://schema.org","@type":"ItemList",name:a,description:n,url:o,numberOfItems:s.length,itemListElement:s.map((y,l)=>({"@type":"ListItem",position:l+1,name:y.name,url:`${i}/shop/${y.id}`,description:gr(y)}))}),b=JSON.stringify({"@context":"https://schema.org","@type":"BreadcrumbList",itemListElement:[{"@type":"ListItem",position:1,name:"마이뷰티맵",item:i},{"@type":"ListItem",position:2,name:`${t} ${e}`,item:o}]});return`<!DOCTYPE html>
<html lang="ko">
<head>
<meta charset="UTF-8"/>
<meta name="viewport" content="width=device-width,initial-scale=1.0,maximum-scale=1.0,user-scalable=no"/>
...
<div class="wrap">
  ...
  <!-- 업체 목록 -->
  <div class="list">
    ${s.length?s.map((y,l)=>{const u=Array.isArray(y.tags)?y.tags:[],v=(y.address||"").split(" ").slice(1,3).join(" "),w=l===0?"🥇":l===1?"🥈":l===2?"🥉":`${l+1}.`;return`<a class="card" href="/shop/${y.id}">
