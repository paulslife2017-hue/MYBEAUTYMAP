...
*/var Wt={};const j=new Ei,fn=Wt.DATABASE_URL||"postgresql://neondb_owner:npg_1PBkOmAWRcf2@ep-round-recipe-aqdgkjfj-pooler.c-8.us-east-1.aws.neon.tech/neondb?sslmode=require",B=Os(fn);function is(t){if(!t)return"";const e=t.split(/\s+/);for(let s=1;s<e.length;s++)if(e[s].endsWith("구")&&!e[s].endsWith("시구"))return e[s].replace(/구$/,"");for(let s=1;s<e.length;s++)if(e[s].endsWith("시")&&e[s]!==e[0])return e[s].replace(/(특별시|광역시|특별자치시|시)$/,"");return e[0]?e[0].replace(/(특별자치도|도)$/,""):""}function Ms(t){if(!t)return"";const e=t.split(/\s+/);return e[0]?e[0].replace(/(특별시|광역시|특별자치시|특별자치도|도|시)$/,""):""}function hn(t){return{마사지:"MassageTherapist",헤드스파:"BeautySalon",피부관리:"BeautySalon",헤어:"HairSalon",메이크업:"BeautySalon",왁싱:"BeautySalon",반영구:"BeautySalon",병원:"MedicalBusiness",그외:"LocalBusiness"}[t]||"LocalBusiness"}function gn(t){const e=is(t.address),s=Ms(t.address),i=e||s;return`${t.name} ${i?i+" ":""}${t.category} 추천 | 마이뷰티맵`}function mr(t){const e=is(t.address),s=Ms(t.address),i=Array.isArray(t.tags)&&t.tags.length?t.tags.join("·")+" ":"",a=e||s,n=t.price?` 가격 ${t.price}.`:"",l=t.desc?" "+t.desc.slice(0,40):"";return`${t.name} | ${a?a+" ":""}${t.category} 잘하는 곳. ${i}예약·위치·가격 한눈에 확인!${n}${l}`.slice(0,160)}function mn(t,e){return`${e} ${t} 추천 TOP | 마이뷰티맵`}function bn(t,e,s){return`${e} ${t} 잘하는 곳 ${s}곳 추천! 가격·위치·예약·후기까지 마이뷰티맵에서 한눈에 확인하세요.`}function vn(t,e){const s=e;return[`${s} ${t}`,`${s} ${t} 추천`,`${s} ${t} 잘하는 곳`,`${s} ${t} 예약`,`${s} ${t} 가격`,`${s} ${t} 후기`,`${s} ${t} 맛집`,`${s} 뷰티샵`,`${s} 뷰티 추천`,`${t} 추천`].join(",")}function ve(){return new Date(Date.now()+540*60*1e3).toISOString().slice(0,10)}function br(){return new Date(Date.now()+540*60*1e3-864e5).toISOString().slice(0,10)}function vr(t){const e=(t??"").trim();if(!e)return"";if(/^[A-Za-z0-9_-]{11}$/.test(e))return e;const s=e.match(/(?:youtu\.be\/|youtube\.com\/(?:watch\?(?:.*&)?v=|embed\/|shorts\/|v\/))([A-Za-z0-9_-]{11})/);return s?s[1]:e}function yn(t){return t.plan!=="shoot"||!t.paid_until?!1:new Date(t.paid_until)>=new Date}function Ue(t){return{id:t.id,name:t.name,category:t.category,tags:t.tags??[],price:t.price??"",address:t.address??"",district:t.district??"",lat:parseFloat(t.lat)||37.5326,lng:parseFloat(t.lng)||127.0246,smartPlaceUrl:t.smart_place_url??"",youtubeId:t.youtube_id??"",featured:t.featured??!1,thumbnail:t.thumbnail??"",phone:t.phone??"",desc:t.description??"",active:t.active??!0,displayMode:t.display_mode??"both",plan:t.plan??"basic",paidUntil:t.paid_until??null,paymentStatus:t.payment_status??"unpaid",paymentMemo:t.payment_memo??"",views:parseInt(t.view_cnt)||0,feedSP:parseInt(t.feed_sp)||0,mapSP:parseInt(t.map_sp)||0,isPremium:yn(t),isRecommended:t.is_recommended??!1}}function xn(t,e,s,i){const n=(s-t)*Math.PI/180,l=(i-e)*Math.PI/180,o=Math.sin(n/2)**2+Math.cos(t*Math.PI/180)*Math.cos(s*Math.PI/180)*Math.sin(l/2)**2;return 6371*2*Math.atan2(Math.sqrt(o),Math.sqrt(1-o))}const ys=10;let Gt=!1,it=null;async function yr(){if(!Gt)return it||(it=(async()=>{try{if(await B`
  ...
  `).map(Ue);if(e==="recommended"?p=p.filter(f=>f.isRecommended):e&&e!=="all"&&(p=p.filter(f=>f.category===e)),s&&(p=p.filter(f=>f.name.toLowerCase().includes(s)||(f.tags||[]).some(g=>g.toLowerCase().includes(s))||f.district.toLowerCase().includes(s))),n&&!isNaN(i)&&!isNaN(a))p=p.map(f=>({...f,dist:xn(i,a,f.lat,f.lng)})).filter(f=>f.dist<=20).sort((f,g)=>f.isPremium&&!g.isPremium?-1:!f.isPremium&&g.isPremium?1:f.dist-g.dist);else if(l){const f=p.filter(x=>x.isPremium),g=p.filter(x=>!x.isPremium);for(let x=f.length-1;x>0;x--){const b=Math.floor(Math.random()*(x+1));[f[x],f[b]]=[f[b],f[x]]}for(let x=g.length-1;x>0;x--){const b=Math.floor(Math.random()*(x+1));[g[x],g[b]]=[g[b],g[x]]}p=[...f,...g]}else p.sort((f,g)=>f.isPremium&&!g.isPremium?-1:!f.isPremium&&g.isPremium?1:0);return t.json(p)});j.get("/api/geocode",async t=>{var o,p,f,g,x;const e=t.req.query("query")??"";if(!e)return t.json({error:"query required"},400);const s=b=>b.replace(/(로|길|번길)\s+(\d)/g,"$1$2"),i=[],a=s(e);a!==e&&i.push(a),i.push(e);const n=a.trim().split(/\s+/);n.length>3&&i.push(n.slice(0,-1).join(" "));const l=async b=>(await fetch(`https://maps.apigw.ntruss.com/map-geocode/v2/geocode?query=${encodeURIComponent(b)}`,{headers:{"X-NCP-APIGW-API-KEY-ID":"xjjg4490h8","X-NCP-APIGW-API-KEY":"RB4DFA4ZEF2iHtkerlNrqzoG8P8YHwE2UddGrAtD"}})).json();try{let b=null;for(const _ of i){const E=await l(_);if((o=E.addresses)!=null&&o.length){b=E.addresses[0];break}}if(!b)return t.json({error:"주소를 찾을 수 없어요"},404);const y=b.addressElements||[],d=((p=y.find(_=>{var E;return(E=_.types)==null?void 0:E.includes("SIDO")}))==null?void 0:p.longName)||"",u=((f=y.find(_=>{var E;return(E=_.types)==null?void 0:E.includes("SIGUGUN")}))==null?void 0:f.longName)||"",v=((g=y.find(_=>{var E,O;return((E=_.types)==null?void 0:E.includes("DONGMYUN"))||((O=_.types)==null?void 0:O.includes("RI"))}))==null?void 0:g.longName)||"",w=[d,u,v].filter(Boolean).join(" ")||((x=b.roadAddress)==null?void 0:x.split(" ").slice(0,3).join(" "))||"";return t.json({lat:parseFloat(b.y),lng:parseFloat(b.x),address:b.roadAddress||b.jibunAddress,district:w})}catch{return t.json({error:"지오코딩 실패"},500)}});j.get("/api/shops/all",async t=>{const e=await B`
    SELECT s.*, COALESCE(st.view_cnt,0) as view_cnt,
           COALESCE(st.feed_sp,0) as feed_sp, COALESCE(st.map_sp,0) as map_sp
    FROM shops s LEFT JOIN stats st ON s.id = st.shop_id
  ...
  `,Xs=!0)}j.post("/api/track/shorts/view/:id",async t=>{const e=+t.req.param("id"),s=ve();await B`UPDATE shorts_items SET view_cnt = view_cnt + 1 WHERE id = ${e}`;try{await ft(),await B`
    ...
    `,p=await B`SELECT COUNT(*) as cnt FROM shorts_items`,f=await B`SELECT COUNT(*) as cnt FROM shorts_items WHERE active = true`;return t.json({total_views:Number(a.total_views),total_sp:Number(a.total_sp),today_views:Number(n.views),today_sp:Number(n.sp),yest_views:Number(l.views),yest_sp:Number(l.sp),week_views:Number(o.views),week_sp:Number(o.sp),total_items:Number(p[0].cnt),active_items:Number(f[0].cnt)})}catch(a){return t.json({error:String(a)},500)}});j.get("/api/admin/shorts/stats/items",async t=>{const e=new Date(Date.now()-5184e5).toISOString().slice(0,10);try{await ft();const s=await B`
...
</html>`}function kn(t,e,s,i){const a=mn(t,e),n=bn(t,e,s.length),l=vn(t,e),o=`${i}/c/${encodeURIComponent(t)}/${encodeURIComponent(e)}`,p={마사지:"💆",헤드스파:"🧖",피부관리:"✨",헤어:"💇",메이크업:"💄",왁싱:"🌸",반영구:"👁",병원:"🏥",그외:"🌟"},f=p[t]||"🌟",g=["마사지","헤드스파","피부관리","헤어","메이크업","왁싱","반영구","병원","그외"],x=JSON.stringify({"@context":"https://schema.org","@type":"ItemList",name:a,description:n,url:o,numberOfItems:s.length,itemListElement:s.map((y,d)=>({"@type":"ListItem",position:d+1,name:y.name,url:`${i}/shop/${y.id}`,description:mr(y)}))}),b=JSON.stringify({"@context":"https://schema.org","@type":"BreadcrumbList",itemListElement:[{"@type":"ListItem",position:1,name:"마이뷰티맵",item:i},{"@type":"ListItem",position:2,name:`${e} ${t}`,item:o}]});return`<!DOCTYPE html>
<html lang="ko">
<head>
<meta charset="UTF-8"/>
<meta name="viewport" content="width=device-width,initial-scale=1.0,maximum-scale=1.0,user-scalable=no"/>
