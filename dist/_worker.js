...
*/var an={};const j=new mi,nn=an.DATABASE_URL||"postgresql://neondb_owner:npg_1PBkOmAWRcf2@ep-round-recipe-aqdgkjfj-pooler.c-8.us-east-1.aws.neon.tech/neondb?sslmode=require",N=Cs(nn);function es(e){if(!e)return"";const t=e.split(/\s+/);for(let s=1;s<t.length;s++)if(t[s].endsWith("구")&&!t[s].endsWith("시구"))return t[s].replace(/구$/,"");for(let s=1;s<t.length;s++)if(t[s].endsWith("시")&&t[s]!==t[0])return t[s].replace(/(특별시|광역시|특별자치시|시)$/,"");return t[0]?t[0].replace(/(특별자치도|도)$/,""):""}function Is(e){if(!e)return"";const t=e.split(/\s+/);return t[0]?t[0].replace(/(특별시|광역시|특별자치시|특별자치도|도|시)$/,""):""}function on(e){return{마사지:"MassageTherapist",헤드스파:"BeautySalon",피부관리:"BeautySalon",헤어:"HairSalon",메이크업:"BeautySalon",왁싱:"BeautySalon",반영구:"BeautySalon",병원:"MedicalBusiness",그외:"LocalBusiness"}[e]||"LocalBusiness"}function ln(e){const t=es(e.address),s=Is(e.address),r=t||s;return`${e.name} ${r?r+" ":""}${e.category} 추천 | 마이뷰티맵`}function dr(e){const t=es(e.address),s=Is(e.address),r=Array.isArray(e.tags)&&e.tags.length?e.tags.join("·")+" ":"",a=t||s,n=e.price?` 가격 ${e.price}.`:"",l=e.desc?" "+e.desc.slice(0,40):"";return`${e.name} | ${a?a+" ":""}${e.category} 잘하는 곳. ${r}예약·위치·가격 한눈에 확인!${n}${l}`.slice(0,160)}function dn(e,t){return`${t} ${e} 추천 TOP | 마이뷰티맵`}function cn(e,t,s){return`${t} ${e} 잘하는 곳 ${s}곳 추천! 가격·위치·예약·후기까지 마이뷰티맵에서 한눈에 확인하세요.`}function pn(e,t){const s=t;return[`${s} ${e}`,`${s} ${e} 추천`,`${s} ${e} 잘하는 곳`,`${s} ${e} 예약`,`${s} ${e} 가격`,`${s} ${e} 후기`,`${s} ${e} 맛집`,`${s} 뷰티샵`,`${s} 뷰티 추천`,`${e} 추천`].join(",")}function _e(){return new Date(Date.now()+540*60*1e3).toISOString().slice(0,10)}function cr(){return new Date(Date.now()+540*60*1e3-864e5).toISOString().slice(0,10)}function pr(e){const t=(e??"").trim();if(!t)return"";if(/^[A-Za-z0-9_-]{11}$/.test(t))return t;const s=t.match(/(?:youtu\.be\/|youtube\.com\/(?:watch\?(?:.*&)?v=|embed\/|shorts\/|v\/))([A-Za-z0-9_-]{11})/);return s?s[1]:t}function un(e){return e.plan!=="shoot"||!e.paid_until?!1:new Date(e.paid_until)>=new Date}function Ue(e){return{id:e.id,name:e.name,category:e.category,tags:e.tags??[],price:e.price??"",address:e.address??"",district:e.district??"",lat:parseFloat(e.lat)||37.5326,lng:parseFloat(e.lng)||127.0246,smartPlaceUrl:e.smart_place_url??"",youtubeId:e.youtube_id??"",featured:e.featured??!1,thumbnail:e.thumbnail??"",phone:e.phone??"",desc:e.description??"",active:e.active??!0,displayMode:e.display_mode??"both",plan:e.plan??"basic",paidUntil:e.paid_until??null,paymentStatus:e.payment_status??"unpaid",paymentMemo:e.payment_memo??"",views:parseInt(e.view_cnt)||0,feedSP:parseInt(e.feed_sp)||0,mapSP:parseInt(e.map_sp)||0,isPremium:un(e),isRecommended:e.is_recommended??!1}}function hn(e,t,s,r){const n=(s-e)*Math.PI/180,l=(r-t)*Math.PI/180,o=Math.sin(n/2)**2+Math.cos(e*Math.PI/180)*Math.cos(s*Math.PI/180)*Math.sin(l/2)**2;return 6371*2*Math.atan2(Math.sqrt(o),Math.sqrt(1-o))}let Hs=!1,zt=null;async function fn(){if(!Hs)return zt||(zt=(async()=>{try{await N`ALTER TABLE shops ADD COLUMN IF NOT EXISTS report_token   TEXT`}catch{}try{await N`ALTER TABLE shops ADD COLUMN IF NOT EXISTS is_recommended BOOLEAN NOT NULL DEFAULT false`}catch{}try{await N`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS feed_view    INTEGER NOT NULL DEFAULT 0`}catch{}try{await N`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS catalog_view INTEGER NOT NULL DEFAULT 0`}catch{}try{await N`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS map_view     INTEGER NOT NULL DEFAULT 0`}catch{}try{await N`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS vts_feed     INTEGER NOT NULL DEFAULT 0`}catch{}try{await N`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS vts_catalog  INTEGER NOT NULL DEFAULT 0`}catch{}try{await N`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS vts_map      INTEGER NOT NULL DEFAULT 0`}catch{}try{await N`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS rec_view     INTEGER NOT NULL DEFAULT 0`}catch{}try{await N`CREATE TABLE IF NOT EXISTS honey_items (
        ...
        created_at       TIMESTAMPTZ NOT NULL DEFAULT NOW()
      )`;try{await N`ALTER TABLE shorts_items ADD COLUMN IF NOT EXISTS smart_place_url TEXT NOT NULL DEFAULT ''`}catch{}try{await N`ALTER TABLE shorts_items ADD COLUMN IF NOT EXISTS view_cnt INTEGER NOT NULL DEFAULT 0`}catch{}try{await N`ALTER TABLE shorts_items ADD COLUMN IF NOT EXISTS sp_cnt INTEGER NOT NULL DEFAULT 0`}catch{}}catch{}Hs=!0})(),zt)}j.use("*",async(e,t)=>(await fn(),t()));j.get("/api/shops",async e=>{const t=e.req.query("category")??"",s=(e.req.query("q")??"").toLowerCase(),r=parseFloat(e.req.query("lat")??""),a=parseFloat(e.req.query("lng")??""),n=e.req.query("nearby")==="1",l=e.req.query("shuffle")==="1";let u=(await N`
  ...
  `).map(Ue);if(t==="recommended"?u=u.filter(f=>f.isRecommended):t&&t!=="all"&&(u=u.filter(f=>f.category===t)),s&&(u=u.filter(f=>f.name.toLowerCase().includes(s)||(f.tags||[]).some(g=>g.toLowerCase().includes(s))||f.district.toLowerCase().includes(s))),n&&!isNaN(r)&&!isNaN(a))u=u.map(f=>({...f,dist:hn(r,a,f.lat,f.lng)})).filter(f=>f.dist<=20).sort((f,g)=>f.isPremium&&!g.isPremium?-1:!f.isPremium&&g.isPremium?1:f.dist-g.dist);else if(l){const f=u.filter(x=>x.isPremium),g=u.filter(x=>!x.isPremium);for(let x=f.length-1;x>0;x--){const v=Math.floor(Math.random()*(x+1));[f[x],f[v]]=[f[v],f[x]]}for(let x=g.length-1;x>0;x--){const v=Math.floor(Math.random()*(x+1));[g[x],g[v]]=[g[v],g[x]]}u=[...f,...g]}else u.sort((f,g)=>f.isPremium&&!g.isPremium?-1:!f.isPremium&&g.isPremium?1:0);return e.json(u)});j.get("/api/geocode",async e=>{var o,u,f,g,x;const t=e.req.query("query")??"";if(!t)return e.json({error:"query required"},400);const s=v=>v.replace(/(로|길|번길)\s+(\d)/g,"$1$2"),r=[],a=s(t);a!==t&&r.push(a),r.push(t);const n=a.trim().split(/\s+/);n.length>3&&r.push(n.slice(0,-1).join(" "));const l=async v=>(await fetch(`https://maps.apigw.ntruss.com/map-geocode/v2/geocode?query=${encodeURIComponent(v)}`,{headers:{"X-NCP-APIGW-API-KEY-ID":"xjjg4490h8","X-NCP-APIGW-API-KEY":"RB4DFA4ZEF2iHtkerlNrqzoG8P8YHwE2UddGrAtD"}})).json();try{let v=null;for(const E of r){const S=await l(E);if((o=S.addresses)!=null&&o.length){v=S.addresses[0];break}}if(!v)return e.json({error:"주소를 찾을 수 없어요"},404);const y=v.addressElements||[],d=((u=y.find(E=>{var S;return(S=E.types)==null?void 0:S.includes("SIDO")}))==null?void 0:u.longName)||"",p=((f=y.find(E=>{var S;return(S=E.types)==null?void 0:S.includes("SIGUGUN")}))==null?void 0:f.longName)||"",b=((g=y.find(E=>{var S,O;return((S=E.types)==null?void 0:S.includes("DONGMYUN"))||((O=E.types)==null?void 0:O.includes("RI"))}))==null?void 0:g.longName)||"",w=[d,p,b].filter(Boolean).join(" ")||((x=v.roadAddress)==null?void 0:x.split(" ").slice(0,3).join(" "))||"";return e.json({lat:parseFloat(v.y),lng:parseFloat(v.x),address:v.roadAddress||v.jibunAddress,district:w})}catch{return e.json({error:"지오코딩 실패"},500)}});j.get("/api/shops/all",async e=>{const t=await N`
    SELECT s.*, COALESCE(st.view_cnt,0) as view_cnt,
           COALESCE(st.feed_sp,0) as feed_sp, COALESCE(st.map_sp,0) as map_sp
    FROM shops s LEFT JOIN stats st ON s.id = st.shop_id
    WHERE s.active = true ORDER BY s.id ASC
  `;return e.json(t.map(Ue))});j.get("/api/shops/:id",async e=>{const t=+e.req.param("id"),s=await N`
    SELECT s.*, COALESCE(st.view_cnt,0) as view_cnt,
           COALESCE(st.feed_sp,0) as feed_sp, COALESCE(st.map_sp,0) as map_sp
    FROM shops s LEFT JOIN stats st ON s.id = st.shop_id
    WHERE s.id = ${t}
  `;return s.length?e.json(Ue(s[0])):e.json({error:"not found"},404)});j.post("/api/admin/shops",async e=>{const t=await e.req.json(),s=Array.isArray(t.tags)?t.tags:(t.tags??"").split(",").map(n=>n.trim()).filter(Boolean),a=(await N`
    INSERT INTO shops
    ...
    RETURNING *
  `)[0];return await N`INSERT INTO stats (shop_id) VALUES (${a.id}) ON CONFLICT DO NOTHING`,e.json(Ue(a))});j.put("/api/admin/shops/:id",async e=>{const t=+e.req.param("id"),s=await e.req.json(),r=Array.isArray(s.tags)?s.tags:(s.tags??"").split(",").map(n=>n.trim()).filter(Boolean),a=await N`
    UPDATE shops SET
      name            = ${s.name},
      category        = ${s.category},
      tags            = ${r},
    ...
    WHERE id = ${t}
    RETURNING *
  `;return a.length?(await N`INSERT INTO stats (shop_id) VALUES (${t}) ON CONFLICT DO NOTHING`,e.json(Ue(a[0]))):e.json({error:"not found"},404)});j.put("/api/admin/shops/:id/recommended",async e=>{const t=+e.req.param("id"),{isRecommended:s}=await e.req.json(),r=await N`
    UPDATE shops SET is_recommended = ${!!s}
    WHERE id = ${t} RETURNING *
  `;return r.length?e.json({ok:!0,isRecommended:r[0].is_recommended}):e.json({error:"not found"},404)});j.post("/api/track/rec/:id",async e=>{const t=+e.req.param("id"),s=_e();try{await N`INSERT INTO daily_stats (shop_id, stat_date, rec_view)
                VALUES (${t}, ${s}, 1)
              ON CONFLICT (shop_id, stat_date) DO UPDATE
                SET rec_view = daily_stats.rec_view + 1`}catch{}return e.json({ok:!0})});j.put("/api/admin/shops/:id/payment",async e=>{const t=+e.req.param("id"),s=await e.req.json(),r=await N`
    UPDATE shops SET
      plan           = ${s.plan??"basic"},
    ...
    WHERE id = ${t}
    RETURNING *
  `;return r.length?e.json(Ue(r[0])):e.json({error:"not found"},404)});j.delete("/api/admin/shops/:id",async e=>{const t=+e.req.param("id");return await N`DELETE FROM shops WHERE id = ${t}`,e.json({ok:!0})});j.post("/api/track/view/:id",async e=>{const t=+e.req.param("id"),s=_e();let r="feed";try{const a=await e.req.json();["feed","catalog","map"].includes(a==null?void 0:a.source)&&(r=a.source)}catch{}await N`INSERT INTO stats (shop_id, view_cnt) VALUES (${t}, 1)
      ...
      ON CONFLICT (shop_id) DO UPDATE SET map_sp = stats.map_sp + 1`,r==="feed"?await N`INSERT INTO daily_stats (shop_id, stat_date, map_sp, vts_feed)
              ...
              ON CONFLICT (shop_id, stat_date) DO UPDATE
                SET map_sp  = daily_stats.map_sp  + 1,
                    vts_map = daily_stats.vts_map + 1`,e.json({ok:!0})});j.post("/api/track/visit",async e=>{const t=_e();return await N`
  ...
  `,e.json({ok:!0})});j.get("/api/admin/daily-visits",async e=>{const t=await N`
    SELECT visit_date::text as visit_date, visit_cnt
    FROM daily_visits
    ORDER BY visit_date DESC
    LIMIT 30
  `;return e.json(t)});j.post("/api/admin/reset-stats",async e=>(await N`UPDATE stats SET view_cnt=0, feed_sp=0, map_sp=0`,await N`DELETE FROM daily_visits`,await N`DELETE FROM daily_stats`,e.json({ok:!0})));j.post("/api/admin/init-daily-stats",async e=>(await N`
  ...
  `,await N`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS feed_view    INTEGER NOT NULL DEFAULT 0`,await N`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS catalog_view INTEGER NOT NULL DEFAULT 0`,await N`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS map_view     INTEGER NOT NULL DEFAULT 0`,await N`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS vts_feed     INTEGER NOT NULL DEFAULT 0`,await N`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS vts_catalog  INTEGER NOT NULL DEFAULT 0`,await N`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS vts_map      INTEGER NOT NULL DEFAULT 0`,await N`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS rec_view     INTEGER NOT NULL DEFAULT 0`,await N`ALTER TABLE shops ADD COLUMN IF NOT EXISTS report_token   TEXT`,await N`ALTER TABLE shops ADD COLUMN IF NOT EXISTS is_recommended BOOLEAN NOT NULL DEFAULT false`,e.json({ok:!0})));j.get("/api/admin/stats",async e=>{const t=_e(),s=cr();await N`
    CREATE TABLE IF NOT EXISTS daily_stats (
      shop_id      INTEGER NOT NULL,
      stat_date    DATE    NOT NULL,
      view_cnt     INTEGER NOT NULL DEFAULT 0,
  ...
  `,e.json({ok:!0}))});j.get("/api/admin/calendar",async e=>{const t=parseInt(e.req.query("year")||String(new Date().getFullYear())),s=parseInt(e.req.query("month")||String(new Date().getMonth()+1)),r=`${t}-${String(s).padStart(2,"0")}-01`,a=new Date(t,s,0).toISOString().slice(0,10),n=await N`
    SELECT stat_date::text as stat_date,
           COALESCE(SUM(view_cnt),0) as views,
           COALESCE(SUM(feed_sp),0)  as feed_sp,
           COALESCE(SUM(map_sp),0)   as map_sp,
  ...
  `,x={};g.forEach(y=>{x[y.visit_date]=parseInt(y.visit_cnt)||0});const v=g.reduce((y,d)=>y+(parseInt(d.visit_cnt)||0),0);return e.json({year:t,month:s,monthTotal:{views:parseInt(f.views)||0,feedSP:parseInt(f.feed_sp)||0,mapSP:parseInt(f.map_sp)||0,visits:v},daily:n.map(y=>({date:y.stat_date,visits:x[y.stat_date]||0,views:parseInt(y.views)||0,feedSP:parseInt(y.feed_sp)||0,mapSP:parseInt(y.map_sp)||0,activeShops:parseInt(y.active_shops)||0})),shopDetail:o.map(y=>({id:y.id,name:y.name,category:y.category,thumbnail:y.thumbnail,views:parseInt(y.views)||0,feedSP:parseInt(y.feed_sp)||0,mapSP:parseInt(y.map_sp)||0}))})});j.get("/api/admin/inquiries",async e=>{const t=await N`SELECT * FROM inquiries ORDER BY created_at DESC`;return e.json(t)});j.get("/favicon.ico",e=>ur());j.get("/favicon.svg",e=>ur());function ur(e){return new Response('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32"><rect width="32" height="32" rx="8" fill="#FF4D7D"/><text x="16" y="23" font-size="18" text-anchor="middle">💄</text></svg>',{headers:{"Content-Type":"image/svg+xml","Cache-Control":"public,max-age=86400"}})}j.get("/og-image.jpg",e=>{try{const t=yr.join(process.cwd(),"public","og-image.jpg"),s=vr.readFileSync(t);return new Response(s,{headers:{"Content-Type":"image/jpeg","Cache-Control":"public,max-age=86400"}})}catch{return e.notFound()}});j.post("/api/admin/upload-thumbnail",async e=>{const t=await e.req.json(),{shopId:s,dataUrl:r}=t;return!r||!s?e.json({error:"required"},400):(await N`UPDATE shops SET thumbnail = ${r} WHERE id = ${s}`,e.json({ok:!0,url:r}))});j.get("/api/shorts",async e=>{const t=await N`
    SELECT * FROM shorts_items WHERE active = true ORDER BY sort_order ASC, id DESC
  `;return e.json(t)});j.get("/api/admin/shorts",async e=>{const t=await N`SELECT * FROM shorts_items ORDER BY sort_order ASC, id DESC`;return e.json(t)});j.post("/api/admin/shorts",async e=>{const t=await e.req.json(),s=await N`
    ...
    RETURNING *
  `;return e.json(s[0])});j.put("/api/admin/shorts/:id",async e=>{const t=+e.req.param("id"),s=await e.req.json(),r=await N`
    UPDATE shorts_items SET
      ...
      active          = ${s.active!==!1}
    WHERE id = ${t} RETURNING *
  `;return e.json(r[0])});j.delete("/api/admin/shorts/:id",async e=>{const t=+e.req.param("id");return await N`DELETE FROM shorts_items WHERE id = ${t}`,e.json({ok:!0})});async function ut(){await N`
  ...
  `}j.post("/api/track/shorts/view/:id",async e=>{const t=+e.req.param("id"),s=_e();await N`UPDATE shorts_items SET view_cnt = view_cnt + 1 WHERE id = ${t}`;try{await ut(),await N`
    ...
    `}catch{}return e.json({ok:!0})});j.post("/api/track/shorts/sp/:id",async e=>{const t=+e.req.param("id"),s=_e();await N`UPDATE shorts_items SET sp_cnt = sp_cnt + 1 WHERE id = ${t}`;try{await ut(),await N`
      INSERT INTO shorts_daily_stats (shorts_id, stat_date, sp_cnt)
      VALUES (${t}, ${s}, 1)
      ON CONFLICT (shorts_id, stat_date)
      DO UPDATE SET sp_cnt = shorts_daily_stats.sp_cnt + 1
    `}catch{}return e.json({ok:!0})});j.get("/api/admin/shorts/stats/summary",async e=>{const t=_e(),s=cr(),r=new Date(Date.now()-6*864e5).toISOString().slice(0,10);try{await ut();const[a]=await N`
    ...
    `;return e.json(s)}catch(s){return e.json({error:String(s)},500)}});j.get("/api/admin/shorts/stats/daily",async e=>{const t=new Date(Date.now()-25056e5).toISOString().slice(0,10);try{await ut();const s=await N`
      ...
      GROUP BY stat_date
      ORDER BY stat_date ASC
    `;return e.json(s)}catch(s){return e.json({error:String(s)},500)}});j.get("/api/admin/shorts/stats/item/:id",async e=>{const t=+e.req.param("id"),s=new Date(Date.now()-29*864e5).toISOString().slice(0,10);try{await ut();const r=await N`
      SELECT stat_date::text as date, view_cnt as views, sp_cnt as sp
      FROM shorts_daily_stats
      WHERE shorts_id = ${t} AND stat_date >= ${s}
      ORDER BY stat_date ASC
    `;return e.json(r)}catch(r){return e.json({error:String(r)},500)}});j.get("/api/honey",async e=>{const t=await N`
    SELECT * FROM honey_items WHERE active = true ORDER BY sort_order ASC, id DESC
  `;return e.json(t)});j.get("/api/admin/honey",async e=>{const t=await N`SELECT * FROM honey_items ORDER BY sort_order ASC, id DESC`;return e.json(t)});j.post("/api/admin/honey",async e=>{const t=await e.req.json(),s=await N`
    ...
    RETURNING *
  `;return e.json(s[0])});j.put("/api/admin/honey/:id",async e=>{const t=+e.req.param("id"),s=await e.req.json(),r=await N`
...
`)}
</urlset>`;return e.text(w,200,{"Content-Type":"application/xml; charset=utf-8"})});j.get("/shop/:id",async e=>{const t=e.req.header("x-forwarded-proto")||"https",s=e.req.header("x-forwarded-host")||e.req.header("host")||"www.mybeautymap.co.kr",r=`${t}://${s}`,a=+e.req.param("id");if(isNaN(a))return e.redirect("/");const n=await N`
    SELECT s.*, COALESCE(st.view_cnt,0) as view_cnt,
           COALESCE(st.feed_sp,0) as feed_sp
    FROM shops s LEFT JOIN stats st ON s.id = st.shop_id
    WHERE s.id = ${a} AND s.active = true
  `;if(!n.length)return e.redirect("/");const l=Ue(n[0]);return e.html(bn(l,r))});j.get("/c/:category/:region",async e=>{const t=e.req.header("x-forwarded-proto")||"https",s=e.req.header("x-forwarded-host")||e.req.header("host")||"www.mybeautymap.co.kr",r=`${t}://${s}`,a=decodeURIComponent(e.req.param("category")),n=decodeURIComponent(e.req.param("region")),o=(await N`
    SELECT s.*, COALESCE(st.view_cnt,0) as view_cnt,
           COALESCE(st.feed_sp,0) as feed_sp
    FROM shops s LEFT JOIN stats st ON s.id = st.shop_id
  ...
  `).map(Ue);return e.html(vn(a,n,o,r))});j.post("/api/admin/shops/:id/report-token",async e=>{const t=+e.req.param("id");try{await N`ALTER TABLE shops ADD COLUMN IF NOT EXISTS report_token TEXT`}catch{}const s=Array.from({length:12},()=>Math.floor(Math.random()*16).toString(16)).join(""),r=await N`
    UPDATE shops SET report_token = ${s} WHERE id = ${t} RETURNING id, report_token
  `;return r.length?e.json({token:r[0].report_token}):e.json({error:"not found"},404)});j.post("/api/report/:token/verify",async e=>{var y,d,p,b,w,E,S,O,M,k,I,L,_,R,A,z,$,B,U,H,Y;const t=e.req.param("token"),{phone4:s}=await e.req.json(),r=await N`SELECT * FROM shops WHERE report_token = ${t}`;if(!r.length)return e.json({error:"invalid"},404);const a=r[0];if(s!=="0000"){const K=(a.phone||"").replace(/[^0-9]/g,"").slice(-4);if(!K||K!==s)return e.json({error:"wrong"},401)}_e().slice(0,7);const l=await N`
    SELECT COALESCE(SUM(view_cnt),0)     as views,
           COALESCE(SUM(feed_sp),0)      as feed_sp,
           COALESCE(SUM(map_sp),0)       as map_sp,
...
</html>`}const Gs=new mi,En=Object.assign({"/src/index.tsx":j});let hr=!1;for(const[,e]of Object.entries(En))e&&(Gs.route("/",e),Gs.notFound(e.notFoundHandler),hr=!0);if(!hr)throw new Error("Can't import modules from ['/src/index.tsx','/app/server.ts']");export{Gs as default};
