...
*/const j=new Jr,Zn=process.env.DATABASE_URL||"postgresql://neondb_owner:npg_1PBkOmAWRcf2@ep-round-recipe-aqdgkjfj-pooler.c-8.us-east-1.aws.neon.tech/neondb?sslmode=require",B=tr(Zn);function xs(e){if(!e)return"";const t=e.split(/\s+/);for(let s=1;s<t.length;s++)if(t[s].endsWith("구")&&!t[s].endsWith("시구"))return t[s].replace(/구$/,"");for(let s=1;s<t.length;s++)if(t[s].endsWith("시")&&t[s]!==t[0])return t[s].replace(/(특별시|광역시|특별자치시|시)$/,"");return t[0]?t[0].replace(/(특별자치도|도)$/,""):""}function sr(e){if(!e)return"";const t=e.split(/\s+/);return t[0]?t[0].replace(/(특별시|광역시|특별자치시|특별자치도|도|시)$/,""):""}function eo(e){return{마사지:"MassageTherapist",헤드스파:"BeautySalon",피부관리:"BeautySalon",헤어:"HairSalon",메이크업:"BeautySalon",왁싱:"BeautySalon",반영구:"BeautySalon",병원:"MedicalBusiness",그외:"LocalBusiness"}[e]||"LocalBusiness"}function to(e){const t=xs(e.address),s=sr(e.address),r=t||s;return`${e.name} ${r?r+" ":""}${e.category} 추천 | 마이뷰티맵`}function Vi(e){const t=xs(e.address),s=sr(e.address),r=Array.isArray(e.tags)&&e.tags.length?e.tags.join("·")+" ":"",i=t||s,a=e.price?` 가격 ${e.price}.`:"",d=e.desc?" "+e.desc.slice(0,40):"";return`${e.name} | ${i?i+" ":""}${e.category} 잘하는 곳. ${r}예약·위치·가격 한눈에 확인!${a}${d}`.slice(0,160)}function so(e,t){return`${t} ${e} 추천 TOP | 마이뷰티맵`}function ro(e,t,s){return`${t} ${e} 잘하는 곳 ${s}곳 추천! 가격·위치·예약·후기까지 마이뷰티맵에서 한눈에 확인하세요.`}function io(e,t){const s=t;return[`${s} ${e}`,`${s} ${e} 추천`,`${s} ${e} 잘하는 곳`,`${s} ${e} 예약`,`${s} ${e} 가격`,`${s} ${e} 후기`,`${s} ${e} 맛집`,`${s} 뷰티샵`,`${s} 뷰티 추천`,`${e} 추천`].join(",")}function ve(){return new Date(Date.now()+540*60*1e3).toISOString().slice(0,10)}function Wi(){return new Date(Date.now()+540*60*1e3-864e5).toISOString().slice(0,10)}function Gi(e){const t=(e??"").trim();if(!t)return"";if(/^[A-Za-z0-9_-]{11}$/.test(t))return t;const s=t.match(/(?:youtu\.be\/|youtube\.com\/(?:watch\?(?:.*&)?v=|embed\/|shorts\/|v\/))([A-Za-z0-9_-]{11})/);return s?s[1]:t}function ao(e){return e.plan!=="shoot"||!e.paid_until?!1:new Date(e.paid_until)>=new Date}function $e(e){return{id:e.id,name:e.name,category:e.category,tags:e.tags??[],price:e.price??"",address:e.address??"",district:e.district??"",lat:parseFloat(e.lat)||37.5326,lng:parseFloat(e.lng)||127.0246,smartPlaceUrl:e.smart_place_url??"",youtubeId:e.youtube_id??"",featured:e.featured??!1,thumbnail:e.thumbnail??"",phone:e.phone??"",desc:e.description??"",active:e.active??!0,displayMode:e.display_mode??"both",plan:e.plan??"basic",paidUntil:e.paid_until??null,paymentStatus:e.payment_status??"unpaid",paymentMemo:e.payment_memo??"",views:parseInt(e.view_cnt)||0,feedSP:parseInt(e.feed_sp)||0,mapSP:parseInt(e.map_sp)||0,isPremium:ao(e),isRecommended:e.is_recommended??!1}}function no(e,t,s,r){const a=(s-e)*Math.PI/180,d=(r-t)*Math.PI/180,o=Math.sin(a/2)**2+Math.cos(e*Math.PI/180)*Math.cos(s*Math.PI/180)*Math.sin(d/2)**2;return 6371*2*Math.atan2(Math.sqrt(o),Math.sqrt(1-o))}const zs=10;let os=!1,dt=null;async function Qi(){if(!os)return dt||(dt=(async()=>{try{if(await B`
      ...
      `,await B`CREATE INDEX IF NOT EXISTS idx_se_session  ON session_events(session_id)`,await B`CREATE INDEX IF NOT EXISTS idx_se_occurred ON session_events(occurred_at)`,await B`ALTER TABLE shops             ADD COLUMN IF NOT EXISTS report_token   TEXT`,await B`ALTER TABLE shops             ADD COLUMN IF NOT EXISTS is_recommended BOOLEAN NOT NULL DEFAULT false`,await B`ALTER TABLE daily_stats       ADD COLUMN IF NOT EXISTS feed_view      INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats       ADD COLUMN IF NOT EXISTS catalog_view   INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats       ADD COLUMN IF NOT EXISTS map_view       INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats       ADD COLUMN IF NOT EXISTS vts_feed       INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats       ADD COLUMN IF NOT EXISTS vts_catalog    INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats       ADD COLUMN IF NOT EXISTS vts_map        INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE daily_stats       ADD COLUMN IF NOT EXISTS rec_view       INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE honey_items       ADD COLUMN IF NOT EXISTS coupang_url2   TEXT    NOT NULL DEFAULT ''`,await B`ALTER TABLE shorts_items      ADD COLUMN IF NOT EXISTS smart_place_url TEXT   NOT NULL DEFAULT ''`,await B`ALTER TABLE shorts_items      ADD COLUMN IF NOT EXISTS view_cnt        INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE shorts_items      ADD COLUMN IF NOT EXISTS sp_cnt          INTEGER NOT NULL DEFAULT 0`,await B`ALTER TABLE session_events    ADD COLUMN IF NOT EXISTS viewed_sec      INTEGER NOT NULL DEFAULT 0`,await B`INSERT INTO schema_migrations (version) VALUES (${zs}) ON CONFLICT DO NOTHING`,os=!0}catch(e){console.error("[migration] error:",e),dt=null}})(),dt)}j.use("*",async(e,t)=>(await Qi(),t()));j.get("/api/shops",async e=>{const t=e.req.query("category")??"",s=(e.req.query("q")??"").toLowerCase(),r=parseFloat(e.req.query("lat")??""),i=parseFloat(e.req.query("lng")??""),a=e.req.query("nearby")==="1",d=e.req.query("shuffle")==="1";let p=(await B`
  ...
  `,e.json({ok:!0})});j.get("/api/admin/daily-visits",async e=>{const t=await B`
    ...
    LIMIT 30
  `;return e.json(t)});j.post("/api/admin/reset-stats",async e=>(await B`UPDATE stats SET view_cnt=0, feed_sp=0, map_sp=0`,await B`DELETE FROM daily_visits`,await B`DELETE FROM daily_stats`,e.json({ok:!0})));j.post("/api/admin/init-daily-stats",async e=>(os=!1,dt=null,await B`DELETE FROM schema_migrations WHERE version = ${zs}`,await Qi(),e.json({ok:!0})));j.get("/api/admin/stats",async e=>{const t=ve(),s=Wi(),r=await B`
  ...
  `,p={};o.forEach(v=>{p[v.shop_id]=parseInt(v.total_rec)||0});const f=await B`
    ...
    FROM stats
  `,h=await B`SELECT COUNT(*) as cnt FROM shops WHERE active = true`,y=f[0]||{},b=i[0]||{},x=a[0]||{},l=await B`
    SELECT shop_id,
  ...
  `;return e.json(s[0])});j.put("/api/admin/shorts/:id",async e=>{const t=+e.req.param("id"),s=await e.req.json(),r=await B`
    UPDATE shorts_items SET
      ...
      active          = ${s.active!==!1}
    WHERE id = ${t} RETURNING *
  `;return e.json(r[0])});j.delete("/api/admin/shorts/:id",async e=>{const t=+e.req.param("id");return await B`DELETE FROM shorts_items WHERE id = ${t}`,e.json({ok:!0})});let kr=!1;async function wt(){kr||(await B`
    CREATE TABLE IF NOT EXISTS shorts_daily_stats (
      shorts_id  INTEGER NOT NULL,
      stat_date  DATE    NOT NULL,
      view_cnt   INTEGER NOT NULL DEFAULT 0,
  ...
  `,kr=!0)}j.post("/api/track/shorts/view/:id",async e=>{const t=+e.req.param("id"),s=ve();await B`UPDATE shorts_items SET view_cnt = view_cnt + 1 WHERE id = ${t}`;try{await wt(),await B`
    ...
    `;return e.json(s)}catch(s){return e.json({error:String(s)},500)}});j.get("/api/admin/shorts/stats/item/:id",async e=>{const t=+e.req.param("id"),s=new Date(Date.now()-29*864e5).toISOString().slice(0,10);try{await wt();const r=await B`
      ...
      ORDER BY stat_date ASC
    `;return e.json(r)}catch(r){return e.json({error:String(r)},500)}});let Sr=!1;async function Qt(){Sr||(await B`
    CREATE TABLE IF NOT EXISTS visitor_sessions (
      id            TEXT    PRIMARY KEY,
      entered_at    TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      last_seen     TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  ...
  `,Sr=!0)}j.post("/api/track/session/start",async e=>{try{await Qt(),await B`DELETE FROM visitor_sessions WHERE entered_at < NOW() - INTERVAL '7 days'`;const t=await e.req.json(),s=t.id||"",r=t.device||"unknown";return s?(await B`
...
`)}
  ...
  `;if(!a.length)return e.redirect("/");const d=$e(a[0]);return e.html(co(d,r))});j.get("/c/:category/:region",async e=>{const t=e.req.header("x-forwarded-proto")||"https",s=e.req.header("x-forwarded-host")||e.req.header("host")||"www.mybeautymap.co.kr",r=`${t}://${s}`,i=decodeURIComponent(e.req.param("category")),a=decodeURIComponent(e.req.param("region")),o=(await B`
    SELECT s.*, COALESCE(st.view_cnt,0) as view_cnt,
           COALESCE(st.feed_sp,0) as feed_sp
    FROM shops s LEFT JOIN stats st ON s.id = st.shop_id
...
</html>`}function po(e,t,s,r){const i=so(e,t),a=ro(e,t,s.length),d=io(e,t),o=`${r}/c/${encodeURIComponent(e)}/${encodeURIComponent(t)}`,p={마사지:"💆",헤드스파:"🧖",피부관리:"✨",헤어:"💇",메이크업:"💄",왁싱:"🌸",반영구:"👁",병원:"🏥",그외:"🌟"},f=p[e]||"🌟",h=["마사지","헤드스파","피부관리","헤어","메이크업","왁싱","반영구","병원","그외"],y=JSON.stringify({"@context":"https://schema.org","@type":"ItemList",name:i,description:a,url:o,numberOfItems:s.length,itemListElement:s.map((x,l)=>({"@type":"ListItem",position:l+1,name:x.name,url:`${r}/shop/${x.id}`,description:Vi(x)}))}),b=JSON.stringify({"@context":"https://schema.org","@type":"BreadcrumbList",itemListElement:[{"@type":"ListItem",position:1,name:"마이뷰티맵",item:r},{"@type":"ListItem",position:2,name:`${t} ${e}`,item:o}]});return`<!DOCTYPE html>
<html lang="ko">
<head>
<meta charset="UTF-8"/>
<meta name="viewport" content="width=device-width,initial-scale=1.0,maximum-scale=1.0,user-scalable=no"/>
...
</html>`}var go=(e,t=bo)=>{const s=/\.([a-zA-Z0-9]+?)$/,r=e.match(s);if(!r)return;let i=t[r[1].toLowerCase()];return i&&i.startsWith("text")&&(i+="; charset=utf-8"),i},mo={aac:"audio/aac",avi:"video/x-msvideo",avif:"image/avif",av1:"video/av1",bin:"application/octet-stream",bmp:"image/bmp",css:"text/css",csv:"text/csv",eot:"application/vnd.ms-fontobject",epub:"application/epub+zip",gif:"image/gif",gz:"application/gzip",htm:"text/html",html:"text/html",ico:"image/x-icon",ics:"text/calendar",jpeg:"image/jpeg",jpg:"image/jpeg",js:"text/javascript",json:"application/json",jsonld:"application/ld+json",map:"application/json",mid:"audio/x-midi",midi:"audio/x-midi",mjs:"text/javascript",mp3:"audio/mpeg",mp4:"video/mp4",mpeg:"video/mpeg",oga:"audio/ogg",ogv:"video/ogg",ogx:"application/ogg",opus:"audio/opus",otf:"font/otf",pdf:"application/pdf",png:"image/png",rtf:"application/rtf",svg:"image/svg+xml",tif:"image/tiff",tiff:"image/tiff",ts:"video/mp2t",ttf:"font/ttf",txt:"text/plain",wasm:"application/wasm",webm:"video/webm",weba:"audio/webm",webmanifest:"application/manifest+json",webp:"image/webp",woff:"font/woff",woff2:"font/woff2",xhtml:"application/xhtml+xml",xml:"application/xml",zip:"application/zip","3gp":"video/3gpp","3g2":"video/3gpp2",gltf:"model/gltf+json",glb:"model/gltf-binary"},bo=mo,vo=/^\s*(?:text\/[^;\s]+|application\/(?:javascript|json|xml|xml-dtd|ecmascript|dart|postscript|rtf|tar|toml|vnd\.dart|vnd\.ms-fontobject|vnd\.ms-opentype|wasm|x-httpd-php|x-javascript|x-ns-proxy-autoconfig|x-sh|x-tar|x-virtualbox-hdd|x-virtualbox-ova|x-virtualbox-ovf|x-virtualbox-vbox|x-virtualbox-vdi|x-virtualbox-vhd|x-virtualbox-vmdk|x-www-form-urlencoded)|font\/(?:otf|ttf)|image\/(?:bmp|vnd\.adobe\.photoshop|vnd\.microsoft\.icon|vnd\.ms-dds|x-icon|x-ms-bmp)|message\/rfc822|model\/gltf-binary|x-shader\/x-fragment|x-shader\/x-vertex|[^;\s]+?\+(?:json|text|xml|yaml))(?:[;\s]|$)/i,js={br:".br",zstd:".zst",gzip:".gz"},yo=Object.keys(js),xo=()=>{const[e,t]=ca.node.split(".").map(s=>parseInt(s));return e>=23||e===22&&t>=7||e===20&&t>=18},wo=xo(),Lr=e=>wo?Ds.toWeb(e):new ReadableStream({start(s){e.on("data",r=>{s.enqueue(r)}),e.on("error",r=>{s.error(r)}),e.on("end",()=>{s.close()})},cancel(){e.destroy()}}),Os=e=>{let t;try{t=da(e)}catch{}return t},_o=(e,t)=>{try{return t(e)}catch{return e.replace(/(?:%[0-9A-Fa-f]{2})+/g,s=>{try{return t(s)}catch{return s}})}},Eo=e=>_o(e,decodeURI),_t=(e={root:""})=>{const t=e.root||"",s=e.path;return t!==""&&!oa(t)&&console.error(`serveStatic: root path '${t}' is not found, are you sure it's correct?`),async(r,i)=>{var b,x,l,u;if(r.finalized)return i();let a;if(s)a=s;else try{if(a=Eo(r.req.path),/(?:^|[\/\\])\.{1,2}(?:$|[\/\\])|[\/\\]{2,}/.test(a))throw new Error}catch{return await((b=e.onNotFound)==null?void 0:b.call(e,r.req.path,r)),i()}let d=hr(t,!s&&e.rewriteRequestPath?e.rewriteRequestPath(a,r):a),o=Os(d);if(o&&o.isDirectory()){const v=e.index??"index.html";d=hr(d,v),o=Os(d)}if(!o)return await((x=e.onNotFound)==null?void 0:x.call(e,d,r)),i();const p=go(d);if(r.header("Content-Type",p||"application/octet-stream"),e.precompressed&&(!p||vo.test(p))){const v=new Set((l=r.req.header("Accept-Encoding"))==null?void 0:l.split(",").map(w=>w.trim()));for(const w of yo){if(!v.has(w))continue;const _=Os(d+js[w]);if(_){r.header("Content-Encoding",w),r.header("Vary","Accept-Encoding",{append:!0}),o=_,d=d+js[w];break}}}let f;const h=o.size,y=r.req.header("range")||"";if(r.req.method=="HEAD"||r.req.method=="OPTIONS")r.header("Content-Length",h.toString()),r.status(200),f=r.body(null);else if(!y)r.header("Content-Length",h.toString()),f=r.body(Lr(fr(d)),200);else{r.header("Accept-Ranges","bytes"),r.header("Date",o.birthtime.toUTCString());const v=y.replace(/bytes=/,"").split("-",2),w=parseInt(v[0],10)||0;let _=parseInt(v[1],10)||h-1;h<_-w+1&&(_=h-1);const E=_-w+1,R=fr(d,{start:w,end:_});r.header("Content-Length",E.toString()),r.header("Content-Range",`bytes ${w}-${_}/${o.size}`),f=r.body(Lr(R),206)}return await((u=e.onFound)==null?void 0:u.call(e,d,r)),f}},We=class extends Error{constructor(e,t){super(e,t),this.name="RequestError"}},ko=e=>e instanceof We?e:new We(e.message,{cause:e}),So=global.Request,Bt=class extends So{constructor(t,s){var r;typeof t=="object"&&yt in t&&(t=t[yt]()),typeof((r=s==null?void 0:s.body)==null?void 0:r.getReader)<"u"&&(s.duplex??(s.duplex="half")),super(t,s)}},To=e=>{const t=[],s=e.rawHeaders;for(let r=0;r<s.length;r+=2){const{[r]:i,[r+1]:a}=s;i.charCodeAt(0)!==58&&t.push([i,a])}return new Headers(t)},Xi=Symbol("wrapBodyStream"),Co=(e,t,s,r,i)=>{const a={method:e,headers:s,signal:i.signal};if(e==="TRACE"){a.method="GET";const d=new Bt(t,a);return Object.defineProperty(d,"method",{get(){return"TRACE"}}),d}if(!(e==="GET"||e==="HEAD"))if("rawBody"in r&&r.rawBody instanceof Buffer)a.body=new ReadableStream({start(d){d.enqueue(r.rawBody),d.close()}});else if(r[Xi]){let d;a.body=new ReadableStream({async pull(o){try{d||(d=Ds.toWeb(r).getReader());const{done:p,value:f}=await d.read();p?o.close():o.enqueue(f)}catch(p){o.error(p)}}})}else a.body=Ds.toWeb(r);return new Bt(t,a)},yt=Symbol("getRequestCache"),ds=Symbol("requestCache"),ls=Symbol("incomingKey"),ps=Symbol("urlKey"),Ms=Symbol("headersKey"),De=Symbol("abortControllerKey"),Io=Symbol("getAbortController"),Yt={get method(){return this[ls].method||"GET"},get url(){return this[ps]},get headers(){return this[Ms]||(this[Ms]=To(this[ls]))},[Io](){return this[yt](),this[De]},[yt](){return this[De]||(this[De]=new AbortController),this[ds]||(this[ds]=Co(this.method,this[ps],this.headers,this[ls],this[De]))}};["body","bodyUsed","cache","credentials","destination","integrity","mode","redirect","referrer","referrerPolicy","signal","keepalive"].forEach(e=>{Object.defineProperty(Yt,e,{get(){return this[yt]()[e]}})});["arrayBuffer","blob","clone","formData","json","text"].forEach(e=>{Object.defineProperty(Yt,e,{value:function(){return this[yt]()[e]()}})});Object.defineProperty(Yt,Symbol.for("nodejs.util.inspect.custom"),{value:function(e,t,s){const r={method:this.method,url:this.url,headers:this.headers,nativeRequest:this[ds]};return`Request (lightweight) ${s(r,{...t,depth:e==null?null:e-1})}`}});Object.setPrototypeOf(Yt,Bt.prototype);var Lo=(e,t)=>{const s=Object.create(Yt);s[ls]=e;const r=e.url||"";if(r[0]!=="/"&&(r.startsWith("http://")||r.startsWith("https://"))){if(e instanceof Ot)throw new We("Absolute URL for :path is not allowed in HTTP/2");try{const o=new URL(r);s[ps]=o.href}catch(o){throw new We("Invalid absolute URL",{cause:o})}return s}const i=(e instanceof Ot?e.authority:e.headers.host)||t;if(!i)throw new We("Missing host header");let a;if(e instanceof Ot){if(a=e.scheme,!(a==="http"||a==="https"))throw new We("Unsupported scheme")}else a=e.socket&&e.socket.encrypted?"https":"http";const d=new URL(`${a}://${i}${r}`);if(d.hostname.length!==i.length&&d.hostname!==i.replace(/:\d+$/,""))throw new We("Invalid host header");return s[ps]=d.href,s},Rt=Symbol("responseCache"),ct=Symbol("getResponseCache"),Ge=Symbol("cache"),ir=global.Response,Ht,Oe,vt,Et=(vt=class{constructor(t,s){J(this,Ht);J(this,Oe);let r;if(W(this,Ht,t),s instanceof vt){const i=s[Rt];if(i){W(this,Oe,i),this[ct]();return}else W(this,Oe,O(s,Oe)),r=new Headers(O(s,Oe).headers)}else W(this,Oe,s);(typeof t=="string"||typeof(t==null?void 0:t.getReader)<"u"||t instanceof Blob||t instanceof Uint8Array)&&(this[Ge]=[(s==null?void 0:s.status)||200,t,r||(s==null?void 0:s.headers)])}[ct](){return delete this[Ge],this[Rt]||(this[Rt]=new ir(O(this,Ht),O(this,Oe)))}get headers(){const t=this[Ge];return t?(t[2]instanceof Headers||(t[2]=new Headers(t[2]||{"content-type":"text/plain; charset=UTF-8"})),t[2]):this[ct]().headers}get status(){var t;return((t=this[Ge])==null?void 0:t[0])??this[ct]().status}get ok(){const t=this.status;return t>=200&&t<300}},Ht=new WeakMap,Oe=new WeakMap,vt);["body","bodyUsed","redirected","statusText","trailers","type","url"].forEach(e=>{Object.defineProperty(Et.prototype,e,{get(){return this[ct]()[e]}})});["arrayBuffer","blob","clone","formData","json","text"].forEach(e=>{Object.defineProperty(Et.prototype,e,{value:function(){return this[ct]()[e]()}})});Object.defineProperty(Et.prototype,Symbol.for("nodejs.util.inspect.custom"),{value:function(e,t,s){const r={status:this.status,headers:this.headers,ok:this.ok,nativeResponse:this[Rt]};return`Response (lightweight) ${s(r,{...t,depth:e==null?null:e-1})}`}});Object.setPrototypeOf(Et,ir);Object.setPrototypeOf(Et.prototype,ir.prototype);async function Ao(e){return Promise.race([e,Promise.resolve().then(()=>Promise.resolve(void 0))])}function Ki(e,t,s){const r=o=>{e.cancel(o).catch(()=>{})};return t.on("close",r),t.on("error",r),(s??e.read()).then(d,i),e.closed.finally(()=>{t.off("close",r),t.off("error",r)});function i(o){o&&t.destroy(o)}function a(){e.read().then(d,i)}function d({done:o,value:p}){try{if(o)t.end();else if(!t.write(p))t.once("drain",a);else return e.read().then(d,i)}catch(f){i(f)}}}function Ro(e,t){if(e.locked)throw new TypeError("ReadableStream is locked.");return t.destroyed?void 0:Ki(e.getReader(),t)}var $s=e=>{const t={};e instanceof Headers||(e=new Headers(e??void 0));const s=[];for(const[r,i]of e)r==="set-cookie"?s.push(i):t[r]=i;return s.length>0&&(t["set-cookie"]=s),t["content-type"]??(t["content-type"]="text/plain; charset=UTF-8"),t},Oo="x-hono-already-sent";typeof global.crypto>"u"&&(global.crypto=fa);var ar=Symbol("outgoingEnded"),Ar=Symbol("incomingDraining"),Mo=500,No=64*1024*1024,Ns=e=>{var o,p,f;const t=e;if(e.destroyed||t[Ar])return;if(t[Ar]=!0,e instanceof Ot){try{(p=(o=e.stream)==null?void 0:o.close)==null||p.call(o,ua.NGHTTP2_NO_ERROR)}catch{}return}let s=0;const r=()=>{clearTimeout(a),e.off("data",d),e.off("end",r),e.off("error",r)},i=()=>{r();const h=e.socket;h&&!h.destroyed&&h.destroySoon()},a=setTimeout(i,Mo);(f=a.unref)==null||f.call(a);const d=h=>{s+=h.length,s>No&&i()};e.on("data",d),e.on("end",r),e.on("error",r),e.resume()},Do=()=>new Response(null,{status:400}),Ji=e=>new Response(null,{status:e instanceof Error&&(e.name==="TimeoutError"||e.constructor.name==="TimeoutError")?504:500}),qs=(e,t)=>{const s=e instanceof Error?e:new Error("unknown error",{cause:e});s.code==="ERR_STREAM_PREMATURE_CLOSE"?console.info("The user aborted a request."):(console.error(e),t.headersSent||t.writeHead(500,{"Content-Type":"text/plain"}),t.end(`Error: ${s.message}`),t.destroy(s))},Zi=e=>{"flushHeaders"in e&&e.writable&&e.flushHeaders()},ea=async(e,t)=>{var d,o;let[s,r,i]=e[Ge],a=!1;if(!i)i={"content-type":"text/plain; charset=UTF-8"};else if(i instanceof Headers)a=i.has("content-length"),i=$s(i);else if(Array.isArray(i)){const p=new Headers(i);a=p.has("content-length"),i=$s(p)}else for(const p in i)if(p.length===14&&p.toLowerCase()==="content-length"){a=!0;break}a||(typeof r=="string"?i["Content-Length"]=Buffer.byteLength(r):r instanceof Uint8Array?i["Content-Length"]=r.byteLength:r instanceof Blob&&(i["Content-Length"]=r.size)),t.writeHead(s,i),typeof r=="string"||r instanceof Uint8Array?t.end(r):r instanceof Blob?t.end(new Uint8Array(await r.arrayBuffer())):(Zi(t),await((d=Ro(r,t))==null?void 0:d.catch(p=>qs(p,t)))),(o=t[ar])==null||o.call(t)},Po=e=>typeof e.then=="function",Bo=async(e,t,s={})=>{var i;if(Po(e))if(s.errorHandler)try{e=await e}catch(a){const d=await s.errorHandler(a);if(!d)return;e=d}else e=await e.catch(Ji);if(Ge in e)return ea(e,t);const r=$s(e.headers);if(e.body){const a=e.body.getReader(),d=[];let o=!1,p;if(r["transfer-encoding"]!=="chunked"){let f=2;for(let h=0;h<f;h++){p||(p=a.read());const y=await Ao(p).catch(b=>{console.error(b),o=!0});if(!y){if(h===1){await new Promise(b=>setTimeout(b)),f=3;continue}break}if(p=void 0,y.value&&d.push(y.value),y.done){o=!0;break}}o&&!("content-length"in r)&&(r["content-length"]=d.reduce((h,y)=>h+y.length,0))}t.writeHead(e.status,r),d.forEach(f=>{t.write(f)}),o?t.end():(d.length===0&&Zi(t),await Ki(a,t,p))}else r[Oo]||(t.writeHead(e.status,r),t.end());(i=t[ar])==null||i.call(t)},Fo=(e,t={})=>{const s=t.autoCleanupIncoming??!0;return t.overrideGlobalObjects!==!1&&global.Request!==Bt&&(Object.defineProperty(global,"Request",{value:Bt}),Object.defineProperty(global,"Response",{value:Et})),async(r,i)=>{let a,d;try{d=Lo(r,t.hostname);let o=!s||r.method==="GET"||r.method==="HEAD";if(o||(r[Xi]=!0,r.on("end",()=>{o=!0}),r instanceof Ot&&(i[ar]=()=>{o||setTimeout(()=>{o||setTimeout(()=>{Ns(r)})})}),i.on("finish",()=>{o||Ns(r)})),i.on("close",()=>{d[De]&&(r.errored?d[De].abort(r.errored.toString()):i.writableFinished||d[De].abort("Client connection prematurely closed.")),o||setTimeout(()=>{o||setTimeout(()=>{Ns(r)})})}),a=e(d,{incoming:r,outgoing:i}),Ge in a)return ea(a,i)}catch(o){if(a)return qs(o,i);if(t.errorHandler){if(a=await t.errorHandler(d?o:ko(o)),!a)return}else d?a=Ji(o):a=Do()}try{return await Bo(a,i,t)}catch(o){return qs(o,i)}}},Uo=e=>{const t=e.fetch,s=Fo(t,{hostname:e.hostname,overrideGlobalObjects:e.overrideGlobalObjects,autoCleanupIncoming:e.autoCleanupIncoming});return(e.createServer||pa)(e.serverOptions||{},s)},zo=(e,t)=>{const s=Uo(e);return s.listen(e==null?void 0:e.port,e.hostname,()=>{s.address()}),s};const Me=new Jr;Me.use("/favicon.svg",_t({root:"./"}));Me.use("/og-image.jpg",_t({root:"./"}));Me.use("/static/*",_t({root:"./"}));Me.use("/.gitignore",_t({root:"./"}));Me.use("/_routes.json",_t({root:"./"}));Me.use("/_worker.js",_t({root:"./"}));const jo=Object.assign({"/src/index.tsx":j});let ta=!1;for(const[,e]of Object.entries(jo))e&&(Me.all("*",t=>{let s;try{s=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,s)}),Me.notFound(t=>{let s;try{s=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,s)}),ta=!0);if(!ta)throw new Error("Can't import modules from ['/src/index.ts','/src/index.tsx','/app/server.ts']");zo({fetch:Me.fetch,port:3e3});export{Me as default};
