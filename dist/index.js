...
*/const j=new Gr,Gn=process.env.DATABASE_URL||"postgresql://neondb_owner:npg_1PBkOmAWRcf2@ep-round-recipe-aqdgkjfj-pooler.c-8.us-east-1.aws.neon.tech/neondb?sslmode=require",P=Zs(Gn);function vs(e){if(!e)return"";const t=e.split(/\s+/);for(let s=1;s<t.length;s++)if(t[s].endsWith("구")&&!t[s].endsWith("시구"))return t[s].replace(/구$/,"");for(let s=1;s<t.length;s++)if(t[s].endsWith("시")&&t[s]!==t[0])return t[s].replace(/(특별시|광역시|특별자치시|시)$/,"");return t[0]?t[0].replace(/(특별자치도|도)$/,""):""}function er(e){if(!e)return"";const t=e.split(/\s+/);return t[0]?t[0].replace(/(특별시|광역시|특별자치시|특별자치도|도|시)$/,""):""}function Qn(e){return{마사지:"MassageTherapist",헤드스파:"BeautySalon",피부관리:"BeautySalon",헤어:"HairSalon",메이크업:"BeautySalon",왁싱:"BeautySalon",반영구:"BeautySalon",병원:"MedicalBusiness",그외:"LocalBusiness"}[e]||"LocalBusiness"}function Yn(e){const t=vs(e.address),s=er(e.address),r=t||s;return`${e.name} ${r?r+" ":""}${e.category} 추천 | 마이뷰티맵`}function zi(e){const t=vs(e.address),s=er(e.address),r=Array.isArray(e.tags)&&e.tags.length?e.tags.join("·")+" ":"",i=t||s,a=e.price?` 가격 ${e.price}.`:"",l=e.desc?" "+e.desc.slice(0,40):"";return`${e.name} | ${i?i+" ":""}${e.category} 잘하는 곳. ${r}예약·위치·가격 한눈에 확인!${a}${l}`.slice(0,160)}function Xn(e,t){return`${t} ${e} 추천 TOP | 마이뷰티맵`}function Kn(e,t,s){return`${t} ${e} 잘하는 곳 ${s}곳 추천! 가격·위치·예약·후기까지 마이뷰티맵에서 한눈에 확인하세요.`}function Jn(e,t){const s=t;return[`${s} ${e}`,`${s} ${e} 추천`,`${s} ${e} 잘하는 곳`,`${s} ${e} 예약`,`${s} ${e} 가격`,`${s} ${e} 후기`,`${s} ${e} 맛집`,`${s} 뷰티샵`,`${s} 뷰티 추천`,`${e} 추천`].join(",")}function ve(){return new Date(Date.now()+540*60*1e3).toISOString().slice(0,10)}function $i(){return new Date(Date.now()+540*60*1e3-864e5).toISOString().slice(0,10)}function ji(e){const t=(e??"").trim();if(!t)return"";if(/^[A-Za-z0-9_-]{11}$/.test(t))return t;const s=t.match(/(?:youtu\.be\/|youtube\.com\/(?:watch\?(?:.*&)?v=|embed\/|shorts\/|v\/))([A-Za-z0-9_-]{11})/);return s?s[1]:t}function Zn(e){return e.plan!=="shoot"||!e.paid_until?!1:new Date(e.paid_until)>=new Date}function je(e){return{id:e.id,name:e.name,category:e.category,tags:e.tags??[],price:e.price??"",address:e.address??"",district:e.district??"",lat:parseFloat(e.lat)||37.5326,lng:parseFloat(e.lng)||127.0246,smartPlaceUrl:e.smart_place_url??"",youtubeId:e.youtube_id??"",featured:e.featured??!1,thumbnail:e.thumbnail??"",phone:e.phone??"",desc:e.description??"",active:e.active??!0,displayMode:e.display_mode??"both",plan:e.plan??"basic",paidUntil:e.paid_until??null,paymentStatus:e.payment_status??"unpaid",paymentMemo:e.payment_memo??"",views:parseInt(e.view_cnt)||0,feedSP:parseInt(e.feed_sp)||0,mapSP:parseInt(e.map_sp)||0,isPremium:Zn(e),isRecommended:e.is_recommended??!1}}function eo(e,t,s,r){const a=(s-e)*Math.PI/180,l=(r-t)*Math.PI/180,o=Math.sin(a/2)**2+Math.cos(e*Math.PI/180)*Math.cos(s*Math.PI/180)*Math.sin(l/2)**2;return 6371*2*Math.atan2(Math.sqrt(o),Math.sqrt(1-o))}let wr=!1,ts=null;async function to(){if(!wr)return ts||(ts=(async()=>{try{await P`ALTER TABLE shops ADD COLUMN IF NOT EXISTS report_token   TEXT`}catch{}try{await P`ALTER TABLE shops ADD COLUMN IF NOT EXISTS is_recommended BOOLEAN NOT NULL DEFAULT false`}catch{}try{await P`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS feed_view    INTEGER NOT NULL DEFAULT 0`}catch{}try{await P`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS catalog_view INTEGER NOT NULL DEFAULT 0`}catch{}try{await P`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS map_view     INTEGER NOT NULL DEFAULT 0`}catch{}try{await P`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS vts_feed     INTEGER NOT NULL DEFAULT 0`}catch{}try{await P`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS vts_catalog  INTEGER NOT NULL DEFAULT 0`}catch{}try{await P`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS vts_map      INTEGER NOT NULL DEFAULT 0`}catch{}try{await P`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS rec_view     INTEGER NOT NULL DEFAULT 0`}catch{}try{await P`CREATE TABLE IF NOT EXISTS honey_items (
        ...
        created_at       TIMESTAMPTZ NOT NULL DEFAULT NOW()
      )`;try{await P`ALTER TABLE shorts_items ADD COLUMN IF NOT EXISTS smart_place_url TEXT NOT NULL DEFAULT ''`}catch{}try{await P`ALTER TABLE shorts_items ADD COLUMN IF NOT EXISTS view_cnt INTEGER NOT NULL DEFAULT 0`}catch{}try{await P`ALTER TABLE shorts_items ADD COLUMN IF NOT EXISTS sp_cnt INTEGER NOT NULL DEFAULT 0`}catch{}}catch{}wr=!0})(),ts)}j.use("*",async(e,t)=>(await to(),t()));j.get("/api/shops",async e=>{const t=e.req.query("category")??"",s=(e.req.query("q")??"").toLowerCase(),r=parseFloat(e.req.query("lat")??""),i=parseFloat(e.req.query("lng")??""),a=e.req.query("nearby")==="1",l=e.req.query("shuffle")==="1";let p=(await P`
  ...
  `;return s.length?e.json(je(s[0])):e.json({error:"not found"},404)});j.post("/api/admin/shops",async e=>{const t=await e.req.json(),s=Array.isArray(t.tags)?t.tags:(t.tags??"").split(",").map(a=>a.trim()).filter(Boolean),i=(await P`
    ...
    RETURNING *
  `)[0];return await P`INSERT INTO stats (shop_id) VALUES (${i.id}) ON CONFLICT DO NOTHING`,e.json(je(i))});j.put("/api/admin/shops/:id",async e=>{const t=+e.req.param("id"),s=await e.req.json(),r=Array.isArray(s.tags)?s.tags:(s.tags??"").split(",").map(a=>a.trim()).filter(Boolean),i=await P`
    UPDATE shops SET
      name            = ${s.name},
      category        = ${s.category},
      tags            = ${r},
    ...
    WHERE id = ${t}
    RETURNING *
  `;return i.length?(await P`INSERT INTO stats (shop_id) VALUES (${t}) ON CONFLICT DO NOTHING`,e.json(je(i[0]))):e.json({error:"not found"},404)});j.put("/api/admin/shops/:id/recommended",async e=>{const t=+e.req.param("id"),{isRecommended:s}=await e.req.json(),r=await P`
    UPDATE shops SET is_recommended = ${!!s}
    WHERE id = ${t} RETURNING *
  `;return r.length?e.json({ok:!0,isRecommended:r[0].is_recommended}):e.json({error:"not found"},404)});j.post("/api/track/rec/:id",async e=>{const t=+e.req.param("id"),s=ve();try{await P`INSERT INTO daily_stats (shop_id, stat_date, rec_view)
                VALUES (${t}, ${s}, 1)
              ON CONFLICT (shop_id, stat_date) DO UPDATE
                SET rec_view = daily_stats.rec_view + 1`}catch{}return e.json({ok:!0})});j.put("/api/admin/shops/:id/payment",async e=>{const t=+e.req.param("id"),s=await e.req.json(),r=await P`
    UPDATE shops SET
      plan           = ${s.plan??"basic"},
    ...
    WHERE id = ${t}
    RETURNING *
  `;return r.length?e.json(je(r[0])):e.json({error:"not found"},404)});j.delete("/api/admin/shops/:id",async e=>{const t=+e.req.param("id");return await P`DELETE FROM shops WHERE id = ${t}`,e.json({ok:!0})});j.post("/api/track/view/:id",async e=>{const t=+e.req.param("id"),s=ve();let r="feed";try{const i=await e.req.json();["feed","catalog","map"].includes(i==null?void 0:i.source)&&(r=i.source)}catch{}await P`INSERT INTO stats (shop_id, view_cnt) VALUES (${t}, 1)
      ...
      ON CONFLICT (shop_id) DO UPDATE SET map_sp = stats.map_sp + 1`,r==="feed"?await P`INSERT INTO daily_stats (shop_id, stat_date, map_sp, vts_feed)
              ...
              ON CONFLICT (shop_id, stat_date) DO UPDATE
                SET map_sp  = daily_stats.map_sp  + 1,
                    vts_map = daily_stats.vts_map + 1`,e.json({ok:!0})});j.post("/api/track/visit",async e=>{const t=ve();return await P`
  ...
  `,e.json({ok:!0})});j.get("/api/admin/daily-visits",async e=>{const t=await P`
    SELECT visit_date::text as visit_date, visit_cnt
    FROM daily_visits
    ORDER BY visit_date DESC
    LIMIT 30
  `;return e.json(t)});j.post("/api/admin/reset-stats",async e=>(await P`UPDATE stats SET view_cnt=0, feed_sp=0, map_sp=0`,await P`DELETE FROM daily_visits`,await P`DELETE FROM daily_stats`,e.json({ok:!0})));j.post("/api/admin/init-daily-stats",async e=>(await P`
  ...
  `,await P`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS feed_view    INTEGER NOT NULL DEFAULT 0`,await P`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS catalog_view INTEGER NOT NULL DEFAULT 0`,await P`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS map_view     INTEGER NOT NULL DEFAULT 0`,await P`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS vts_feed     INTEGER NOT NULL DEFAULT 0`,await P`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS vts_catalog  INTEGER NOT NULL DEFAULT 0`,await P`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS vts_map      INTEGER NOT NULL DEFAULT 0`,await P`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS rec_view     INTEGER NOT NULL DEFAULT 0`,await P`ALTER TABLE shops ADD COLUMN IF NOT EXISTS report_token   TEXT`,await P`ALTER TABLE shops ADD COLUMN IF NOT EXISTS is_recommended BOOLEAN NOT NULL DEFAULT false`,e.json({ok:!0})));j.get("/api/admin/stats",async e=>{const t=ve(),s=$i();await P`
    CREATE TABLE IF NOT EXISTS daily_stats (
      shop_id      INTEGER NOT NULL,
      stat_date    DATE    NOT NULL,
      view_cnt     INTEGER NOT NULL DEFAULT 0,
  ...
  `,p={};o.forEach(v=>{p[v.shop_id]=parseInt(v.total_rec)||0});const f=await P`
    SELECT
      SUM(view_cnt) as total_views,
      SUM(feed_sp)  as total_feed_sp,
      SUM(map_sp)   as total_map_sp
    FROM stats
  `,h=await P`SELECT COUNT(*) as cnt FROM shops WHERE active = true`,y=f[0]||{},b=i[0]||{},x=a[0]||{},d=await P`
    SELECT shop_id,
  ...
  `,e.json({ok:!0}))});j.get("/api/admin/calendar",async e=>{const t=parseInt(e.req.query("year")||String(new Date().getFullYear())),s=parseInt(e.req.query("month")||String(new Date().getMonth()+1)),r=`${t}-${String(s).padStart(2,"0")}-01`,i=new Date(t,s,0).toISOString().slice(0,10),a=await P`
    SELECT stat_date::text as stat_date,
           COALESCE(SUM(view_cnt),0) as views,
           COALESCE(SUM(feed_sp),0)  as feed_sp,
           COALESCE(SUM(map_sp),0)   as map_sp,
  ...
  `,y={};h.forEach(x=>{y[x.visit_date]=parseInt(x.visit_cnt)||0});const b=h.reduce((x,d)=>x+(parseInt(d.visit_cnt)||0),0);return e.json({year:t,month:s,monthTotal:{views:parseInt(f.views)||0,feedSP:parseInt(f.feed_sp)||0,mapSP:parseInt(f.map_sp)||0,visits:b},daily:a.map(x=>({date:x.stat_date,visits:y[x.stat_date]||0,views:parseInt(x.views)||0,feedSP:parseInt(x.feed_sp)||0,mapSP:parseInt(x.map_sp)||0,activeShops:parseInt(x.active_shops)||0})),shopDetail:o.map(x=>({id:x.id,name:x.name,category:x.category,thumbnail:x.thumbnail,views:parseInt(x.views)||0,feedSP:parseInt(x.feed_sp)||0,mapSP:parseInt(x.map_sp)||0}))})});j.get("/api/admin/inquiries",async e=>{const t=await P`SELECT * FROM inquiries ORDER BY created_at DESC`;return e.json(t)});j.get("/favicon.ico",e=>qi());j.get("/favicon.svg",e=>qi());function qi(e){return new Response('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32"><rect width="32" height="32" rx="8" fill="#FF4D7D"/><text x="16" y="23" font-size="18" text-anchor="middle">💄</text></svg>',{headers:{"Content-Type":"image/svg+xml","Cache-Control":"public,max-age=86400"}})}j.get("/og-image.jpg",e=>{try{const t=ra.join(process.cwd(),"public","og-image.jpg"),s=ea.readFileSync(t);return new Response(s,{headers:{"Content-Type":"image/jpeg","Cache-Control":"public,max-age=86400"}})}catch{return e.notFound()}});j.post("/api/admin/upload-thumbnail",async e=>{const t=await e.req.json(),{shopId:s,dataUrl:r}=t;return!r||!s?e.json({error:"required"},400):(await P`UPDATE shops SET thumbnail = ${r} WHERE id = ${s}`,e.json({ok:!0,url:r}))});j.get("/api/shorts",async e=>{const t=await P`
    SELECT * FROM shorts_items WHERE active = true ORDER BY sort_order ASC, id DESC
  `;return e.json(t)});j.get("/api/admin/shorts",async e=>{const t=await P`SELECT * FROM shorts_items ORDER BY sort_order ASC, id DESC`;return e.json(t)});j.post("/api/admin/shorts",async e=>{const t=await e.req.json(),s=await P`
    ...
    RETURNING *
  `;return e.json(s[0])});j.put("/api/admin/shorts/:id",async e=>{const t=+e.req.param("id"),s=await e.req.json(),r=await P`
    UPDATE shorts_items SET
      ...
      active          = ${s.active!==!1}
    WHERE id = ${t} RETURNING *
  `;return e.json(r[0])});j.delete("/api/admin/shorts/:id",async e=>{const t=+e.req.param("id");return await P`DELETE FROM shorts_items WHERE id = ${t}`,e.json({ok:!0})});async function wt(){await P`
  ...
  `}j.post("/api/track/shorts/view/:id",async e=>{const t=+e.req.param("id"),s=ve();await P`UPDATE shorts_items SET view_cnt = view_cnt + 1 WHERE id = ${t}`;try{await wt(),await P`
    ...
    `}catch{}return e.json({ok:!0})});j.post("/api/track/shorts/sp/:id",async e=>{const t=+e.req.param("id"),s=ve();await P`UPDATE shorts_items SET sp_cnt = sp_cnt + 1 WHERE id = ${t}`;try{await wt(),await P`
      INSERT INTO shorts_daily_stats (shorts_id, stat_date, sp_cnt)
      VALUES (${t}, ${s}, 1)
      ON CONFLICT (shorts_id, stat_date)
      DO UPDATE SET sp_cnt = shorts_daily_stats.sp_cnt + 1
    `}catch{}return e.json({ok:!0})});j.get("/api/admin/shorts/stats/summary",async e=>{const t=ve(),s=$i(),r=new Date(Date.now()-6*864e5).toISOString().slice(0,10);try{await wt();const[i]=await P`
    ...
    `;return e.json(s)}catch(s){return e.json({error:String(s)},500)}});j.get("/api/admin/shorts/stats/daily",async e=>{const t=new Date(Date.now()-25056e5).toISOString().slice(0,10);try{await wt();const s=await P`
      ...
      GROUP BY stat_date
      ORDER BY stat_date ASC
    `;return e.json(s)}catch(s){return e.json({error:String(s)},500)}});j.get("/api/admin/shorts/stats/item/:id",async e=>{const t=+e.req.param("id"),s=new Date(Date.now()-29*864e5).toISOString().slice(0,10);try{await wt();const r=await P`
  ...
  `;const e=["shorts_book","feed_card_cnt","feed_book_cnt","map_pin_cnt","map_book_cnt","search_cnt","inquiry_cnt"];for(const t of e)try{await P`ALTER TABLE visitor_sessions ADD COLUMN IF NOT EXISTS ${P.unsafe(t)} INTEGER NOT NULL DEFAULT 0`}catch{}}j.post("/api/track/session/start",async e=>{try{await ys(),await P`DELETE FROM visitor_sessions WHERE entered_at < NOW() - INTERVAL '7 days'`;const t=await e.req.json(),s=t.id||"",r=t.device||"unknown";return s?(await P`
    ...
    `,e.json({ok:!0})):e.json({ok:!1})}catch(t){return e.json({ok:!1,error:String(t)})}});j.post("/api/track/session/update",async e=>{try{await ys();const t=await e.req.json(),{id:s,duration_sec:r,tabs_visited:i,exited:a,shorts_count:l,shorts_book:o,feed_card_cnt:p,feed_book_cnt:f,map_pin_cnt:h,map_book_cnt:y,search_cnt:b,inquiry_cnt:x}=t;if(!s)return e.json({ok:!1});const d=(o||0)+(f||0)+(y||0);return await P`
      UPDATE visitor_sessions SET
        ...
        exited        = ${a||!1}
      WHERE id = ${s}
    `,e.json({ok:!0})}catch{return e.json({ok:!1})}});j.get("/api/admin/sessions",async e=>{try{await ys();const t=ve(),s=await P`
      ...
      LIMIT 200
    `;return e.json(s)}catch{return e.json([])}});j.get("/api/admin/sessions/summary",async e=>{try{await ys();const t=ve(),[s]=await P`
      SELECT
        COUNT(*)                                              as total,
  ...
  `;return e.json(t)});j.get("/api/admin/honey",async e=>{const t=await P`SELECT * FROM honey_items ORDER BY sort_order ASC, id DESC`;return e.json(t)});j.post("/api/admin/honey",async e=>{const t=await e.req.json(),s=await P`
    ...
    RETURNING *
  `;return e.json(s[0])});j.put("/api/admin/honey/:id",async e=>{const t=+e.req.param("id"),s=await e.req.json(),r=await P`
...
`)}
</urlset>`;return e.text(w,200,{"Content-Type":"application/xml; charset=utf-8"})});j.get("/shop/:id",async e=>{const t=e.req.header("x-forwarded-proto")||"https",s=e.req.header("x-forwarded-host")||e.req.header("host")||"www.mybeautymap.co.kr",r=`${t}://${s}`,i=+e.req.param("id");if(isNaN(i))return e.redirect("/");const a=await P`
    SELECT s.*, COALESCE(st.view_cnt,0) as view_cnt,
           COALESCE(st.feed_sp,0) as feed_sp
    FROM shops s LEFT JOIN stats st ON s.id = st.shop_id
    WHERE s.id = ${i} AND s.active = true
  `;if(!a.length)return e.redirect("/");const l=je(a[0]);return e.html(io(l,r))});j.get("/c/:category/:region",async e=>{const t=e.req.header("x-forwarded-proto")||"https",s=e.req.header("x-forwarded-host")||e.req.header("host")||"www.mybeautymap.co.kr",r=`${t}://${s}`,i=decodeURIComponent(e.req.param("category")),a=decodeURIComponent(e.req.param("region")),o=(await P`
    SELECT s.*, COALESCE(st.view_cnt,0) as view_cnt,
           COALESCE(st.feed_sp,0) as feed_sp
    FROM shops s LEFT JOIN stats st ON s.id = st.shop_id
  ...
  `).map(je);return e.html(ao(i,a,o,r))});j.post("/api/admin/shops/:id/report-token",async e=>{const t=+e.req.param("id");try{await P`ALTER TABLE shops ADD COLUMN IF NOT EXISTS report_token TEXT`}catch{}const s=Array.from({length:12},()=>Math.floor(Math.random()*16).toString(16)).join(""),r=await P`
    UPDATE shops SET report_token = ${s} WHERE id = ${t} RETURNING id, report_token
  `;return r.length?e.json({token:r[0].report_token}):e.json({error:"not found"},404)});j.post("/api/report/:token/verify",async e=>{var x,d,u,v,w,E,_,O,M,S,I,L,k,D,A,z,$,B,U,V,Y;const t=e.req.param("token"),{phone4:s}=await e.req.json(),r=await P`SELECT * FROM shops WHERE report_token = ${t}`;if(!r.length)return e.json({error:"invalid"},404);const i=r[0];if(s!=="0000"){const K=(i.phone||"").replace(/[^0-9]/g,"").slice(-4);if(!K||K!==s)return e.json({error:"wrong"},401)}ve().slice(0,7);const l=await P`
    SELECT COALESCE(SUM(view_cnt),0)     as views,
           COALESCE(SUM(feed_sp),0)      as feed_sp,
           COALESCE(SUM(map_sp),0)       as map_sp,
...
</html>`):e.text("url required",400)});j.get("/",e=>{const t=e.req.header("x-forwarded-proto")||"https",s=e.req.header("x-forwarded-host")||e.req.header("host")||"www.mybeautymap.co.kr",r=`${t}://${s}`;return e.html(ro(r))});const Er=["전체","마사지","헤드스파","피부관리","헤어","메이크업","왁싱","반영구","병원","그외"],_r={전체:"🏠",마사지:"💆",헤드스파:"🧖",피부관리:"✨",헤어:"💇",메이크업:"💄",왁싱:"🌸",반영구:"👁",병원:"🏥",그외:"🌟"},so=`<button class="cp cp-rec" onclick="filterFeed(this,'recommended')">⭐ 추천</button>`;function ro(e="https://www.mybeautymap.co.kr"){return`<!DOCTYPE html>
<html lang="ko">
<head>
<meta charset="UTF-8"/>
...
<div class="cat-bar" id="catBar">
  <div class="cat-scroll">
    ${Er.map((t,s)=>`<button class="cp${s===0?" active":""}" onclick="filterFeed(this,'${t==="전체"?"all":t}')">${_r[t]} ${t}</button>${s===0?so:""}`).join("")}
  </div>
</div>
...
</html>`}function ao(e,t,s,r){const i=Xn(e,t),a=Kn(e,t,s.length),l=Jn(e,t),o=`${r}/c/${encodeURIComponent(e)}/${encodeURIComponent(t)}`,p={마사지:"💆",헤드스파:"🧖",피부관리:"✨",헤어:"💇",메이크업:"💄",왁싱:"🌸",반영구:"👁",병원:"🏥",그외:"🌟"},f=p[e]||"🌟",h=["마사지","헤드스파","피부관리","헤어","메이크업","왁싱","반영구","병원","그외"],y=JSON.stringify({"@context":"https://schema.org","@type":"ItemList",name:i,description:a,url:o,numberOfItems:s.length,itemListElement:s.map((x,d)=>({"@type":"ListItem",position:d+1,name:x.name,url:`${r}/shop/${x.id}`,description:zi(x)}))}),b=JSON.stringify({"@context":"https://schema.org","@type":"BreadcrumbList",itemListElement:[{"@type":"ListItem",position:1,name:"마이뷰티맵",item:r},{"@type":"ListItem",position:2,name:`${t} ${e}`,item:o}]});return`<!DOCTYPE html>
<html lang="ko">
<head>
<meta charset="UTF-8"/>
<meta name="viewport" content="width=device-width,initial-scale=1.0,maximum-scale=1.0,user-scalable=no"/>
...
</html>`}var co=(e,t=uo)=>{const s=/\.([a-zA-Z0-9]+?)$/,r=e.match(s);if(!r)return;let i=t[r[1].toLowerCase()];return i&&i.startsWith("text")&&(i+="; charset=utf-8"),i},po={aac:"audio/aac",avi:"video/x-msvideo",avif:"image/avif",av1:"video/av1",bin:"application/octet-stream",bmp:"image/bmp",css:"text/css",csv:"text/csv",eot:"application/vnd.ms-fontobject",epub:"application/epub+zip",gif:"image/gif",gz:"application/gzip",htm:"text/html",html:"text/html",ico:"image/x-icon",ics:"text/calendar",jpeg:"image/jpeg",jpg:"image/jpeg",js:"text/javascript",json:"application/json",jsonld:"application/ld+json",map:"application/json",mid:"audio/x-midi",midi:"audio/x-midi",mjs:"text/javascript",mp3:"audio/mpeg",mp4:"video/mp4",mpeg:"video/mpeg",oga:"audio/ogg",ogv:"video/ogg",ogx:"application/ogg",opus:"audio/opus",otf:"font/otf",pdf:"application/pdf",png:"image/png",rtf:"application/rtf",svg:"image/svg+xml",tif:"image/tiff",tiff:"image/tiff",ts:"video/mp2t",ttf:"font/ttf",txt:"text/plain",wasm:"application/wasm",webm:"video/webm",weba:"audio/webm",webmanifest:"application/manifest+json",webp:"image/webp",woff:"font/woff",woff2:"font/woff2",xhtml:"application/xhtml+xml",xml:"application/xml",zip:"application/zip","3gp":"video/3gpp","3g2":"video/3gpp2",gltf:"model/gltf+json",glb:"model/gltf-binary"},uo=po,fo=/^\s*(?:text\/[^;\s]+|application\/(?:javascript|json|xml|xml-dtd|ecmascript|dart|postscript|rtf|tar|toml|vnd\.dart|vnd\.ms-fontobject|vnd\.ms-opentype|wasm|x-httpd-php|x-javascript|x-ns-proxy-autoconfig|x-sh|x-tar|x-virtualbox-hdd|x-virtualbox-ova|x-virtualbox-ovf|x-virtualbox-vbox|x-virtualbox-vdi|x-virtualbox-vhd|x-virtualbox-vmdk|x-www-form-urlencoded)|font\/(?:otf|ttf)|image\/(?:bmp|vnd\.adobe\.photoshop|vnd\.microsoft\.icon|vnd\.ms-dds|x-icon|x-ms-bmp)|message\/rfc822|model\/gltf-binary|x-shader\/x-fragment|x-shader\/x-vertex|[^;\s]+?\+(?:json|text|xml|yaml))(?:[;\s]|$)/i,Us={br:".br",zstd:".zst",gzip:".gz"},ho=Object.keys(Us),mo=()=>{const[e,t]=ia.node.split(".").map(s=>parseInt(s));return e>=23||e===22&&t>=7||e===20&&t>=18},go=mo(),Sr=e=>go?Ds.toWeb(e):new ReadableStream({start(s){e.on("data",r=>{s.enqueue(r)}),e.on("error",r=>{s.error(r)}),e.on("end",()=>{s.close()})},cancel(){e.destroy()}}),Os=e=>{let t;try{t=sa(e)}catch{}return t},bo=(e,t)=>{try{return t(e)}catch{return e.replace(/(?:%[0-9A-Fa-f]{2})+/g,s=>{try{return t(s)}catch{return s}})}},vo=e=>bo(e,decodeURI),st=(e={root:""})=>{const t=e.root||"",s=e.path;return t!==""&&!ta(t)&&console.error(`serveStatic: root path '${t}' is not found, are you sure it's correct?`),async(r,i)=>{var b,x,d,u;if(r.finalized)return i();let a;if(s)a=s;else try{if(a=vo(r.req.path),/(?:^|[\/\\])\.{1,2}(?:$|[\/\\])|[\/\\]{2,}/.test(a))throw new Error}catch{return await((b=e.onNotFound)==null?void 0:b.call(e,r.req.path,r)),i()}let l=pr(t,!s&&e.rewriteRequestPath?e.rewriteRequestPath(a,r):a),o=Os(l);if(o&&o.isDirectory()){const v=e.index??"index.html";l=pr(l,v),o=Os(l)}if(!o)return await((x=e.onNotFound)==null?void 0:x.call(e,l,r)),i();const p=co(l);if(r.header("Content-Type",p||"application/octet-stream"),e.precompressed&&(!p||fo.test(p))){const v=new Set((d=r.req.header("Accept-Encoding"))==null?void 0:d.split(",").map(w=>w.trim()));for(const w of ho){if(!v.has(w))continue;const E=Os(l+Us[w]);if(E){r.header("Content-Encoding",w),r.header("Vary","Accept-Encoding",{append:!0}),o=E,l=l+Us[w];break}}}let f;const h=o.size,y=r.req.header("range")||"";if(r.req.method=="HEAD"||r.req.method=="OPTIONS")r.header("Content-Length",h.toString()),r.status(200),f=r.body(null);else if(!y)r.header("Content-Length",h.toString()),f=r.body(Sr(cr(l)),200);else{r.header("Accept-Ranges","bytes"),r.header("Date",o.birthtime.toUTCString());const v=y.replace(/bytes=/,"").split("-",2),w=parseInt(v[0],10)||0;let E=parseInt(v[1],10)||h-1;h<E-w+1&&(E=h-1);const _=E-w+1,O=cr(l,{start:w,end:E});r.header("Content-Length",_.toString()),r.header("Content-Range",`bytes ${w}-${E}/${o.size}`),f=r.body(Sr(O),206)}return await((u=e.onFound)==null?void 0:u.call(e,l,r)),f}},We=class extends Error{constructor(e,t){super(e,t),this.name="RequestError"}},yo=e=>e instanceof We?e:new We(e.message,{cause:e}),xo=global.Request,Bt=class extends xo{constructor(t,s){var r;typeof t=="object"&&yt in t&&(t=t[yt]()),typeof((r=s==null?void 0:s.body)==null?void 0:r.getReader)<"u"&&(s.duplex??(s.duplex="half")),super(t,s)}},wo=e=>{const t=[],s=e.rawHeaders;for(let r=0;r<s.length;r+=2){const{[r]:i,[r+1]:a}=s;i.charCodeAt(0)!==58&&t.push([i,a])}return new Headers(t)},Hi=Symbol("wrapBodyStream"),Eo=(e,t,s,r,i)=>{const a={method:e,headers:s,signal:i.signal};if(e==="TRACE"){a.method="GET";const l=new Bt(t,a);return Object.defineProperty(l,"method",{get(){return"TRACE"}}),l}if(!(e==="GET"||e==="HEAD"))if("rawBody"in r&&r.rawBody instanceof Buffer)a.body=new ReadableStream({start(l){l.enqueue(r.rawBody),l.close()}});else if(r[Hi]){let l;a.body=new ReadableStream({async pull(o){try{l||(l=Ds.toWeb(r).getReader());const{done:p,value:f}=await l.read();p?o.close():o.enqueue(f)}catch(p){o.error(p)}}})}else a.body=Ds.toWeb(r);return new Bt(t,a)},yt=Symbol("getRequestCache"),ns=Symbol("requestCache"),os=Symbol("incomingKey"),ds=Symbol("urlKey"),Rs=Symbol("headersKey"),Pe=Symbol("abortControllerKey"),_o=Symbol("getAbortController"),Gt={get method(){return this[os].method||"GET"},get url(){return this[ds]},get headers(){return this[Rs]||(this[Rs]=wo(this[os]))},[_o](){return this[yt](),this[Pe]},[yt](){return this[Pe]||(this[Pe]=new AbortController),this[ns]||(this[ns]=Eo(this.method,this[ds],this.headers,this[os],this[Pe]))}};["body","bodyUsed","cache","credentials","destination","integrity","mode","redirect","referrer","referrerPolicy","signal","keepalive"].forEach(e=>{Object.defineProperty(Gt,e,{get(){return this[yt]()[e]}})});["arrayBuffer","blob","clone","formData","json","text"].forEach(e=>{Object.defineProperty(Gt,e,{value:function(){return this[yt]()[e]()}})});Object.defineProperty(Gt,Symbol.for("nodejs.util.inspect.custom"),{value:function(e,t,s){const r={method:this.method,url:this.url,headers:this.headers,nativeRequest:this[ns]};return`Request (lightweight) ${s(r,{...t,depth:e==null?null:e-1})}`}});Object.setPrototypeOf(Gt,Bt.prototype);var So=(e,t)=>{const s=Object.create(Gt);s[os]=e;const r=e.url||"";if(r[0]!=="/"&&(r.startsWith("http://")||r.startsWith("https://"))){if(e instanceof Ot)throw new We("Absolute URL for :path is not allowed in HTTP/2");try{const o=new URL(r);s[ds]=o.href}catch(o){throw new We("Invalid absolute URL",{cause:o})}return s}const i=(e instanceof Ot?e.authority:e.headers.host)||t;if(!i)throw new We("Missing host header");let a;if(e instanceof Ot){if(a=e.scheme,!(a==="http"||a==="https"))throw new We("Unsupported scheme")}else a=e.socket&&e.socket.encrypted?"https":"http";const l=new URL(`${a}://${i}${r}`);if(l.hostname.length!==i.length&&l.hostname!==i.replace(/:\d+$/,""))throw new We("Invalid host header");return s[ds]=l.href,s},At=Symbol("responseCache"),ct=Symbol("getResponseCache"),Ge=Symbol("cache"),tr=global.Response,qt,Me,vt,Et=(vt=class{constructor(t,s){J(this,qt);J(this,Me);let r;if(W(this,qt,t),s instanceof vt){const i=s[At];if(i){W(this,Me,i),this[ct]();return}else W(this,Me,R(s,Me)),r=new Headers(R(s,Me).headers)}else W(this,Me,s);(typeof t=="string"||typeof(t==null?void 0:t.getReader)<"u"||t instanceof Blob||t instanceof Uint8Array)&&(this[Ge]=[(s==null?void 0:s.status)||200,t,r||(s==null?void 0:s.headers)])}[ct](){return delete this[Ge],this[At]||(this[At]=new tr(R(this,qt),R(this,Me)))}get headers(){const t=this[Ge];return t?(t[2]instanceof Headers||(t[2]=new Headers(t[2]||{"content-type":"text/plain; charset=UTF-8"})),t[2]):this[ct]().headers}get status(){var t;return((t=this[Ge])==null?void 0:t[0])??this[ct]().status}get ok(){const t=this.status;return t>=200&&t<300}},qt=new WeakMap,Me=new WeakMap,vt);["body","bodyUsed","redirected","statusText","trailers","type","url"].forEach(e=>{Object.defineProperty(Et.prototype,e,{get(){return this[ct]()[e]}})});["arrayBuffer","blob","clone","formData","json","text"].forEach(e=>{Object.defineProperty(Et.prototype,e,{value:function(){return this[ct]()[e]()}})});Object.defineProperty(Et.prototype,Symbol.for("nodejs.util.inspect.custom"),{value:function(e,t,s){const r={status:this.status,headers:this.headers,ok:this.ok,nativeResponse:this[At]};return`Response (lightweight) ${s(r,{...t,depth:e==null?null:e-1})}`}});Object.setPrototypeOf(Et,tr);Object.setPrototypeOf(Et.prototype,tr.prototype);async function ko(e){return Promise.race([e,Promise.resolve().then(()=>Promise.resolve(void 0))])}function Vi(e,t,s){const r=o=>{e.cancel(o).catch(()=>{})};return t.on("close",r),t.on("error",r),(s??e.read()).then(l,i),e.closed.finally(()=>{t.off("close",r),t.off("error",r)});function i(o){o&&t.destroy(o)}function a(){e.read().then(l,i)}function l({done:o,value:p}){try{if(o)t.end();else if(!t.write(p))t.once("drain",a);else return e.read().then(l,i)}catch(f){i(f)}}}function To(e,t){if(e.locked)throw new TypeError("ReadableStream is locked.");return t.destroyed?void 0:Vi(e.getReader(),t)}var zs=e=>{const t={};e instanceof Headers||(e=new Headers(e??void 0));const s=[];for(const[r,i]of e)r==="set-cookie"?s.push(i):t[r]=i;return s.length>0&&(t["set-cookie"]=s),t["content-type"]??(t["content-type"]="text/plain; charset=UTF-8"),t},Co="x-hono-already-sent";typeof global.crypto>"u"&&(global.crypto=oa);var sr=Symbol("outgoingEnded"),kr=Symbol("incomingDraining"),Io=500,Lo=64*1024*1024,Ms=e=>{var o,p,f;const t=e;if(e.destroyed||t[kr])return;if(t[kr]=!0,e instanceof Ot){try{(p=(o=e.stream)==null?void 0:o.close)==null||p.call(o,na.NGHTTP2_NO_ERROR)}catch{}return}let s=0;const r=()=>{clearTimeout(a),e.off("data",l),e.off("end",r),e.off("error",r)},i=()=>{r();const h=e.socket;h&&!h.destroyed&&h.destroySoon()},a=setTimeout(i,Io);(f=a.unref)==null||f.call(a);const l=h=>{s+=h.length,s>Lo&&i()};e.on("data",l),e.on("end",r),e.on("error",r),e.resume()},Ao=()=>new Response(null,{status:400}),Wi=e=>new Response(null,{status:e instanceof Error&&(e.name==="TimeoutError"||e.constructor.name==="TimeoutError")?504:500}),$s=(e,t)=>{const s=e instanceof Error?e:new Error("unknown error",{cause:e});s.code==="ERR_STREAM_PREMATURE_CLOSE"?console.info("The user aborted a request."):(console.error(e),t.headersSent||t.writeHead(500,{"Content-Type":"text/plain"}),t.end(`Error: ${s.message}`),t.destroy(s))},Gi=e=>{"flushHeaders"in e&&e.writable&&e.flushHeaders()},Qi=async(e,t)=>{var l,o;let[s,r,i]=e[Ge],a=!1;if(!i)i={"content-type":"text/plain; charset=UTF-8"};else if(i instanceof Headers)a=i.has("content-length"),i=zs(i);else if(Array.isArray(i)){const p=new Headers(i);a=p.has("content-length"),i=zs(p)}else for(const p in i)if(p.length===14&&p.toLowerCase()==="content-length"){a=!0;break}a||(typeof r=="string"?i["Content-Length"]=Buffer.byteLength(r):r instanceof Uint8Array?i["Content-Length"]=r.byteLength:r instanceof Blob&&(i["Content-Length"]=r.size)),t.writeHead(s,i),typeof r=="string"||r instanceof Uint8Array?t.end(r):r instanceof Blob?t.end(new Uint8Array(await r.arrayBuffer())):(Gi(t),await((l=To(r,t))==null?void 0:l.catch(p=>$s(p,t)))),(o=t[sr])==null||o.call(t)},Oo=e=>typeof e.then=="function",Ro=async(e,t,s={})=>{var i;if(Oo(e))if(s.errorHandler)try{e=await e}catch(a){const l=await s.errorHandler(a);if(!l)return;e=l}else e=await e.catch(Wi);if(Ge in e)return Qi(e,t);const r=zs(e.headers);if(e.body){const a=e.body.getReader(),l=[];let o=!1,p;if(r["transfer-encoding"]!=="chunked"){let f=2;for(let h=0;h<f;h++){p||(p=a.read());const y=await ko(p).catch(b=>{console.error(b),o=!0});if(!y){if(h===1){await new Promise(b=>setTimeout(b)),f=3;continue}break}if(p=void 0,y.value&&l.push(y.value),y.done){o=!0;break}}o&&!("content-length"in r)&&(r["content-length"]=l.reduce((h,y)=>h+y.length,0))}t.writeHead(e.status,r),l.forEach(f=>{t.write(f)}),o?t.end():(l.length===0&&Gi(t),await Vi(a,t,p))}else r[Co]||(t.writeHead(e.status,r),t.end());(i=t[sr])==null||i.call(t)},Mo=(e,t={})=>{const s=t.autoCleanupIncoming??!0;return t.overrideGlobalObjects!==!1&&global.Request!==Bt&&(Object.defineProperty(global,"Request",{value:Bt}),Object.defineProperty(global,"Response",{value:Et})),async(r,i)=>{let a,l;try{l=So(r,t.hostname);let o=!s||r.method==="GET"||r.method==="HEAD";if(o||(r[Hi]=!0,r.on("end",()=>{o=!0}),r instanceof Ot&&(i[sr]=()=>{o||setTimeout(()=>{o||setTimeout(()=>{Ms(r)})})}),i.on("finish",()=>{o||Ms(r)})),i.on("close",()=>{l[Pe]&&(r.errored?l[Pe].abort(r.errored.toString()):i.writableFinished||l[Pe].abort("Client connection prematurely closed.")),o||setTimeout(()=>{o||setTimeout(()=>{Ms(r)})})}),a=e(l,{incoming:r,outgoing:i}),Ge in a)return Qi(a,i)}catch(o){if(a)return $s(o,i);if(t.errorHandler){if(a=await t.errorHandler(l?o:yo(o)),!a)return}else l?a=Wi(o):a=Ao()}try{return await Ro(a,i,t)}catch(o){return $s(o,i)}}},Do=e=>{const t=e.fetch,s=Mo(t,{hostname:e.hostname,overrideGlobalObjects:e.overrideGlobalObjects,autoCleanupIncoming:e.autoCleanupIncoming});return(e.createServer||aa)(e.serverOptions||{},s)},Po=(e,t)=>{const s=Do(e);return s.listen(e==null?void 0:e.port,e.hostname,()=>{s.address()}),s};const Te=new Gr;Te.use("/favicon.svg",st({root:"./"}));Te.use("/og-image.jpg",st({root:"./"}));Te.use("/static/*",st({root:"./"}));Te.use("/.gitignore",st({root:"./"}));Te.use("/.vercel/*",st({root:"./"}));Te.use("/_routes.json",st({root:"./"}));Te.use("/_worker.js",st({root:"./"}));const Bo=Object.assign({"/src/index.tsx":j});let Yi=!1;for(const[,e]of Object.entries(Bo))e&&(Te.all("*",t=>{let s;try{s=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,s)}),Te.notFound(t=>{let s;try{s=t.executionCtx}catch{}return e.fetch(t.req.raw,t.env,s)}),Yi=!0);if(!Yi)throw new Error("Can't import modules from ['/src/index.ts','/src/index.tsx','/app/server.ts']");Po({fetch:Te.fetch,port:3e3});export{Te as default};
