...
*/const j=new Qr,Qn=process.env.DATABASE_URL||"postgresql://neondb_owner:npg_1PBkOmAWRcf2@ep-round-recipe-aqdgkjfj-pooler.c-8.us-east-1.aws.neon.tech/neondb?sslmode=require",R=Zs(Qn);function ys(e){if(!e)return"";const t=e.split(/\s+/);for(let s=1;s<t.length;s++)if(t[s].endsWith("구")&&!t[s].endsWith("시구"))return t[s].replace(/구$/,"");for(let s=1;s<t.length;s++)if(t[s].endsWith("시")&&t[s]!==t[0])return t[s].replace(/(특별시|광역시|특별자치시|시)$/,"");return t[0]?t[0].replace(/(특별자치도|도)$/,""):""}function er(e){if(!e)return"";const t=e.split(/\s+/);return t[0]?t[0].replace(/(특별시|광역시|특별자치시|특별자치도|도|시)$/,""):""}function Yn(e){return{마사지:"MassageTherapist",헤드스파:"BeautySalon",피부관리:"BeautySalon",헤어:"HairSalon",메이크업:"BeautySalon",왁싱:"BeautySalon",반영구:"BeautySalon",병원:"MedicalBusiness",그외:"LocalBusiness"}[e]||"LocalBusiness"}function Xn(e){const t=ys(e.address),s=er(e.address),r=t||s;return`${e.name} ${r?r+" ":""}${e.category} 추천 | 마이뷰티맵`}function ji(e){const t=ys(e.address),s=er(e.address),r=Array.isArray(e.tags)&&e.tags.length?e.tags.join("·")+" ":"",i=t||s,a=e.price?` 가격 ${e.price}.`:"",d=e.desc?" "+e.desc.slice(0,40):"";return`${e.name} | ${i?i+" ":""}${e.category} 잘하는 곳. ${r}예약·위치·가격 한눈에 확인!${a}${d}`.slice(0,160)}function Kn(e,t){return`${t} ${e} 추천 TOP | 마이뷰티맵`}function Jn(e,t,s){return`${t} ${e} 잘하는 곳 ${s}곳 추천! 가격·위치·예약·후기까지 마이뷰티맵에서 한눈에 확인하세요.`}function Zn(e,t){const s=t;return[`${s} ${e}`,`${s} ${e} 추천`,`${s} ${e} 잘하는 곳`,`${s} ${e} 예약`,`${s} ${e} 가격`,`${s} ${e} 후기`,`${s} ${e} 맛집`,`${s} 뷰티샵`,`${s} 뷰티 추천`,`${e} 추천`].join(",")}function ve(){return new Date(Date.now()+540*60*1e3).toISOString().slice(0,10)}function $i(){return new Date(Date.now()+540*60*1e3-864e5).toISOString().slice(0,10)}function qi(e){const t=(e??"").trim();if(!t)return"";if(/^[A-Za-z0-9_-]{11}$/.test(t))return t;const s=t.match(/(?:youtu\.be\/|youtube\.com\/(?:watch\?(?:.*&)?v=|embed\/|shorts\/|v\/))([A-Za-z0-9_-]{11})/);return s?s[1]:t}function eo(e){return e.plan!=="shoot"||!e.paid_until?!1:new Date(e.paid_until)>=new Date}function $e(e){return{id:e.id,name:e.name,category:e.category,tags:e.tags??[],price:e.price??"",address:e.address??"",district:e.district??"",lat:parseFloat(e.lat)||37.5326,lng:parseFloat(e.lng)||127.0246,smartPlaceUrl:e.smart_place_url??"",youtubeId:e.youtube_id??"",featured:e.featured??!1,thumbnail:e.thumbnail??"",phone:e.phone??"",desc:e.description??"",active:e.active??!0,displayMode:e.display_mode??"both",plan:e.plan??"basic",paidUntil:e.paid_until??null,paymentStatus:e.payment_status??"unpaid",paymentMemo:e.payment_memo??"",views:parseInt(e.view_cnt)||0,feedSP:parseInt(e.feed_sp)||0,mapSP:parseInt(e.map_sp)||0,isPremium:eo(e),isRecommended:e.is_recommended??!1}}function to(e,t,s,r){const a=(s-e)*Math.PI/180,d=(r-t)*Math.PI/180,o=Math.sin(a/2)**2+Math.cos(e*Math.PI/180)*Math.cos(s*Math.PI/180)*Math.sin(d/2)**2;return 6371*2*Math.atan2(Math.sqrt(o),Math.sqrt(1-o))}let _r=!1,ss=null;async function so(){if(!_r)return ss||(ss=(async()=>{try{await R`ALTER TABLE shops ADD COLUMN IF NOT EXISTS report_token   TEXT`}catch{}try{await R`ALTER TABLE shops ADD COLUMN IF NOT EXISTS is_recommended BOOLEAN NOT NULL DEFAULT false`}catch{}try{await R`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS feed_view    INTEGER NOT NULL DEFAULT 0`}catch{}try{await R`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS catalog_view INTEGER NOT NULL DEFAULT 0`}catch{}try{await R`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS map_view     INTEGER NOT NULL DEFAULT 0`}catch{}try{await R`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS vts_feed     INTEGER NOT NULL DEFAULT 0`}catch{}try{await R`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS vts_catalog  INTEGER NOT NULL DEFAULT 0`}catch{}try{await R`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS vts_map      INTEGER NOT NULL DEFAULT 0`}catch{}try{await R`ALTER TABLE daily_stats ADD COLUMN IF NOT EXISTS rec_view     INTEGER NOT NULL DEFAULT 0`}catch{}try{await R`CREATE TABLE IF NOT EXISTS honey_items (
        ...
        created_at       TIMESTAMPTZ NOT NULL DEFAULT NOW()
      )`;try{await R`ALTER TABLE shorts_items ADD COLUMN IF NOT EXISTS smart_place_url TEXT NOT NULL DEFAULT ''`}catch{}try{await R`ALTER TABLE shorts_items ADD COLUMN IF NOT EXISTS view_cnt INTEGER NOT NULL DEFAULT 0`}catch{}try{await R`ALTER TABLE shorts_items ADD COLUMN IF NOT EXISTS sp_cnt INTEGER NOT NULL DEFAULT 0`}catch{}}catch{}_r=!0})(),ss)}j.use("*",async(e,t)=>(await so(),t()));j.get("/api/shops",async e=>{const t=e.req.query("category")??"",s=(e.req.query("q")??"").toLowerCase(),r=parseFloat(e.req.query("lat")??""),i=parseFloat(e.req.query("lng")??""),a=e.req.query("nearby")==="1",d=e.req.query("shuffle")==="1";let p=(await R`
  ...
  `;return e.json(s[0])});j.put("/api/admin/shorts/:id",async e=>{const t=+e.req.param("id"),s=await e.req.json(),r=await R`
    UPDATE shorts_items SET
      ...
      active          = ${s.active!==!1}
    WHERE id = ${t} RETURNING *
  `;return e.json(r[0])});j.delete("/api/admin/shorts/:id",async e=>{const t=+e.req.param("id");return await R`DELETE FROM shorts_items WHERE id = ${t}`,e.json({ok:!0})});async function xt(){await R`
  ...
  `}j.post("/api/track/shorts/view/:id",async e=>{const t=+e.req.param("id"),s=ve();await R`UPDATE shorts_items SET view_cnt = view_cnt + 1 WHERE id = ${t}`;try{await xt(),await R`
    ...
    `}catch{}return e.json({ok:!0})});j.post("/api/track/shorts/sp/:id",async e=>{const t=+e.req.param("id"),s=ve();await R`UPDATE shorts_items SET sp_cnt = sp_cnt + 1 WHERE id = ${t}`;try{await xt(),await R`
      INSERT INTO shorts_daily_stats (shorts_id, stat_date, sp_cnt)
      VALUES (${t}, ${s}, 1)
      ON CONFLICT (shorts_id, stat_date)
      DO UPDATE SET sp_cnt = shorts_daily_stats.sp_cnt + 1
    `}catch{}return e.json({ok:!0})});j.get("/api/admin/shorts/stats/summary",async e=>{const t=ve(),s=$i(),r=new Date(Date.now()-6*864e5).toISOString().slice(0,10);try{await xt();const[i]=await R`
    ...
    `;return e.json(s)}catch(s){return e.json({error:String(s)},500)}});j.get("/api/admin/shorts/stats/daily",async e=>{const t=new Date(Date.now()-25056e5).toISOString().slice(0,10);try{await xt();const s=await R`
      ...
      GROUP BY stat_date
      ORDER BY stat_date ASC
    `;return e.json(s)}catch(s){return e.json({error:String(s)},500)}});j.get("/api/admin/shorts/stats/item/:id",async e=>{const t=+e.req.param("id"),s=new Date(Date.now()-29*864e5).toISOString().slice(0,10);try{await xt();const r=await R`
...
Sitemap: ${t}://${s}/sitemap.xml`,200,{"Content-Type":"text/plain; charset=utf-8"})});j.get("/ads.txt",e=>e.text("google.com, pub-6943282483618134, DIRECT, f08c47fec0942fa0",200,{"Content-Type":"text/plain; charset=utf-8"}));j.get("/sitemap.xml",async e=>{const t=e.req.header("x-forwarded-proto")||"https",s=e.req.header("x-forwarded-host")||e.req.header("host")||"www.mybeautymap.co.kr",r=`${t}://${s}`,i=ve(),a=await R`SELECT id, name, category, address FROM shops WHERE active = true ORDER BY id`,d=new Set;for(const _ of a){const E=ys(_.address);E&&d.add(`${_.category}|||${E}`)}const o=[{loc:r,priority:"1.0",freq:"daily"},{loc:`${r}/map`,priority:"0.8",freq:"weekly"}],p=a.map(_=>({loc:`${r}/shop/${_.id}`,priority:"0.9",freq:"weekly",lastmod:i})),f=[...d].map(_=>{const[E,O]=_.split("|||");return{loc:`${r}/c/${encodeURIComponent(E)}/${encodeURIComponent(O)}`,priority:"0.8",freq:"weekly",lastmod:i}}),h=["마사지","헤드스파","피부관리","헤어","메이크업","왁싱","반영구"],y=["강남구","서초구","마포구","용산구","성동구","종로구","중구","송파구","강서구","분당구"],b=h.flatMap(_=>y.map(E=>({loc:`${r}/c/${encodeURIComponent(_)}/${encodeURIComponent(E)}`,priority:"0.7",freq:"weekly",lastmod:i}))),x=new Set(f.map(_=>_.loc)),l=b.filter(_=>!x.has(_.loc)),w=`<?xml version="1.0" encoding="UTF-8"?>
...
</html>`):e.text("url required",400)});j.get("/",e=>{const t=e.req.header("x-forwarded-proto")||"https",s=e.req.header("x-forwarded-host")||e.req.header("host")||"www.mybeautymap.co.kr",r=`${t}://${s}`;return e.html(io(r))});const Er=["전체","마사지","헤드스파","피부관리","헤어","메이크업","왁싱","반영구","병원","그외"],Sr={전체:"🏠",마사지:"💆",헤드스파:"🧖",피부관리:"✨",헤어:"💇",메이크업:"💄",왁싱:"🌸",반영구:"👁",병원:"🏥",그외:"🌟"},ro=`<button class="cp cp-rec" onclick="filterFeed(this,'recommended')">⭐ 추천</button>`;function io(e="https://www.mybeautymap.co.kr"){return`<!DOCTYPE html>
<html lang="ko">
<head>
<meta charset="UTF-8"/>
...
<div class="cat-bar" id="catBar">
  <div class="cat-scroll">
    ${Er.map((t,s)=>`<button class="cp${s===0?" active":""}" onclick="filterFeed(this,'${t==="전체"?"all":t}')">${Sr[t]} ${t}</button>${s===0?ro:""}`).join("")}
  </div>
</div>
